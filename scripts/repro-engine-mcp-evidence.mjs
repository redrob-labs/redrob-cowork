...
async function bootstrapDen(denApiUrl) {
  ...
  const password = `Redrob Cowork-${runTag}-Evidence!`;
...
async function waitForServerListening(child, logStream) {
  let stdout = "";
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("Timed out waiting for Redrob Cowork server listening log")), 90_000);
    const finish = (value) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (stdout.includes("Redrob Cowork server listening")) finish();
    });
    child.stderr.on("data", (chunk) => writeLog(logStream, chunk));
    child.once("error", fail);
    child.once("exit", (code, signal) => fail(new Error(`Redrob Cowork server exited before listening (code ${code}, signal ${signal})`)));
  });
}

async function startRedrobServer(paths, serverPort, opencodeBin) {
  await mkdir(paths.workspaceRoot, { recursive: true });
  await mkdir(paths.xdgRedrob, { recursive: true });
  ...
  log(`Starting Redrob Cowork server on 127.0.0.1:${serverPort}`);
  const child = spawn("bun", args, {
    ...
    env: {
      ...
      REDROB_SERVER_CONFIG: join(paths.xdgRedrob, "server.json"),
  ...
  log(`Redrob Cowork server log: ${serverLog}`);
...
async function pollHealedHealth(baseUrl, workspaceId) {
  ...
  while (Date.now() - started < 30_000) {
    last = await serverJson(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/health`);
...
async function main() {
  ...
  const paths = { workspaceRoot: join(reproDir, "ws"), xdg: join(reproDir, "xdg"), xdgRedrob: join(reproDir, "xdg", "redrob"), home: join(reproDir, "home"), logs: join(reproDir, "logs") };

  const den = await bootstrapDen(denApiUrl);
  const proxy = await startDelayProxy(denApiUrl, proxyPort);
  const redrob = await startRedrobServer(paths, serverPort, opencodeBin);
  const workspaces = await serverJson(redrob.baseUrl, "/workspaces");
  ...
  log("PHASE 1: seeding delayed redrob-cloud reconcile");
  ...
  const seedReconcile = await serverJson(redrob.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/reconcile`, {
    method: "POST",
    body: JSON.stringify(reconcilePayload),
  });
  const seedMcp = await serverJson(redrob.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp`);
  const seedHealth = await serverJson(redrob.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/health`);

  log("PHASE 2: waiting for delayed Den MCP handshake to heal live engine state");
  await sleep(armedUntilMs + 8000 - Date.now());
  const healedHealth = await pollHealedHealth(redrob.baseUrl, workspaceId);
  const healedMcp = await serverJson(redrob.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/mcp`);
  ...
  const diagnosticsReport = await serverJson(redrob.baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/diagnostics/agent-context`, {
    method: "POST",
  ...
  const redrobCloudSyncStatuses = mcps.filter((mcp) => own(mcp, "name") === "redrob-cloud").map((mcp) => ({
  ...
  const evidence = {
    ...
    phases: {
      ...
      diagnostics: { engineMcpSyncCheck, redrobCloudSyncStatuses, firstFailedCheck: own(diagnosticsReport, "firstFailedCheck") ?? null, overall: own(diagnosticsReport, "overall") ?? null },
    },
