...
const outputDir = resolve(process.env.RUNNER_TEMP || ".", "redrob-electron-manifests");
...
function isReleaseAsset(path) {
  ...
  if (!basename(path).startsWith("redrob-")) return false;
...
function validateManifest(name, manifest) {
  ...
  const expectedPrefix = distribution === "public" ? "redrob-" : `redrob-${distribution}-`;
  if (
    urls.some((url) => !url.startsWith(expectedPrefix))
    || (distribution === "public" && urls.some((url) => /^redrob-(?:cloud|enterprise)-/.test(url)))
  ) {
