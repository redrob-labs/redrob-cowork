...
assert.ok(
  cloudDocs.includes("`app.redrob.io/api/den` is an internal same-origin desktop proxy"),
  "Cloud MCP docs must describe app.redrob.io/api/den as an internal same-origin desktop proxy",
);
assert.ok(cloudDocs.includes("https://app.redrob.io/api/auth"), "Cloud MCP docs are missing the auth server origin");
