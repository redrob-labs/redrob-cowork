...
#   kill-orphans    remove orphan redrob/opencode processes (ppid == launchd)
...
#   wait-healthy    block until redrob-server reports /health = 200
...
#   4. opencode sidecars and redrob-server helpers
#
...
DEV_LOG_FILE="${REDROB_DEV_LOG_FILE:-$HOME/.redrob/debug/redrob-dev.log}"
PNPM_DEV_LOG="${REDROB_PNPM_DEV_LOG:-/tmp/redrob-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${REDROB_PNPM_DEV_PID:-/tmp/redrob-test/pnpm-dev.pid}"
...
discover_redrob_server_port() {
  ps -Ao command \
    | grep -E "redrob-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.redrob/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|apps\/desktop\/resources\/sidecars\/opencode( |\/)|redrob-server|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== redrob-server ==="
  local port
  port=$(discover_redrob_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev redrob-server running)"
  else
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /redrob-server|opencode( |\/)/' | head
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /redrob-server|opencode( |\/)/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  port=$(discover_redrob_server_port)
  if [[ -n "$port" ]]; then
    echo "  redrob-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no redrob-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/redrob-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "redrob-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "redrob-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the redrob-workspaces.json registry or server-side tokens.
