...
1. Fetch `origin/main`. Work only from a clean checkout based on its current
   tip. If the user's checkout is dirty or `main` is already checked out, create
...
3. Run `pnpm release:ship`. A rejected direct push to protected `main` is
   expected: `release:ship` must push a `release/vX.Y.Z-main-sync` branch and
   open the backfill PR. Never bypass or force-push `main`.
...
6. Verify the public release assets resolve, `npm view redrob-server version`
