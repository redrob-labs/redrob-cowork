...
const paths = {
  ...
  models: path.resolve("ee/apps/inference/src/models/redrob-models.json"),
  aliases: path.resolve("packages/types/src/den/inference.ts"),
}

function usage() {
  console.log(`Usage:
  redrob-models.mjs search <query>
  redrob-models.mjs add <query-or-id>
  redrob-models.mjs remove <id> [<id2>]
  redrob-models.mjs discount <usageFactor> <id> [<id2>]
  redrob-models.mjs sync
  redrob-models.mjs validate`)
}
...
async function readRedrobModels() {
...
function resolveExactRedrobIds(redrobModels, ids) {
  const resolved = []
  for (const id of ids) {
    if (redrobModels[id]) {
      resolved.push(id)
    ...
    const matches = findMatches(redrobModels, id)
    console.error(`Redrob Work model "${id}" was not exact. Please clarify with exact id:`)
    displayMatches(matches)
    process.exitCode = 2
    return null
  }
...
async function syncAliases(models, usageOverrides = new Map()) {
  ...
  const entries = Object.entries(models).map(([id, model]) => {
    ...
    const displayName = `Redrob Work: ${model.name}`
...
async function validate() {
  const models = await readRedrobModels()
  ...
  console.log(`validated ${Object.keys(models).length} Redrob Work model(s)`)
}
...
async function remove(ids) {
  const models = await readRedrobModels()
  const resolved = resolveExactRedrobIds(models, ids)
  if (!resolved) return
...
if (command === "search") {
  ...
  await syncAll(await readRedrobModels())
