---
name: redrob-models
description: Manage Redrob Work inference model aliases, redrob model overlays, discounts, validation, and automated base model refreshes from models.dev. Use when adding, removing, discounting, auditing, or updating Redrob Work models, including requests like "update the models" that should trigger the GitHub update-models workflow and report when its PR merges.
---
...
- `ee/apps/inference/src/models/redrob-models.json`

Managed file:

- `packages/types/src/den/inference.ts`

`ee/apps/inference/scripts/build-models.mjs` reads `redrob-models.json` and
...
```bash
node .opencode/skills/redrob-models/scripts/extract-source-models.mjs
node .opencode/skills/redrob-models/scripts/extract-source-models.mjs --query "zai 5.1"
```

Manage Redrob Work models:

```bash
node .opencode/skills/redrob-models/scripts/redrob-models.mjs search "zai 5.1"
node .opencode/skills/redrob-models/scripts/redrob-models.mjs add "z-ai/glm-5.1"
node .opencode/skills/redrob-models/scripts/redrob-models.mjs remove "z-ai/glm-5.1"
node .opencode/skills/redrob-models/scripts/redrob-models.mjs discount 0.1 "z-ai/glm-5.1"
node .opencode/skills/redrob-models/scripts/redrob-models.mjs sync
node .opencode/skills/redrob-models/scripts/redrob-models.mjs validate
```

Trigger the remote base model refresh workflow:

```bash
node .opencode/skills/redrob-models/scripts/trigger-update-workflow.mjs
node .opencode/skills/redrob-models/scripts/trigger-update-workflow.mjs --base dev --ref dev
```
...
1. Run `extract-source-models.mjs --query "<user text>"` or
   `redrob-models.mjs search "<user text>"`.
...
3. If there are multiple plausible matches, list the matching IDs and ask which
   one(s) to add.
4. Once exact IDs are known, run `redrob-models.mjs add "<id>"` for each ID.
   The script copies the full model block from `openrouter.models`, adds it to
   `redrob-models.json`, syncs aliases, and validates.
...
"model/id": {
  ...
  displayName: "Redrob Work: " + model.name,
  enabled: true,
  usageFactor: 1,
}
```
...
2. Run `redrob-models.mjs remove "<id>"`.
3. The script removes the model from `redrob-models.json`, removes the alias
...
```bash
node .opencode/skills/redrob-models/scripts/redrob-models.mjs discount <factor> "<id>" ["<id2>"]
```
...
Validation checks JSON validity, Redrob Work model ID consistency, and alias
