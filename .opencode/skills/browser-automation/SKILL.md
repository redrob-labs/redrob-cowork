...
description: Local Redrob Cowork Electron browser automation with CDP. Use when driving a local Electron dev app, browser_list, browser_snapshot, browser_eval, composer automation, or local UI smoke tests.
---
...
- Attach OpenCode browser tools to the Redrob Cowork Electron app during local development.
...
```sh
nohup pnpm dev > /var/folders/d9/xqhkvsp94rg0n0n523snqztm0000gn/T/opencode/redrob-dev.log 2>&1 &
```
...
2. Select the `Redrob Cowork` target ID.
...
Use this `browser_eval` pattern after selecting the Redrob Cowork target:
...
- Electron CDP is used for development test tooling. User browser tasks should use the built-in Redrob Cowork Browser target.
- A successful local attach should show a `Redrob Cowork` target at `http://127.0.0.1:9823`.
