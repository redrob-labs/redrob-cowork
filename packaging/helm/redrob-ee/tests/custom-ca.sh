...
assert_failure() {
  ...
  if helm template redrob-ee "$chart_dir" -f "$values_file" > "$output_file" 2> "$error_file"; then
...
helm template redrob-ee "$chart_dir" --set inference.enabled=true > "$default_rendered"
...
customCa:
  ...
  key: ignored.pem
YAML
helm template redrob-ee "$chart_dir" -f "$disabled_values" > "$disabled_rendered"
cmp -s "$default_rendered" "$disabled_rendered"
assert_not_contains "$default_rendered" 'name: NODE_EXTRA_CA_CERTS'
assert_not_contains "$default_rendered" '/etc/redrob/custom-ca'
...
customCa:
  enabled: true
  existingSecret: redrob-ca-secret
  key: corp-root.pem
YAML
helm template redrob-ee "$chart_dir" -f "$secret_values" > "$secret_rendered"
assert_count "$secret_rendered" 'name: NODE_EXTRA_CA_CERTS' 4
assert_count "$secret_rendered" 'value: "/etc/redrob/custom-ca/ca-bundle.pem"' 5
assert_count "$secret_rendered" 'name: custom-ca' 8
assert_count "$secret_rendered" 'secretName: "redrob-ca-secret"' 4
assert_count "$secret_rendered" 'key: "corp-root.pem"' 4
assert_count "$secret_rendered" 'path: ca-bundle.pem' 4
assert_count "$secret_rendered" 'mountPath: "/etc/redrob/custom-ca"' 4
assert_not_contains "$secret_rendered" 'subPath:'
assert_not_contains "$secret_rendered" 'name: "redrob-ca-config"'
assert_not_contains "$secret_rendered" 'name: redrob-ee-inference'
...
customCa:
  enabled: true
  existingConfigMap: redrob-ca-config
  key: ca.crt
YAML
helm template redrob-ee "$chart_dir" -f "$configmap_values" > "$configmap_rendered"
assert_contains "$configmap_rendered" 'name: redrob-ee-den-api'
assert_contains "$configmap_rendered" 'name: redrob-ee-den-web'
assert_contains "$configmap_rendered" 'name: redrob-ee-inference'
assert_contains "$configmap_rendered" 'name: redrob-ee-migrate'
assert_count "$configmap_rendered" 'name: NODE_EXTRA_CA_CERTS' 5
assert_count "$configmap_rendered" 'value: "/etc/redrob/custom-ca/ca-bundle.pem"' 6
assert_count "$configmap_rendered" 'name: custom-ca' 10
assert_count "$configmap_rendered" 'name: "redrob-ca-config"' 5
assert_count "$configmap_rendered" 'key: "ca.crt"' 5
assert_count "$configmap_rendered" 'path: ca-bundle.pem' 5
assert_count "$configmap_rendered" 'mountPath: "/etc/redrob/custom-ca"' 5
assert_not_contains "$configmap_rendered" 'subPath:'
assert_not_contains "$configmap_rendered" 'secretName: "redrob-ca-secret"'
...
YAML
helm template redrob-ee "$chart_dir" -f "$installer_values" > "$installer_rendered"
