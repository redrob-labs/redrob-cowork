...
const containerName = `redrob-custom-ca-mysql-${process.pid}-${Date.now()}`;
const tmp = mkdtempSync(join(tmpdir(), "redrob-custom-ca-mysql-"));
...
const databaseName = "redrob_den";
const databaseUser = "redrob";
const databasePassword = "redrob_tls_test_password";
...
`,
  ...
  runOpenSsl(["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", "/CN=Redrob Work Test MySQL CA", "-keyout", caKey, "-out", caCert]);
...
function startMysqlContainer() {
  const result = docker([
    ...
    "127.0.0.1::3306",
    "--env",
    "MYSQL_ROOT_PASSWORD=redrob_root_password",
    "--env",
    "MYSQL_ROOT_HOST=%",
    "--env",
...
function runRedrobWorkMysqlQuery(databaseUrl, query, extraEnv = {}) {
  const source = `
import { createRequire } from "node:module";
...
function runBootstrap(databaseUrl) {
  return run(process.execPath, [bootstrapPath], {
    ...
    env: childEnv({
      ...
      DEN_DB_ENCRYPTION_KEY: "redrob-custom-ca-mysql-tls-test-key-1234567890",
...
try {
  ...
  const withoutCa = runRedrobWorkMysqlQuery(databaseUrl, "select 1 as ok");
  if (withoutCa.status === 0) {
    throw new Error("Strict Redrob Work/mysql2 unexpectedly connected without NODE_EXTRA_CA_CERTS");
  }
  const withoutCaError = `${withoutCa.stdout}\n${withoutCa.stderr}`.trim();
  if (!/certificate|verify|self-signed|unable/i.test(withoutCaError)) {
    throw new Error(`Strict Redrob Work/mysql2 failed for a non-certificate reason:\n${withoutCaError}`);
  }
  ...
  console.log(`Strict Redrob Work/mysql2 without custom CA: rejected (${certificateError})`);

  const bootstrap = runBootstrap(databaseUrl);
  ensureSuccess(bootstrap, "Redrob Work migration bootstrap with NODE_EXTRA_CA_CERTS");
  console.log("Redrob Work migration bootstrap completed with NODE_EXTRA_CA_CERTS and strict DATABASE_URL");
  console.log(bootstrap.stdout.trim());

  const query = runRedrobWorkMysqlQuery(
    databaseUrl,
  ...
  ensureSuccess(query, "Redrob Work/mysql2 strict post-migration query with NODE_EXTRA_CA_CERTS");
  ...
  if (tableCount <= 0 || migrationCount <= 0) {
    throw new Error(`Expected migrated Redrob Work tables and migration ledger rows, got ${query.stdout}`);
  }
  console.log(`Redrob Work/mysql2 strict query succeeded after migration: tables=${tableCount}, migrations=${migrationCount}`);
} finally {
  cleanup();
}
