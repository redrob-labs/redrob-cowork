...
YAML
helm template redrob-ee "$chart_dir" -f "$otel_values" > "$otel_rendered"
...
YAML
helm template redrob-ee "$chart_dir" -f "$legacy_values" > "$legacy_rendered"
...
observability:
  ...
  sentry:
    ...
    dist: web
YAML
helm template redrob-ee "$chart_dir" -f "$sentry_values" > "$sentry_rendered"
