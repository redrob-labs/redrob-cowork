...
config:
  ...
  public:
    ...
    webOrigin: "https://redrob.example.com"
    ...
    apiOrigin: ""
    mcpResourceUrl: ""
    mcpClaimNamespace: "https://redrob.example.com"
    desktopDenBaseUrl: ""
    corsOrigins: ""
    ...
    authCallbackUrl: "https://redrob.example.com"
  githubConnector:
    appId: ""
    clientId: ""
...
secret:
  values:
    ...
    databaseUrl: "mysql://redrob:REPLACE_ME@mysql.example.internal:3306/redrob_den?sslaccept=accept"
...
ingress:
  ...
  className: nginx
  web:
    host: redrob.example.com
  api:
    host: api.redrob.example.com
```
...
```bash
helm template redrob-ee ./packaging/helm/redrob-ee -f values.prod.yaml
helm upgrade --install redrob-ee ./packaging/helm/redrob-ee -f values.prod.yaml
```
...
secret:
  create: false
  existingSecret: redrob-ee-secrets
```
...
secret:
  values:
    databaseRedisUrl: "rediss://redis-master.redrob.svc.cluster.local:6379"
```
...
```bash
kubectl create secret generic redrob-custom-ca \
  --namespace redrob \
...
customCa:
  enabled: true
  existingSecret: redrob-custom-ca
  existingConfigMap: ""
  key: ca.crt
```
...
```bash
kubectl create configmap redrob-custom-ca \
  --namespace redrob \
  --from-file=ca.crt=./corp-root-ca.pem
```
...
customCa:
  enabled: true
  existingSecret: ""
  existingConfigMap: redrob-custom-ca
  key: ca.crt
```
...
`/etc/redrob/custom-ca/ca-bundle.pem` and sets `NODE_EXTRA_CA_CERTS` to that
...
secret:
  values:
    databaseUrl: "mysql://redrob:REPLACE_DB_PASSWORD@mysql.example.internal:3306/redrob_den?sslmode=verify-full"
```
...
```bash
kubectl rollout restart deployment/redrob-ee-den-api --namespace redrob
kubectl rollout restart deployment/redrob-ee-den-web --namespace redrob
kubectl rollout restart deployment/redrob-ee-inference --namespace redrob
```
...
```bash
kubectl create namespace redrob
```
...
```bash
kubectl create secret generic redrob-otel-headers \
  --namespace redrob \
...
observability:
  backend: otel
  serviceNames:
    denApi: redrob-den-api
    denWeb: redrob-den-web
  otel:
    ...
    tracesSamplerArg: ""
    headers:
      existingSecret: redrob-otel-headers
...
The commands below assume the Helm release is named `redrob-ee`. If you use a
different release name, run `kubectl get deployments,services --namespace
redrob` to find the generated resource names.

Confirm that the workloads are ready:

```bash
kubectl get pods --namespace redrob
kubectl rollout status deployment/redrob-ee-den-api --namespace redrob
kubectl rollout status deployment/redrob-ee-den-web --namespace redrob
```
...
```bash
kubectl describe deployment/redrob-ee-den-api --namespace redrob
kubectl describe deployment/redrob-ee-den-web --namespace redrob
```
...
```bash
kubectl port-forward service/redrob-ee-den-web 3005:3005 --namespace redrob
```
...
Your observability backend should show `redrob-den-web` and
`redrob-den-api`, with one connected trace for the request. Logs from both
...
observability:
  ...
  sentry:
    dsnSecret:
      existingSecret: redrob-sentry-runtime
      key: SENTRY_DSN
...
- Setup URL: `https://redrob.example.com/dashboard/integrations/github`
- Webhook URL: `https://api.redrob.example.com/v1/webhooks/connectors/github`
...
- `DEN_API_BASE=http://<release>-redrob-ee-den-api:8788`
- `DEN_AUTH_FALLBACK_BASE=http://<release>-redrob-ee-den-api:8788`
- `INFERENCE_PROXY_BASE_URL=http://<release>-redrob-ee-inference:8791` when `inference.enabled=true`
...
config:
  public:
    ...
    installerReleaseRepo: "redrob-labs/redrob-work"

installerArtifacts:
  enabled: true
  existingClaim: redrob-desktop-artifacts
  mountPath: /var/lib/redrob/installer-artifacts
```
