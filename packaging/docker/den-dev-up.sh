...
DEN_SINGLE_ORG_NAME="${DEN_SINGLE_ORG_NAME:-Redrob Work}"
...
echo "" >&2
echo "Redrob Work web UI:        http://localhost:$DEN_WEB_PORT" >&2
echo "Redrob Work web UI (LAN/public):      http://$PUBLIC_HOST:$DEN_WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "Redrob Work web UI (LAN IP):          http://$LAN_IPV4:$DEN_WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "Redrob Work web UI (Tailscale):       http://$TAILSCALE_DNS_NAME:$DEN_WEB_PORT" >&2
fi
