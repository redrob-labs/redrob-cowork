...
if [ -n "${REDROB_WORKSPACE_DIR:-}" ]; then
  REPO_DIR="$REDROB_WORKSPACE_DIR"
...
export NEXT_PUBLIC_REDROB_AUTH_CALLBACK_URL="${NEXT_PUBLIC_REDROB_AUTH_CALLBACK_URL:-$DEN_WEB_PUBLIC_URL}"
...
pnpm --filter @redrob-ee/den-db db:push > /tmp/den-db-push.log 2>&1

echo "==> Building Den API runtime assets..."
pnpm --filter @redrob-ee/den-api run build:mcp-apps
...
nohup env \
  ...
  REDROB_DEV_MODE="$REDROB_DEV_MODE" \
  NODE_OPTIONS="--conditions=development" \
  pnpm --filter @redrob-ee/den-api exec tsx watch src/main.ts > /tmp/den-api.log 2>&1 &
...
nohup env \
  ...
  DAYTONA_REDROB_PORT="${DAYTONA_REDROB_PORT:-8787}" \
  DAYTONA_SIGNED_PREVIEW_EXPIRES_SECONDS="${DAYTONA_SIGNED_PREVIEW_EXPIRES_SECONDS:-86400}" \
  pnpm --filter @redrob-ee/den-worker-proxy exec tsx watch src/server.ts > /tmp/den-worker-proxy.log 2>&1 &
...
if ! env \
  ...
  NEXT_PUBLIC_REDROB_AUTH_CALLBACK_URL="$NEXT_PUBLIC_REDROB_AUTH_CALLBACK_URL" \
  NEXT_PUBLIC_POSTHOG_KEY= \
  NEXT_PUBLIC_POSTHOG_API_KEY= \
  DEN_ORG_MODE="$DEN_ORG_MODE" \
  REDROB_DEV_MODE="$REDROB_DEV_MODE" \
  DEN_WEB_ALLOWED_DEV_ORIGINS="$DEN_WEB_ALLOWED_DEV_ORIGINS" \
  bash -c 'pnpm --filter @redrob/ui build && pnpm --filter @redrob-ee/utils build && pnpm --filter @redrob-ee/den-web build' > /tmp/den-web-build.log 2>&1; then
...
nohup env \
  ...
  REDROB_DEV_MODE="$REDROB_DEV_MODE" \
  DEN_WEB_ALLOWED_DEV_ORIGINS="$DEN_WEB_ALLOWED_DEV_ORIGINS" \
  pnpm --filter @redrob-ee/den-web exec next start --hostname 0.0.0.0 --port "$DEN_WEB_PORT" > /tmp/den-web.log 2>&1 &
