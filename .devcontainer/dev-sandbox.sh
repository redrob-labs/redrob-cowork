...
cd "$REPO_DIR"
export REDROB_WORKSPACE_DIR="$REPO_DIR"
...
echo "==> Waiting for Electron CDP on :$REDROB_ELECTRON_REMOTE_DEBUG_PORT..."
for _ in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:$REDROB_ELECTRON_REMOTE_DEBUG_PORT/json/list" >/dev/null 2>&1; then
