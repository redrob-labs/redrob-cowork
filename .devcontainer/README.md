...
```bash
bash .devcontainer/create-daytona-redrob-snapshot.sh   # one-time / refresh when deps change
bash .devcontainer/test-on-daytona.sh [branch-or-commit]
```

The test script creates a sandbox from the reusable `redrob-eval-vnc` snapshot
...
`redrob-eval-pnpm-store` volume so the image stays under Daytona's 20 GB limit.
...
Future Daytona test sandboxes mount `redrob-eval-secrets:/daytona-secrets`
...
The artifacts flow mounts `redrob-eval-artifacts:/daytona-artifacts`, starts a
...
Do not use the generic `daytona create https://github.com/redrob-labs/redrob-work`
...
```bash
bash .devcontainer/create-daytona-redrob-server-snapshot.sh  # one-time / refresh when deps change
...
2. `.devcontainer/create-daytona-redrob-snapshot.sh` bakes that image into
   `redrob-eval-vnc` without `node_modules`.
3. `/opt/redrob-daytona/start-daytona-vnc.sh` starts Xvfb, XFCE, x11vnc, and
   noVNC on display `:99`.
4. `test-on-daytona.sh` installs dependencies through the reusable
   `redrob-eval-pnpm-store` volume when `node_modules` is missing or the
   lockfile changed.
5. Vite serves the React UI on port 5173.
6. `/opt/redrob-daytona/start-daytona-electron.sh` sources optional secrets,
