...
VOLUME_NAME="${DAYTONA_SECRETS_VOLUME:-redrob-eval-secrets}"
MOUNT_PATH="${DAYTONA_SECRETS_MOUNT:-/daytona-secrets}"
LOCAL_ENV_FILE="${1:-.newtoken}"
DEST_ENV_FILE="${2:-openai.env}"
SANDBOX="redrob-secrets-setup-$(date +%Y%m%d-%H%M%S)"
SNAPSHOT_NAME="${DAYTONA_EVAL_SNAPSHOT:-redrob-eval-vnc}"
