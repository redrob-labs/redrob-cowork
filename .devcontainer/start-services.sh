...
cd apps/app
REDROB_DEV_MODE=1 nohup npx vite --host 0.0.0.0 --port 5173 > /tmp/vite.log 2>&1 &
cd /workspace
sleep 3
...
if mysql -h mysql -u root -ppassword -e "SELECT 1" >/dev/null 2>&1; then
  ...
  pnpm --filter @redrob-ee/den-db db:push 2>&1 || echo "  DB push failed (may be up to date)"
