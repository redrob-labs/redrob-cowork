...
These scopes are never requested by the built-in Redrob Work OAuth client. They are only requested when the user supplies their own Google OAuth client and explicitly enables the matching permission in `Settings -> Extensions -> Google Workspace -> Advanced`:
...
Redrob Work uses these scopes to identify the connected Google account in the app, display the signed-in account email to the user, and help users confirm they connected the intended Google account.

### `calendar.readonly`

Redrob Work uses this scope to read upcoming Google Calendar events when the user asks for meeting context or meeting preparation. Redrob Work does not modify calendars with this scope. The initial Google Workspace integration uses calendar data to show event title, time, attendees, and linked resources so the user can prepare for upcoming meetings.

### `drive.file`

Redrob Work uses this scope to access only the specific Google Drive files that the user selects, opens, or creates with Redrob Work. Redrob Work does not request broad Drive read access in Phase 1. This lets the user ask Redrob Work to summarize or use a selected document while keeping the rest of the user's Drive outside the app's access.

### `gmail.compose`

Redrob Work uses this scope to create Gmail drafts for the user to review in Gmail. Phase 1 does not expose an automatic send-email tool. Draft creation is used for workflows like drafting a meeting follow-up after the user asks for it. Users remain in control and send messages themselves from Gmail.

## Data Use Statement

Redrob Work uses Google Workspace data only to provide user-requested features, such as reading calendar context, reading explicitly selected Drive files, and creating Gmail drafts. Redrob Work does not sell Google user data, use Google user data for advertising, or use Google user data to train generalized AI models. Desktop OAuth tokens are stored locally using encrypted OS storage when available.
...
5. Show the Google OAuth consent screen, including the app name `Redrob Work` and requested scopes.

6. Approve access with a test account.

7. Return to Redrob Work and show the connected account email.
...
9. Click `Run scope smoke test` and show the success state, including the created Drive file and Gmail draft IDs. This verifies all requested Phase 1 scopes:
   ...
   - Drive selected/app-created file access by creating and reading `Redrob Work Google Workspace smoke test.txt`.
   - Gmail compose access by creating `Redrob Work Google Workspace smoke test draft` in Gmail drafts.

10. Open Google Drive and show the created smoke-test file.

11. Open Gmail drafts and show the created smoke-test draft. State that Redrob Work created a draft only and did not send email automatically.

12. Click `Disconnect` in Redrob Work and state that Redrob Work revokes the Google OAuth token and removes the local encrypted token vault entry.
