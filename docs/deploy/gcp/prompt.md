...
Copy this prompt into your agent and replace every `{{PLACEHOLDER}}` before running it. The agent is expected to operate from a local Redrob Work repository checkout and use CLI/API tools for infrastructure. Any browser-only step should be handed back to the operator.

```text
You are deploying Redrob Work EE to Google Cloud from this repository checkout:
