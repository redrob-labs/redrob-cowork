...
Redrob Work discovers and authorizes Den-managed external MCP connections without
...
deployment. Redrob Work derives both public OAuth URLs exclusively from this
...
Redrob Work as a web OAuth client. Each self-hosted Den deployment has its own
...
In Cloud → Connections, enter the MCP server URL. Redrob Work automatically runs
...
When authorization starts, Redrob Work uses this registration priority:
...
challenge nor the administrator selects scopes, Redrob Work falls back to the
...
- **Additional permission required**: review and approve the newly challenged
  scope before reconnecting; Redrob Work does not silently expand access.
