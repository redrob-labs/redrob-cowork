...
Redrob Cowork discovers and authorizes Den-managed external MCP connections without
...
deployment. Redrob Cowork derives both public OAuth URLs exclusively from this
...
Redrob Cowork as a web OAuth client. Each self-hosted Den deployment has its own
...
In Cloud → Connections, enter the MCP server URL. Redrob Cowork automatically runs
...
When authorization starts, Redrob Cowork uses this registration priority:
...
challenge nor the administrator selects scopes, Redrob Cowork falls back to the
...
- **Additional permission required**: review and approve the newly challenged
  scope before reconnecting; Redrob Cowork does not silently expand access.
