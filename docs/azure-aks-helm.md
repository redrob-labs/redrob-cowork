...
add-on's managed NGINX Ingress controller so Redrob Work gets stable HTTPS host
...
production workloads through November 2026. The current Redrob Work chart emits
...
Redrob Work. Azure Load Balancer is a layer 4 load balancer. It is useful for TCP
...
identities, and network security groups. The Redrob Work Helm chart owns Redrob Work
...
The Redrob Work EE release artifact is already a Helm chart, and AKS gives the
...
not a different Redrob Work packaging format.
...
database. This is an Azure subscription/region eligibility issue, not Redrob Work
quota usage.
...
subscription cannot use, not that Redrob Work has consumed quota. Pick an allowed
...
- Region capacity varies. If AKS Automatic reports SKU/capacity failures, retry
  in a known-good region before debugging the Redrob Work chart.
...
indicate an Azure MySQL subscription/region eligibility issue before Redrob Work or
...
on without requiring a cloud CA bundle to be mounted into the Redrob Work image.
...
**Identifier (Entity ID)** is the Redrob Work auth origin and the certificate saved
in Redrob Work is the active Entra token-signing certificate.
