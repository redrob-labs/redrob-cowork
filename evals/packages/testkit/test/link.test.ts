...
test("Daytona link commands upload the runner-side script before launching its temp path", () => {
  ...
  assert(commands.cleanup.includes("pid=$(</tmp/redrob-den-link-server.pid)"));
  assert(commands.cleanup.includes('kill "$pid"'));
  assert(commands.cleanup.includes("rm -f /tmp/redrob-den-link-server.pid /tmp/redrob-den-link-server.mjs /tmp/redrob-den-link-server.mjs.b64"));
  assert(!commands.cleanup.includes("pkill"));
  assert.equal(commands.upload[0], ": > /tmp/redrob-den-link-server.mjs.b64");
  const chunkPattern = /^printf %s ([A-Za-z0-9+/=]+) >> \/tmp\/redrob-den-link-server\.mjs\.b64$/;
  ...
  assert(finalize?.includes("base64 -d /tmp/redrob-den-link-server.mjs.b64 > /tmp/redrob-den-link-server.mjs"));
  assert(finalize?.includes("actual_bytes=$(wc -c < /tmp/redrob-den-link-server.mjs)"));
  assert(finalize?.includes("rm -f /tmp/redrob-den-link-server.mjs.b64"));
  ...
  assert(commands.detach.includes('"node", "/tmp/redrob-den-link-server.mjs"'));
  assert(commands.detach.includes("pid_file.write(str(process.pid)"));
  assert(commands.detach.includes('os.replace(temporary_pid, "/tmp/redrob-den-link-server.pid")'));
