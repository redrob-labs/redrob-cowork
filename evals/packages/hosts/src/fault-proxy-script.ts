...
async function control(incoming, response, path) {
  if (path === "/__redrob_faults/health" && incoming.method === "GET") {
    json(response, 200, { ok: true, issuer });
    return;
  }
  if (incoming.headers["x-redrob-fault-token"] !== controlToken) {
    json(response, 401, { error: "Unauthorized" });
    return;
  }
  if (path === "/__redrob_faults/requests" && incoming.method === "GET") {
    json(response, 200, { requests });
    return;
  }
  if (path === "/__redrob_faults/clear" && incoming.method === "POST") {
    rules.length = 0;
    response.writeHead(204);
    response.end();
    return;
  }
  if (path === "/__redrob_faults/rules" && incoming.method === "POST") {
    try {
...
const server = createServer((incoming, response) => {
  void (async () => {
    const path = incoming.url ?? "/";
    if (path.startsWith("/__redrob_faults")) {
