...
export function desktopSandboxName(name: string): string {
  ...
  return `redrob-connector-${safeName}-${sandboxTimestamp()}-${process.pid}-${randomBytes(4).toString("hex")}`;
}

export function serverSandboxName(): string {
  return `redrob-server-${sandboxTimestamp()}-${process.pid}-${randomBytes(4).toString("hex")}`;
}
...
export async function provisionDesktopSandbox(options: DesktopSandboxOptions & ProvisionExecOptions): Promise<DesktopSandbox> {
  ...
  await timedStep(log, "sandbox gate", async () => {
    if (reused) {
      ...
      const snapshot = options.snapshot ?? "redrob-eval-vnc";
      ...
      await checkedExec(
        ...
        [
          ...
          "--name", sandbox,
          "--snapshot", id,
          ...(options.secrets === true ? ["--volume", "redrob-eval-secrets:/daytona-secrets"] : []),
          "--auto-stop", "60",
          "--public",
          "--target", "us",
        ],
  ...
  await timedStep(log, "install gate", async () => {
    await execInSandbox(
      exec,
      sandbox,
      "cd /workspace; pnpm install --store-dir /workspace/.redrob-daytona/pnpm-store",
  ...
  await timedStep(log, "cleanup and disk gate", async () => {
    const result = await execInSandbox(
      exec,
      sandbox,
      "rm -rf /workspace/.redrob-daytona/profiles /tmp/redrob-* 2>/dev/null; df -P /workspace | tail -1",
    ...
    if (used > 85) {
      const sizes = await execInSandbox(
        exec,
        sandbox,
        "du -sh /workspace/node_modules /workspace/.redrob-daytona/pnpm-store 2>&1 || true",
...
echo detached`;
  ...
  await timedStep(log, "first boot gate", async () => {
    ...
    // redrob-server tsc build, and the engine cold start. Paid INSIDE a
...
subprocess.Popen(["bash", "-lc", "cd /workspace && env REDROB_ELECTRON_USERDATA=/tmp/warmup-profile REDROB_ELECTRON_REMOTE_DEBUG_PORT=9825 bash .devcontainer/start-daytona-electron.sh"], stdin=subprocess.DEVNULL, stdout=log, stderr=subprocess.STDOUT, start_new_session=True, close_fds=True)
PYEOF
echo detached`;
...
function runDenProvisionScript(ref: string, repoRoot: string, bootstrapAdminEmail: string | undefined, log: (line: string) => void, urlsFile: string): Promise<LocalProcessResult> {
  return new Promise((resolve, reject) => {
    const env: NodeJS.ProcessEnv = { ...process.env, REDROB_DEN_URLS_FILE: urlsFile };
...
async function proveDenSeed(apiUrl: string, webUrl: string, sandbox: string, reused: boolean): Promise<void> {
  ...
  const password = process.env.REDROB_EVAL_DEMO_PASSWORD ?? "RedrobWorkDemo123!";
...
export async function provisionDenSandbox(options: DenSandboxOptions & ProvisionExecOptions): Promise<DenSandbox> {
  ...
  if (reused) {
    ...
    const urlsDir = await mkdtemp(path.join(os.tmpdir(), "redrob-den-urls-"));
...
export async function startFaultProxyOnSandbox(options: FaultProxyOnSandboxOptions & ProvisionExecOptions): Promise<FaultProxyOnSandbox> {
  ...
  await timedStep(log, "fault proxy process cleanup", async () => {
    await execInSandbox(
      exec,
      options.sandbox,
      "pkill -f redrob-fault-proxy || true",
  ...
  await timedStep(log, "fault proxy script upload", async () => {
    ...
    await execInSandbox(
      exec,
      options.sandbox,
      `printf %s ${encoded} | base64 -d > /tmp/redrob-fault-proxy.mjs`,
  ...
  await timedStep(log, "fault proxy process detach", async () => {
    const detachScript = `python3 - <<PYEOF
import subprocess
log = open("/tmp/redrob-fault-proxy.log", "ab", buffering=0)
subprocess.Popen(["bash", "-lc", "env PORT=${port} UPSTREAM=http://127.0.0.1:${upstreamPort} ISSUER=${url} CONTROL_TOKEN=${token} node /tmp/redrob-fault-proxy.mjs"], stdin=subprocess.DEVNULL, stdout=log, stderr=subprocess.STDOUT, start_new_session=True, close_fds=True)
PYEOF
echo detached`;
  ...
  await timedStep(log, "fault proxy health gate", async () => {
    ...
    while (Date.now() < deadline) {
      ...
      let responseOk = false;
      try {
        const response = await fetchImpl(`${url}/__redrob_faults/health`, { signal: AbortSignal.timeout(5_000) });
        body = await response.json();
        responseOk = response.ok;
        if (!response.ok) last = `HTTP ${response.status}`;
      } catch (error) {
    ...
    const proxyLog = await execInSandbox(
      exec,
      options.sandbox,
      "tail -80 /tmp/redrob-fault-proxy.log 2>&1 || true",
  ...
  return {
    ...
    async stop(): Promise<void> {
      await execInSandbox(
        exec,
        options.sandbox,
        "pkill -f redrob-fault-proxy.mjs || true",
...
export function renderConnectorE2eTestEnv(facts: ConnectorE2eTestEnv): string {
  ...
  return [
    ...
    "REDROB_EVAL_E2E_TESTS=1",
    "REDROB_EVAL_CONNECTOR_E2E_TEST=1",
    `REDROB_EVAL_DEN_API_URL=${shellQuote(facts.denApiUrl)}`,
    `REDROB_EVAL_DEN_WEB_URL=${shellQuote(facts.denWebUrl)}`,
    `REDROB_EVAL_DAYTONA_SANDBOX_A=${shellQuote(facts.sandboxA)}`,
    `REDROB_EVAL_DAYTONA_SANDBOX_B=${shellQuote(facts.sandboxB)}`,
    `REDROB_EVAL_CONNECTOR_MOCK_PUBLIC_URL=${shellQuote(facts.mockUrl)}`,
    "REDROB_EVAL_MODEL=big-pickle",
    "",
  ].join("\n");
}
...
export function parseConnectorE2eTestEnv(content: string): ConnectorE2eTestEnv {
  ...
  required("REDROB_EVAL_E2E_TESTS");
  required("REDROB_EVAL_CONNECTOR_E2E_TEST");
  required("REDROB_EVAL_MODEL");
  ...
  return {
    denApiUrl: required("REDROB_EVAL_DEN_API_URL"),
    denWebUrl: required("REDROB_EVAL_DEN_WEB_URL"),
    sandboxA: required("REDROB_EVAL_DAYTONA_SANDBOX_A"),
    sandboxB: required("REDROB_EVAL_DAYTONA_SANDBOX_B"),
    mockUrl: required("REDROB_EVAL_CONNECTOR_MOCK_PUBLIC_URL"),
    ref,
