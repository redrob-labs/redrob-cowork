...
function desktopFake(diskUse = "40%"):
  ...
  const exec: DaytonaExec = async (args, opts) => {
    ...
    if (args[0] === "snapshot") {
      return { stdout: JSON.stringify([{ name: "redrob-eval-vnc", id: "snapshot-123" }]), stderr: "", code: 0 };
    }
...
test("server sandbox names are unique within the same CI process and second", () => {
  ...
  assert.match(first, new RegExp(`^redrob-server-\\d{8}-\\d{6}-${process.pid}-[0-9a-f]{8}$`));
...
test("desktop sandbox names stay unique when parallel workers use the same surface name", () => {
  ...
  assert.match(first, new RegExp(`^redrob-connector-testkit-admin-\\d{8}-\\d{6}-${process.pid}-[0-9a-f]{8}$`));
  assert.notEqual(first, second);
});
...
test("the eval secrets volume is mounted only when explicitly asked for", async () => {
  ...
  const create = calls.find((call) => call.args[0] === "create");
  assert(create?.args.includes("redrob-eval-secrets:/daytona-secrets"));
});
...
test("startFaultProxyOnSandbox uploads and detaches the proxy after resolving its preview URL", async () => {
  ...
  assert.match(scripts[0] ?? "", /pkill -f redrob-fault-proxy/);
  assert.match(scripts[1] ?? "", /^printf %s [A-Za-z0-9+/=]+ \| base64 -d > \/tmp\/redrob-fault-proxy\.mjs$/);
  ...
  assert.match(scripts[2] ?? "", /node \/tmp\/redrob-fault-proxy\.mjs/);
  assertRemoteCommandsAreSingleArgument(calls);

  await proxy.stop();
  assert.match(calls.at(-1)?.args[3] ?? "", /pkill -f redrob-fault-proxy\.mjs/);
});
