...
test("fault proxy script exposes authenticated controls and preserves local fault semantics", async () => {
  ...
  const directory = await mkdtemp(join(tmpdir(), "redrob-fault-proxy-"));
  ...
  const auth = { "x-redrob-fault-token": token };
  ...
  try {
    ...
    while (Date.now() < deadline) {
      try {
        health = await fetch(`${url}/__redrob_faults/health`);
        if (health.ok) break;
      } catch {
    ...
    assert.equal((await fetch(`${url}/__redrob_faults/requests`)).status, 401);
    assert.equal((await fetch(`${url}/__redrob_faults/clear`, { method: "POST" })).status, 401);
    assert.equal((await post("/__redrob_faults/rules", {
      kind: "status",
      pathPrefix: "/flaky",
      statusCode: 429,
      times: 2,
    ...
    assert.equal((await post("/__redrob_faults/clear")).status, 204);
    assert.equal((await fetch(`${url}/cleared`)).status, 200);

    const logResponse = await fetch(`${url}/__redrob_faults/requests`, { headers: auth });
