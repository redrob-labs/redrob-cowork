...
test("claim builder supports present, absent, and unexpected group claim shapes", () => {
  ...
  assert.deepEqual(present.groups, ["Engineering", "Redrob Work Lab"]);
  assert.equal("groups" in absent, false);
  assert.deepEqual(odd.groups, { primary: "Engineering", all: ["Engineering", "Redrob Work Lab"] });
});
...
test("blocked-user response uses the Entra-style policy wording", () => {
  ...
  assert.match(response.html, /Redrob Work Mock IdP policy/);
});
