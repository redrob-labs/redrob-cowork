...
const DEFAULT_REPO = "redrob-labs/redrob-work";
...
export function desktopReleaseAssetFileName(
  ...
  const prefix = distribution === "public" ? "redrob" : `redrob-${distribution}`;
...
function assetBody(input: ReleaseAssetInput, version: string): Uint8Array {
  ...
  return textEncoder.encode(`redrob release ${version} ${input.platform}\n`);
}
