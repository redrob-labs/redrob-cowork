...
export function opensslCertificateCommands(input: OpenSslCommandInput): OpenSslCommand[] {
  ...
  const rootSubject = input.corporateIssuer
    ? "/CN=Redrob Work Egress Lab Corporate Root CA"
    : "/CN=Redrob Work Egress Lab Root CA";
  const intermediateSubject = input.corporateIssuer
    ? "/CN=Redrob Work Egress Lab Corporate Interception CA"
    : "/CN=Redrob Work Egress Lab Intermediate CA";
  return [
...
async function generateCertificateMaterial(input: { hostname: string; aiaUrl: string | null; corporateIssuer: boolean }): Promise<CertificateMaterial> {
  const dir = await mkdtemp(path.join(tmpdir(), "redrob-egress-lab-"));
  ...
  if (method === "initialize") {
    jsonResponse(response, 200, {
      ...
      result: {
        ...
        serverInfo: { name: "redrob-egress-lab", version: "1.0.0" },
      },
    }, {
...
export async function stageTrustedEnterpriseTlsRoot(
  ...
  try {
    stagingDir = await mkdtemp(path.join(tmpdir(), "redrob-enterprise-tls-trust-"));
  } catch (error) {
...
export function linuxTrustStorePlan(rootPemPath: string): LinuxTrustStorePlan {
  const certificatePath = "/usr/local/share/ca-certificates/redrob-enterprise-tls-edge.crt";
...
export async function writeTempPemBundle(pems: string[]): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "redrob-egress-ca-bundle-"));
