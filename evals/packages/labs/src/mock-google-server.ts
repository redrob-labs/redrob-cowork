...
function chooserPage(state: MockGoogleState, pending: PendingAuthorization): string {
  ...
  return `<!doctype html><html><head><title>Choose an account</title></head><body><main style="font-family: system-ui, sans-serif; max-width: 560px; margin: 48px auto;"><h1>Choose an account</h1><p>Continue to Redrob Work</p><ul>${accounts}</ul></main></body></html>`;
}
