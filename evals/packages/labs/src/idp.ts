...
const DEFAULT_CLIENT_ID = "redrob-eval-oidc-client";
const DEFAULT_CLIENT_SECRET = "redrob-eval-oidc-secret";
const DEFAULT_ISSUER = "http://127.0.0.1/mock-redrob-idp";
const DEFAULT_EVAL_ISSUER = "http://127.0.0.1:19190";
const DEFAULT_GROUPS = ["Engineering", "Redrob Work Lab"];
...
export function buildOidcClaims(input: {
  ...
  const claims: Record<string, unknown> = {
    ...
    picture: `https://avatar.redrob.test/${encodeURIComponent(input.subject.email)}`,
...
export function buildBlockedUserResponse(subject: MockIdpSubjectInput, policyName = "Redrob Work Mock IdP policy"): BlockedUserResponseShape {
  ...
  return {
    ...
    errorDescription,
    message,
    html: `<!doctype html><html><head><title>Redrob Work Mock IdP policy block</title></head><body><main style="font-family: sans-serif; max-width: 720px; margin: 48px auto;"><p>Redrob Work Mock IdP policy</p><h1>Sign-in blocked by identity provider policy</h1><p>${escapeHtml(message)}</p><p>error=access_denied</p></main></body></html>`,
  };
}
...
function samlPostBindingPage(config: NormalizedMockIdpConfig): HtmlDocument {
  return {
    title: "Redrob Work Mock IdP SAML endpoint",
    heading: "SAML POST binding endpoint",
    paragraphs: [
      "Redrob Work Mock IdP lab",
      "This fixture exposes SAML metadata and a POST binding endpoint, but the Redrob Work eval lab drives OIDC because this checkout implements OIDC and SAML SSO and OIDC keeps the signed-token path dependency-free.",
      `Issuer: ${config.issuer}`,
    ],
  };
}
...
function authorize(state: MockIdpState, url: URL, response: ServerResponse): void {
  ...
  if (matchesBlockedUser(subject, state.config.knobs.blockedUser)) {
    ...
    sendHtml(response, blocked.status, {
      title: "Redrob Work Mock IdP policy block",
      heading: "Sign-in blocked by identity provider policy",
      paragraphs: ["Redrob Work Mock IdP policy", blocked.message, "error=access_denied"],
    });
    return;
  }
