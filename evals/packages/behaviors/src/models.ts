...
async function openModelPicker(app: Surface): Promise<void> {
  ...
  if (open !== true) {
    await waitFor(app, `window.__redrobControl?.listActions().some((entry) => entry.id === "session.model_picker.open" && entry.disabled === false)`, {
      timeoutMs: 30_000,
...
export async function selectModel(app: Surface, name: string, options?: { provider?: string }): Promise<ModelFacts> {
  ...
  const persisted = await evalIn(app, `(() => {
    try {
      const preferences = JSON.parse(localStorage.getItem("redrob.preferences") || "{}");
...
export async function recoverInvalidModelSelection(
  ...
  await evalIn(app, `(() => {
    let preferences = {};
    try { preferences = JSON.parse(localStorage.getItem("redrob.preferences") || "{}"); } catch {}
    delete preferences.defaultModel;
    delete preferences.modelVariant;
    localStorage.setItem("redrob.preferences", JSON.stringify(preferences));
...
export async function seedUnavailableModel(app: Surface): Promise<UnavailableModelSeed> {
  await waitFor(app, `window.__redrobControl?.listActions().some((entry) => entry.id === "eval.model_not_available.seed" && entry.disabled === false)`, {
    timeoutMs: 45_000,
