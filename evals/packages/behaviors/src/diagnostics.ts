...
async function main() {
  ...
  const cloudProbe = await cloudModule.probeRedrobCloudCatalog({
...
export function productDiagnosticsPrecondition(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  if (bun.status !== 0) {
    ...
    return `Bun is required to import Redrob Work's shipped TypeScript diagnostics modules for product-verdict egress proofs (${reason}).`;
  }
...
async function diagnoseTlsLab(lab: EgressLabHandle): Promise<DiagnosticVerdict> {
  ...
  if (lab.profile === "intercept" && /corporate|interception|proxy|mitm/u.test(issuerLower)) {
    return {
      profile: lab.profile,
      text: `LIKELY TLS INTERCEPTION: endpoint leaf is re-signed by ${servedIssuer}, not the expected public issuer. Install or pass the corporate root, or bypass TLS inspection for Redrob Work hosts.`,
      evidence: lines.join("\n"),
...
export async function readDeniedHostFacts(
  lab: EgressLabHandle,
  targetUrl = "https://github.com/redrob-labs/redrob-work/releases/latest",
...
async function diagnoseDenyLab(lab: EgressLabHandle): Promise<DiagnosticVerdict> {
  ...
  const facts = await readDeniedHostFacts(lab, `https://${host}/redrob-labs/redrob-work/releases/latest`);
...
const needle = (process.env.REDROB_TLS_REPRO_CA_MATCH || "Redrob Work Egress Lab").toLowerCase();
