...
test("Daytona defaults to two workers and never prepares more slots than explicit files", () => {
  assert.equal(suiteWorkerCount(["vitest", "specs/a.test.ts", "specs/b.test.ts"], { REDROB_EVAL_DAYTONA: "1" }), 2);
  assert.equal(suiteWorkerCount(["vitest", "specs/a.test.ts"], { REDROB_EVAL_DAYTONA: "1", REDROB_EVAL_MAX_WORKERS: "8" }), 1);
  assert.equal(suiteWorkerCount(["vitest", "specs/*.test.ts"], { REDROB_EVAL_DAYTONA: "1", REDROB_EVAL_MAX_WORKERS: "4" }), 4);
});
