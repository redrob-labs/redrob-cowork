...
test("Microsoft OAuth redirects with state and Graph serves deterministic mail, calendar, and files", async (context) => {
  ...
  authorizeUrl.searchParams.set("state", "state-from-redrob");
  ...
  assert.equal(callback.searchParams.get("state"), "state-from-redrob");
...
test("worker mock rejects partial credentials and completes a prompt after a busy snapshot", async (context) => {
  ...
  const partial = await fetch(`${origin}/worker/workspaces`, {
    headers: { "x-redrob-host-token": MOCK_WORKER_HOST_TOKEN },
  });
  assert.equal(partial.status, 401);

  const headers = {
    "x-redrob-host-token": MOCK_WORKER_HOST_TOKEN,
  ...
  assert.equal(idle.item.messages.at(-1).parts[0].text, "Redrob Work worker reply: Summarize the launch notes");
});
