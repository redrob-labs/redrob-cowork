...
const title = missingRequirements.length > 0
  ...
  : "Redrob Work owns local MCP OAuth while OpenCode consumes the gateway tools";
...
test(title, async ({ evidence, place }) => {
  ...
  evidence.recordAssertionEvidence(
    "The desktop's embedded Redrob Work server owns the provider OAuth callback and credential",
