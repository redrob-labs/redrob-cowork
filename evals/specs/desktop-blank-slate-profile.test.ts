...
test("any desktop build can launch with an isolated blank-slate profile", async ({ evidence }) => {
  ...
  try {
    ...
    expect(firstEnv.REDROB_DESKTOP_BOOTSTRAP_PATH).toBe(first.environment.REDROB_DESKTOP_BOOTSTRAP_PATH);
    ...
    evidence.recordAssertionEvidence(
      ...
      "REDROB_DESKTOP_BOOTSTRAP_PATH points inside the temporary root, so an installed localhost bootstrap cannot bypass Enterprise activation.",
      firstEnv.REDROB_DESKTOP_BOOTSTRAP_PATH === first.environment.REDROB_DESKTOP_BOOTSTRAP_PATH,
    );
    evidence.recordAssertionEvidence(
      "Package flavor is preserved without development mode",
      "Enterprise remains Enterprise and the blank-slate profile does not set REDROB_DEV_MODE.",
