...
briefTest(testBrief({
  ...
  expect(testEvidencePackage).toContain('"name": "@redrob/test-evidence"');
  expect(testArtifactsPackage).toContain('"name": "@redrob/test-artifacts"');
  expect(evalsPackage).not.toContain('"@redrob/fraimz"');
  expect(evalsPackage).not.toContain('"@redrob/evidence"');
  ...
  prove.evidencePackages(
    true,
    "the workspace exposes @redrob/test-evidence, @redrob/test-artifacts, evidence:judge, artifacts:publish, and --test-run",
  );
