...
Read-only admin analytics MCP server for the Redrob Work Den database. Ask Redrob Work
...
> connect "Redrob Work Admin Analytics" from Settings -> Connections -> MCP
...
GRANT SELECT ON redrob_den.* TO 'den_readonly'@'%';
```

## Register in Redrob Work
...
```jsonc
{
  "mcp": {
    "den-admin": {
      ...
      "environment": { "DATABASE_URL": "mysql://den_readonly:...@host:3306/redrob_den" },
      "enabled": true
    }
  }
}
