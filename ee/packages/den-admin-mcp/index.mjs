#!/usr/bin/env node
// den-admin-mcp — read-only admin analytics MCP server for the Redrob Work Den database.
//
// Tools: den_overview, den_growth, den_retention, den_company_users,
//        den_users_search, den_org_overview, den_query
//
// Config: DATABASE_URL (mysql://user:pass@host:3306/redrob_den).
...
//
// Register in Redrob Work (opencode.json):
...
function getPool() {
  if (!pool) {
    ...
    if (!url) {
      throw new Error(
        "DATABASE_URL is required (e.g. mysql://root:password@127.0.0.1:3306/redrob_den)",
      );
    }
