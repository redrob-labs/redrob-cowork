...
const upstream = createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/den/v1/auth/desktop-handoff" && request.method === "POST") {
    ...
    const redrobUrl = new URL("redrob://den-auth")
    redrobUrl.searchParams.set("grant", "one-time-smoke-grant")
    redrobUrl.searchParams.set("denBaseUrl", `${upstreamOrigin}/api/den`)
    return json(response, 200, { redrobUrl: redrobUrl.toString() })
  }
...
try {
  ...
  const routeCookie = browserCookies.find((cookie) => cookie.startsWith("redrob_connect_debug_route="))?.split(";", 1)[0]
  ...
  const handoffDenBaseUrl = new URL(handoffPayload.redrobUrl).searchParams.get("denBaseUrl")
