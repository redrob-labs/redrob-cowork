...
describe("Connect debug proxy configuration", () => {
  test("accepts only encoded origins whose host is allowlisted", () => {
    ...
    process.env.DEBUG_PROXY_DEFAULT_UPSTREAM = "https://app.redrob.io"
    process.env.DEBUG_PROXY_ALLOWED_UPSTREAMS = "staging.redrob.io,http://127.0.0.1:4545"
    const config = connectDebugProxyConfig()
    expect(allowedUpstreamOverride(encodeUpstreamParameter("https://staging.redrob.io"), config)).toBe("https://staging.redrob.io")
...
describe("Connect debug proxy rewriting and streaming", () => {
  test("rewrites upstream Location, absolute JSON URLs, and cookie scope", async () => {
    const upstreamUrl = new URL("https://api.redrob.io/v1/session")
    ...
    expect(rewriteSetCookieHeader("session=value; Domain=api.redrob.io; Path=/; Secure; HttpOnly", "/via/default/access-key"))
    ...
    const response = await buildConnectDebugProxyResponse({
      proxyBase,
      tamperMode: null,
      upstream: Response.json({ redirect: "https://api.redrob.io/next" }),
  ...
  test("keeps the desktop auth handoff on the selected proxy base", async () => {
    const upstreamUrl = new URL("https://app.redrob.io/api/den/v1/auth/desktop-handoff")
    const proxyBase = "https://proxy.example/via/default/access-key"
    const upstreamDenBaseUrl = "https://app.redrob.io/api/den"
    const deepLink = `redrob://den-auth?grant=one-time-grant&denBaseUrl=${encodeURIComponent(upstreamDenBaseUrl)}`

    expect(rewriteLocationHeader(deepLink, upstreamUrl, proxyBase)).toBe(
      `redrob://den-auth?grant=one-time-grant&denBaseUrl=${encodeURIComponent(`${proxyBase}/api/den`)}`,
    )
  ...
  test("keeps the proxy key private while translating same-origin browser headers", () => {
    const headers = forwardRequestHeaders(new Headers({
      ...
      upstreamOrigin: "https://api.redrob.io",
    })
    expect(headers.has("x-connect-debug-proxy-key")).toBe(false)
    expect(headers.get("origin")).toBe("https://api.redrob.io")
    expect(headers.get("referer")).toBe("https://api.redrob.io/login?next=1")
  ...
  test("passes SSE through without waiting for the stream to finish", async () => {
    ...
    const response = await buildConnectDebugProxyResponse({
      ...
      upstreamRequestUrl: new URL("https://api.redrob.io/mcp/agent"),
    })
  ...
  test("rewrites chunked JSON without buffering the complete response", async () => {
    ...
    const upstreamBody = new ReadableStream<Uint8Array>({
      async start(controller) {
        controller.enqueue(encoder.encode(`{"padding":"${"x".repeat(80)}","url":"https://api.redrob.io`))
        await secondChunk
  ...
  test("keeps the bad protocol header consistent with the tampered initialize payload", async () => {
    const response = await buildConnectDebugProxyResponse({
      ...
      upstreamRequestUrl: new URL("https://app.redrob.io/api/den/mcp/agent"),
    })
