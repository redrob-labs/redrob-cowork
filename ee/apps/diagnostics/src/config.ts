...
export function diagnosticsConfig() {
  ...
  return {
    adminPassword: process.env.DIAGNOSTICS_ADMIN_PASSWORD ?? (hosted ? "" : "RedrobWorkDiagnosticsLocal!"),
    adminUsername: process.env.DIAGNOSTICS_ADMIN_USERNAME ?? "diagnostics-admin",
    bearerToken: process.env.DIAGNOSTICS_MCP_BEARER_TOKEN ?? (hosted ? "" : "RedrobDiagnosticsToken!"),
