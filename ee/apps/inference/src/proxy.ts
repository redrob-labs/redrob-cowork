...
function sanitizeHeaders(request: Request, apiKey: string, redrobRequestId: string) {
  const headers = new Headers()
  const accept = request.headers.get("accept")
  if (accept) headers.set("accept", accept)
  headers.set("authorization", `Bearer ${apiKey}`)
  headers.set("content-type", "application/json")
  headers.set("x-redrob-request-id", redrobRequestId)
  if (env.proxyBaseUrl) {
    headers.set("http-referer", env.proxyBaseUrl)
  }
  headers.set("x-title", "Redrob Work Inference")
  return headers
}
...
async function logUpstreamError(input: {
  ...
  logProxyError("Upstream OpenRouter request failed", {
    redrobRequestId: input.redrobRequestId,
...
async function prepareBody(request: Request, input: {
  ...
  if (blockedSelection) {
    ...
    return { error: openAiError(400, "unsupported_model_selection", `Redrob Work inference does not allow alternate model selection (${blockedSelection}).`), incomingModel: requestedModel, upstreamModel: model ? model.upstreamModel : null }
  }
  ...
  const body = json
  if (!model) {
    logProxyError("Unknown Redrob Work model alias", {
      redrobRequestId: input.redrobRequestId,
      organizationId: input.organizationId,
      orgMembershipId: input.orgMembershipId,
      inferenceKeyId: input.inferenceKeyId,
    ...
    return { error: openAiError(404, "model_not_found", `Unknown Redrob Work model alias: ${requestedModel}`), incomingModel: requestedModel, upstreamModel: null }
  }

  body.model = model.upstreamModel
  body.user = input.orgMembershipId
  body.session_id = input.redrobRequestId
  body.trace = {
    trace_id: input.redrobRequestId,
    trace_name: "Redrob Work Inference",
    generation_name: model.alias,
    org_membership_id: input.orgMembershipId,
    inference_key_id: input.inferenceKeyId,
    redrob_request_id: input.redrobRequestId,
  }
...
function localRouteRejection(path: string, method: string) {
  ...
  return openAiError(404, "not_found", `Unsupported Redrob Work inference route: ${method} ${path}.`)
}
...
export function registerProxyRoutes(app: Hono, dependencies: ProxyDependencies = defaultProxyDependencies) {
  ...
  async function handleApiRequest(c: Context) {
    ...
    if (new URL(c.req.url).search) {
      ...
      return openAiError(400, "unsupported_query_parameters", "Redrob Work chat completions does not accept query parameters.")
    }
    ...
    if (!limits.ok) {
      c.header("x-redrob-limit-bucket-id", limits.limitedBy)
    ...
    try {
      const upstreamInit: ProxyRequestInit = {
        method: c.req.method,
        headers: sanitizeHeaders(c.req.raw, providerKey.encrypted_api_key, redrobRequestId),
        body: prepared.body,
        duplex: "half",
      }
