 ...
 *   DEN_API_BASE_URL       Den API root that serves /v1, e.g. https://app.redrob.io/api/den
 ...
 *
 * Usage:
 *   DEN_API_BASE_URL=https://app.redrob.io/api/den \
 *   DEN_API_BEARER_TOKEN=<session bearer token> \
 *   DEN_CONFIG_OBJECT_ID=cob_... \
 *   pnpm --filter @redrob-ee/den-api run smoke:config-object-payload-boundary -- --dry-run
 *
...
const requiredEnv: EnvDoc[] = [
  {
    ...
    description: "Use the deployed Den API root that serves /v1, for example https://app.redrob.io/api/den.",
  },
  {
