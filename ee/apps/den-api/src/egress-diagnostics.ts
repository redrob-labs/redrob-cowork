...
function httpFailure(status: number, category: EgressDiagnosticCategory): DiagnosticFailure {
  ...
  if (status >= 500) {
    return new DiagnosticFailure({
      action: "Give Redrob Work support the run ID and diagnostic reference so the Diagnostics deployment can be inspected.",
      category,
      code: `http_${status}`,
  ...
  return new DiagnosticFailure({
    action: "Give Redrob Work support the run ID and response status, then compare the matching remote trace.",
    category,
    code: `http_${status}`,
...
async function readJson(response: Response): Promise<Record<string, unknown>> {
  ...
  if (!reader) throw new DiagnosticFailure({
    action: "Give Redrob Work support the run ID and diagnostic reference.",
    category: "http",
  ...
  while (true) {
    ...
    if (bytes > maximumBytes) {
      ...
      throw new DiagnosticFailure({
        action: "Give Redrob Work support the run ID; the diagnostic response exceeded its declared safety bound.",
        category: "http",
...
function requestHeaders(runId: string, step: string, additional: HeadersInit = {}): Headers {
  ...
  headers.set("user-agent", "redrob-den-egress-diagnostics/1.0")
  return headers
}

function diagnosticRunSignature(secret: string, runId: string, step: string): string {
  return createHmac("sha256", secret)
    .update(`redrob-diagnostics-v1\n${runId}\n${step}`)
    .digest("hex")
}
...
async function sendRequest(input: {
  ...
  if (!diagnosticIdPattern.test(diagnosticId)) {
    throw new DiagnosticFailure({
      action: "Ask the network administrator to inspect whether a proxy, gateway, or service mesh replaced the response or removed x-redrob-diagnostic-id.",
...
function protocolFailure(category: EgressDiagnosticCategory, code: string, message: string): DiagnosticFailure {
  return new DiagnosticFailure({
    action: "Give Redrob Work support the run ID and diagnostic reference so the response contract can be compared with the remote trace.",
    category,
    code,
    message,
    owner: "redrob-support",
  ...
  const tasks: Record<EgressDiagnosticStepId, (evidence: StepEvidence) => Promise<void>> = {
    reachability: async (evidence) => {
      ...
      requireValue(body.ok === true, {
        ...
        message: "The reachability endpoint returned an unexpected response.", owner: "redrob-support",
      })
    },
    "http-methods": async (evidence) => {
      ...
      const response = await sendRequest({
        ...
        init: {
          body: JSON.stringify({ probe: "redrob-egress-diagnostic" }),
    ...
    redirect: async (evidence) => {
      ...
      requireValue(redirectUrl.origin === origin && redirectUrl.pathname === "/diagnostics/egress", {
        ...
        message: "The controlled redirect did not remain on the Diagnostics origin.", owner: "redrob-support",
      })
    ...
    "oauth-discovery": async (evidence) => {
      ...
      requireValue(protectedMetadata.resource === `${origin}/mcp` && authorizationServers.includes(origin), {
        ...
        message: "OAuth protected-resource metadata did not describe the expected MCP resource.", owner: "redrob-support",
      })
      ...
      requireValue(authorizationMetadata.issuer === origin && authorizationMetadata.token_endpoint === `${origin}/oauth/token`, {
        ...
        message: "OAuth authorization-server metadata did not describe the expected token endpoint.", owner: "redrob-support",
      })
    },
    "oauth-token": async (evidence) => {
      ...
      const response = await sendRequest({
        ...
        init: {
          body: form.toString(),
          headers: {
            authorization: `Basic ${Buffer.from(`redrob-diagnostics:${input.bearerToken}`).toString("base64")}`,
      ...
      requireValue(body.token_type === "Bearer" && typeof body.access_token === "string" && body.access_token.length > 20, {
        ...
        message: "The OAuth-shaped token response did not contain a usable synthetic Bearer token.", owner: "redrob-support",
      })
    ...
    "mcp-handshake": async (evidence) => {
      ...
      const initialized = await sendRequest({
        ...
        init: {
          body: JSON.stringify({
            ...
            params: { capabilities: {}, clientInfo: { name: "redrob-den-egress-diagnostics", version: "1.0" }, protocolVersion: "2025-11-25" },
          }),
      ...
      requireValue(result !== null && result.protocolVersion === "2025-11-25" && session.length > 20 && version === "2025-11-25", {
        action: "Give Redrob Work support the run ID and initialize diagnostic reference.",
        category: "mcp", code: "mcp_initialize_contract_mismatch",
        message: "MCP initialization did not return the expected protocol version and session.", owner: "redrob-support",
      })
      ...
      requireValue(catalogResult !== null && Array.isArray(catalogResult.tools) && catalogResult.tools.length === 1, {
        action: "Give Redrob Work support the run ID and catalog diagnostic reference.",
        category: "mcp", code: "mcp_catalog_contract_mismatch",
        message: "MCP tool discovery did not return the single synthetic diagnostic tool.", owner: "redrob-support",
      })
      ...
      requireValue(tool !== null && typeof tool.name === "string" && tool.name.length > 0, {
        ...
        message: "MCP tool discovery returned a tool without a usable name.", owner: "redrob-support",
      })
      ...
      requireValue(toolResult !== null && toolResult.isError === false, {
        action: "Give Redrob Work support the run ID and tool-call diagnostic reference.",
        category: "mcp", code: "mcp_tool_contract_mismatch",
        message: "The synthetic MCP tool call was not reported as successful.", owner: "redrob-support",
      })
    },
  }
