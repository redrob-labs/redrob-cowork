...
export const GENERATED_ARTIFACT_VIEW_COMPILER = "redrob-react-view"
...
const HOST_GLOBAL_DEFINES = Object.fromEntries(HOST_GLOBAL_NAMES.map((name, index) => [
  name,
  `__redrob_forbidden_host_global_${index}__`,
]))
...
async function sourcePolicyDiagnostic(reactSource: string, cssSource: string): Promise<GeneratedArtifactViewBuildDiagnostic | null> {
  ...
  const hostGlobal = HOST_GLOBAL_NAMES.find((_, index) =>
    scopeAnalyzedSource.includes(`__redrob_forbidden_host_global_${index}__`))
  if (hostGlobal) {
...
(() => {
  ...
  window.__redrobReportArtifactRuntimeError = report;
...
async function buildClientBundle(reactSource: string): Promise<string> {
  const entry = `
    ...
    const mount = document.getElementById("redrob-artifact-view-root");
    const reportRuntimeError = (stage, error) => {
      const report = window.__redrobReportArtifactRuntimeError;
    ...
    const app = new App(
      { name: "Redrob Work Generated Artifact", version: "1.0.0" },
      {},
...
export async function buildGeneratedArtifactViewInWorker(input: GeneratedArtifactViewBuildInput): Promise<GeneratedArtifactViewBuildResult> {
  ...
  try {
    // esbuild parses and bundles generated source, but Redrob Work never executes
    ...
    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'sha256-${runtimeReporterDigest}' 'sha256-${scriptDigest}'; script-src-attr 'none'; style-src 'unsafe-inline'; img-src data:; font-src data:; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; worker-src 'none'"><title>${input.title.replace(/[<&>]/gu, "")}</title><style>${cssSource}</style></head><body><div id="redrob-artifact-view-root"></div><script>${GENERATED_ARTIFACT_RUNTIME_REPORTER}</script><script>${javascript}</script></body></html>`
