...
function safeBaseMessageFor(input: {
  ...
  if (input.code === "MCP_LIFECYCLE_DEADLINE") {
    return input.phase === "MCP_TOOL_EXECUTION"
      ...
      : "The MCP lifecycle exceeded Redrob Work's bounded diagnostic deadline."
  }
  ...
  if (input.code === "MCP_PROVIDER_DECLARED_ERROR") {
    const message = input.jsonRpcCode === undefined
      ? "The provider answered with a JSON-RPC error Redrob Work does not recognize."
      : `The provider answered with a JSON-RPC error Redrob Work does not recognize (code ${input.jsonRpcCode}).`
    return input.providerErrorMessage
      ? `${message} Provider-declared message (untrusted): "${input.providerErrorMessage}".`
      : message
  }
  if (input.code === "MCP_RESPONSE_BODY_LIMIT") {
    return "The MCP server returned a response body larger than Redrob Work can safely process."
  }
  if (input.highestPassed && HEALTH_RANK[input.highestPassed] >= HEALTH_RANK.protocol_ready && isUninformativeClassification(input)) {
    return "The MCP server answered, but Redrob Work could not interpret its response for the current request."
  }
  ...
  if (input.phase === "AUTH_CLIENT_REGISTRATION") {
    return "Redrob Work could not register or identify its OAuth client with the authorization server."
  }
  ...
  if (input.phase === "MCP_TOOL_DISCOVERY") {
    return input.code === "MCP_CATALOG_CURSOR_LOOP"
      ? "The MCP server repeated a tool-catalog cursor, so Redrob Work stopped safely."
      : "Redrob Work could not retrieve a complete, valid MCP tool catalog."
  }
  ...
  return "The MCP connection failed before Redrob Work could complete the protocol lifecycle."
}
...
function classifyError(error: unknown, fallbackPhase: ExternalMcpDiagnosticPhase): Classification {
  ...
  if (hasUnsupportedVersionMessage(error)) {
    return {
      ...
      retryable: false,
      actionOwner: "provider_admin",
      operatorAction: "Configure the provider to support an MCP protocol version compatible with Redrob Work.",
    }
  }
