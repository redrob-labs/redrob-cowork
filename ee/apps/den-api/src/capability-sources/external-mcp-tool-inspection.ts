...
export function diagnoseExternalMcpToolCall(input: {
  ...
  if (!input.inspection.request) {
    if (input.diagnostic?.phase.startsWith("NETWORK_")) {
      return {
        status: "failed",
        layer: "network",
        summary: "Redrob Work could not reach the remote MCP while preparing the session, so tools/call was not sent.",
      }
    }
    if (
    ...
    return {
      status: "failed",
      layer: "redrob",
      summary: "The call failed inside Redrob Work before an outbound tools/call request was sent.",
    }
  }
  if (!input.inspection.response) {
    ...
    if (
      ...
      return {
        status: "failed",
        layer: "redrob",
        summary: "Redrob Work's outbound network safety policy blocked this tools/call request, so it was not sent to the remote MCP.",
      }
    }
    ...
    return {
      status: "failed",
      layer: "network",
      summary: "Redrob Work started tools/call but did not capture an HTTP response. This does not prove the remote MCP caused the failure.",
    }
  }
  ...
  return {
    ...
    summary: input.diagnostic?.code === "MCP_PROVIDER_AUTH_REQUIRED"
      ...
      : "The remote MCP answered, but Redrob Work could not accept the MCP response as a successful tool result.",
  }
}
