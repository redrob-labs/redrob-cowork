...
import { createHeadlessThreadClient, type HeadlessThreadTranscript } from "@redrob/headless-threads"
import { and, asc, eq, isNull } from "@redrob-ee/den-db/drizzle"
import { MemberTable, OrganizationTable, WorkerTable } from "@redrob-ee/den-db/schema"
import { normalizeDenTypeId } from "@redrob-ee/utils/typeid"
import type { AutomationAction, AutomationError, AutomationUsage } from "@redrob/types/automations"
...
async function readyWorker(scope: OwnerScope, signal: AbortSignal) {
  const worker = await ownerCloudWorker(scope)
  if (!worker) return { ok: false as const, message: "Set up Redrob Work Cloud before creating a Cloud Automation." }
  if (worker.status === "failed") return { ok: false as const, message: "The Redrob Work Cloud runtime needs repair before this Automation can run." }
  ...
  return { ok: false as const, message: "Redrob Work Cloud could not be started for this Automation run." }
}
...
async function connectHealth(input: {
  ...
  let value = await request("GET", `/workspace/${encodedWorkspace}/mcp/redrob-cloud/health?${query}`)
  let health = value
  if (health?.usable !== true || health.usableByCurrentModel !== true) {
    value = await request("POST", `/workspace/${encodedWorkspace}/mcp/redrob-cloud/engine-refresh`, {
      provider: input.action.model.providerId,
      model: input.action.model.modelId,
  ...
  if (health?.usable === true && health.usableByCurrentModel !== true) {
    return { ok: false, code: "model_access_lost", message: "The selected model cannot use the current Redrob Work Connect capabilities." }
  }
  ...
  return {
    ...
    message: typeof failure?.message === "string"
      ? failure.message
      : "Redrob Work Connect is not ready in the Cloud runtime. Reconnect it before retrying this Automation.",
  }
}
...
export async function executeCloudAgent(input: CloudAgentExecutorInput): Promise<CloudAgentExecution> {
  ...
  try {
    ...
    if (!runtime.ok) {
      ...
      return {
        ...
        message: cancelled ? "The Automation run was cancelled."
          : deadlineController.signal.aborted ? "The Automation run exceeded its maximum runtime while starting Redrob Work Cloud." : runtime.message,
        retryable: false,
    ...
    if (authorityFailure) {
      ...
      if (recoveringNativeThread && !await abortAndObserve(client, nativeThreadId)) {
        return {
          ...
          message: "Model authority was revoked, but Redrob Work Cloud could not confirm that the native thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
    ...
    if (waited.outcome !== "settled") {
      ...
      if (!stopped) {
        return {
          ...
          status: "failed",
          code: "execution_failed",
          message: "Redrob Work Cloud could not confirm that the cancelled agent thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
    ...
    const resultSummary = transcript.finalAssistantText.trim() || "Redrob Work Cloud completed the Automation run."
    return {
      ok: true,
    ...
    if (client && nativeThreadId) {
      ...
      if (!stopped) {
        return {
          ...
          status: "failed",
          code: "execution_failed",
          message: "Redrob Work Cloud could not confirm that the interrupted agent thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
          events: [{ type: "warning", payload: { code: "abort_not_observed", nativeThreadId } }],
        }
