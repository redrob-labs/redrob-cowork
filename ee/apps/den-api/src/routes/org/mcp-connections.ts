...
const clientMetadataResponseSchema = z.object({
  client_id: z.string(),
  client_name: z.literal("Redrob Work"),
...
const connectionToolInspectionDiagnosisSchema = z.object({
  ...
  layer: z.enum(["redrob", "network", "mcp_connection", "remote_http", "mcp_tool"]),
  summary: z.string(),
...
const externalMcpDiagnosticSchema = z.object({
  ...
  retryable: z.boolean(),
  actionOwner: z.enum(["redrob", "network_admin", "provider_admin", "organization_admin", "member"]),
...
function legacyExternalMcpConnectionIdsFromPayload(payload: Record<string, unknown> | null): string[] {
  ...
  const collect = (value: unknown) => {
    ...
    if (value.redrobManaged !== "den_external_mcp") return
...
export function registerMcpConnectionRoutes<T extends { Variables: OrgRouteVariables & RequestIdVariables }>(app: Hono<T>) {
  app.get(
    ...
    describeRoute({
      tags: ["Authentication"],
      summary: "Redrob Work external MCP OAuth client metadata",
    ...
    (c) => {
      ...
      return c.json({
        client_id: clientId,
        client_name: "Redrob Work" as const,
    ...
    async (c) => {
      ...
      if (body.kind === "native_provider") {
        if (isAgentOAuthClientConnection({ oauthClient: body.oauthClient, sessionId })) {
          return c.json({ error: "invalid_request", message: "OAuth client credentials cannot be set from the agent. Add them in the Redrob Work Cloud dashboard under Extensions." }, 400)
        }
      ...
      if (isAgentApiKeyConnection({ authType: body.authType, sessionId })) {
        return c.json({ error: "invalid_request", message: "API-key connections cannot be created from the agent. Add them in the Redrob Work Cloud dashboard under Extensions." }, 400)
      }
    ...
    async (c) => {
      ...
      if (sessionId === "mcp_internal" && (body.apiKey !== undefined || body.oauthClient !== undefined)) {
        return c.json({
          ...
          message: "Connection credentials cannot be edited from the agent. Use the Redrob Work Cloud dashboard under Connections.",
        }, 400)
      }
      ...
      try {
        ...
        if (diagnostic.code === "MCP_OAUTH_CONFIGURATION_REQUIRED") {
          return c.json({
            ...
            message: "This authorization server requires a pre-registered OAuth client before Redrob Work can connect.",
            ...
            manualRequirements: [
              ...
              "Allowlist the callback URL shown by Redrob Work.",
              "Save the client ID and optional client secret in Redrob Work.",
            ],
          }, 409)
        }
        if (diagnostic.code === "MCP_OAUTH_ISSUER_MISMATCH") {
          return c.json({
            ...
            message: issuerRepairRequiresAdmin
              ...
              : "Redrob Work could not safely verify the authorization server selected for this MCP connection. Ask a workspace admin to review its OAuth setup.",
          }, 409)
        }
        return c.json({
