...
function buildInstallConfig(input: { organization: { name: string; logo: string | null; metadata: unknown }; request: Request }) {
  ...
  return installConfigSchema.parse({
    appName: typeof metadata.brandAppName === "string" ? metadata.brandAppName : "Redrob Work",
...
export function registerOrgInstallLinkRoutes<T extends { Variables: OrgRouteVariables }>(
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Mints a shareable Redrob Work desktop install link for a signed-in organization member. Older active links remain valid unless an owner or admin explicitly requests rotation.",
      responses: {
  ...
  app.get(
    ...
    describeRoute({
      tags: ["Users"],
      summary: "Download current user's managed Redrob Work desktop",
  ...
  app.get(
    ...
    describeRoute({
      tags: ["Organizations"],
      summary: "Download managed Redrob Work desktop",
