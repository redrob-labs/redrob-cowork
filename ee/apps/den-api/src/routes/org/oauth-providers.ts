...
export function registerOAuthProviderRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  app.post(
    ...
    describeRoute({
      ...
      description: "Admin-only. Lets an org bring its own OAuth app (client id + secret) for a native provider such as google-workspace, instead of relying on an Redrob Work-owned client.",
      responses: {
  ...
  app.get(
    ...
    describeRoute({
      ...
      description: "The provider redirects here with code+state after the member consents. Identity is carried entirely by the signed state token, not a session cookie, since the redirect may arrive in a fresh browser context. Serves a small static HTML page that deep-links back to Redrob Work.",
      responses: {
      ...
      try {
        ...
        if (!saved) {
          return c.html(connectCallbackPage({
            ...
            message: "This Redrob Work connection request is no longer active.",
          }), 400)
        }
      } catch (error) {
        ...
        return c.html(connectCallbackPage({
          ok: false,
          name: provider.displayName,
          message: "Redrob Work could not finish the OAuth connection. Try Connect again; if it still fails, contact support with the diagnostic reference.",
          referenceId: requestId,
        }), 400)
      }
