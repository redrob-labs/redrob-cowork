...
async function configurePluginMcpConnectionResponse(c: OrgContext) {
  try {
    ...
    if (isAgentPluginMcpSecretSetup({ apiKey: body.apiKey, oauthClient: body.oauthClient, sessionId: c.get("session")?.id })) {
      return c.json({ error: "invalid_request", message: "Plugin MCP credentials cannot be set from the agent. Add them in the Redrob Work Cloud dashboard under Connections." }, 400)
    }
...
export function registerPluginArchRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  ...
  withPluginArchOrgContext(app, "get", pluginArchRoutePaths.meLibrary,
    describeRoute({
      ...
      description: "Lists the Workflows, Remote MCP Apps, plugins, and connections the caller can use, with every applicable access edge. Workflows and Remote MCP Apps remain config objects contained by their parent Redrob Work Connect Plugin.",
      responses: {
  ...
  withPluginArchOrgContext(app, "post", pluginArchRoutePaths.connectorInstanceDiscoveryApply,
    ...
    describeRoute({
      ...
      description: "Creates Redrob Work plugins and connector mappings from selected discovery candidates.",
      responses: {
