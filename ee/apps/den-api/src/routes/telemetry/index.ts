...
export function registerTelemetryRoutes<T extends { Variables: TelemetryRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Receives a batch of telemetry events from the Redrob Work app or workers. Auth provides org and member identity. Unknown event types and disallowed fields are dropped. Always returns 204.",
      responses: {
