import { asc, and, eq, gt, isNull, lt, lte } from "@redrob-ee/den-db/drizzle"
import { AuthSessionTable, AuthUserTable, InvitationTable, MemberTable, OAuthConsentTable, OrganizationTable } from "@redrob-ee/den-db/schema"
import { normalizeDenTypeId, type DenTypeId, type DenTypeIdName } from "@redrob-ee/utils/typeid"
...
redisClient?.on("error", (error) => {
  console.error("redrob_cache_redis_error", error)
})
...
async function getOrSet<T>(input: {
  ...
  try {
    ...
    console.error("redrob_cache_get_failed", { key, error })
  }
  ...
  try {
    ...
    console.error("redrob_cache_set_failed", { key, error })
  }
  return loaded
}
...
async function getAuthSessionResult(token: string): Promise<CacheResult<CachedAuthSession | null>> {
  ...
  if (redis) {
    try {
      ...
      console.error("redrob_cache_get_failed", { ...cacheLogDetails(keyInput), error })
    }
  }
  ...
  if (ttl > 0) {
    try {
      ...
      console.error("redrob_cache_set_failed", { ...cacheLogDetails(keyInput), error })
    }
  }
  return { value: loaded, source: "loader" }
}
...
async function deleteAuthSession(token: string) {
  ...
  try {
    ...
    console.error("redrob_cache_delete_failed", { ...cacheLogDetails(keyInput), error })
  }
}
...
async function revokeAuthSession(token: string) {
  ...
  try {
    ...
    console.error("redrob_cache_delete_failed", { ...cacheLogDetails(revokedKeyInput), error })
  }
}
...
async function deleteOrgMemberList(organizationId: OrgId) {
  ...
  try {
    ...
    console.error("redrob_cache_delete_failed", { key, error })
  }
}
