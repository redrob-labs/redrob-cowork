...
export const DEFAULT_INSTALLER_RELEASE_REPO = "redrob-labs/redrob-work"
...
export function desktopReleaseAssetName(platform: string, releaseTag: string) {
  ...
  if (platform === "mac-arm64" || platform === "mac-x64") {
    return `redrob-${platform}-${version}.dmg`
  }
  if (platform === "win-x64") {
    return `redrob-${platform}-${version}.exe`
  }
  if (platform === "linux-x64") {
    return `redrob-linux-x86_64-${version}.AppImage`
  }
  if (platform === "linux-arm64") {
    return `redrob-linux-arm64-${version}.AppImage`
  }
  return null
}
...
export function enterpriseDesktopReleaseAssetName(platform: string, releaseTag: string) {
  ...
  return publicName?.replace(/^redrob-/, "redrob-enterprise-") ?? null
}

export function cloudDesktopReleaseAssetName(platform: string, releaseTag: string) {
  const publicName = desktopReleaseAssetName(platform, releaseTag)
  return publicName?.replace(/^redrob-/, "redrob-cloud-") ?? null
}
