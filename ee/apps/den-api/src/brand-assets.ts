...
export async function validateAndNormalizeBrandAsset(input: {
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) {
      return failure("invalid-image", "Redrob Work could not read that image's dimensions.")
    }
    ...
    return failure("invalid-image", "Redrob Work could not decode that image. Use a valid PNG or JPEG file.")
  }
}
