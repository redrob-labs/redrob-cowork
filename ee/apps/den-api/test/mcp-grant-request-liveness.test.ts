...
const GRANT_CLAIM = "https://redrob.io/grant_id"
...
function validMcpJwtPayload(includeGrant: boolean) {
  const payload: Record<string, unknown> = {
    ...
    "https://redrob.io/resource": AGENT_RESOURCE,
...
beforeAll(async () => {
  seedRequiredEnv()
  mock.module("@redrob-ee/den-db/schema", () => ({
    AuthSessionTable,
    AuthUserTable,
    InvitationTable,
    MemberTable,
  ...
  mock.module("@redrob-ee/den-db/drizzle", () => ({
...
test("grant-claim tokens stay valid without consulting their dead login session", async () => {
  ...
  try {
    ...
    if (!(principal instanceof Response)) {
      expect(principal.organizationId).toBe(jwtPayload["https://redrob.io/org_id"])
    }
