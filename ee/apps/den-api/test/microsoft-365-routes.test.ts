...
describe("Microsoft 365 injected routes", () => {
  test("maps Graph mail for the calling member and blocks disconnected or under-scoped accounts", async () => {
    ...
    expect(await missingScopeResponse.json()).toEqual({
      ...
      message: "Your connected Microsoft account is missing the Outlook mail read permission. An admin can enable it on the Microsoft 365 connector in Redrob Work Cloud -> Connectors; then reconnect your account.",
    })
    expect(graphCalls).toBe(1)
  ...
  test("executes enabled write and Teams capabilities with the member's delegated scopes", async () => {
    ...
    const graphFetch: typeof fetch = async (input, init) => {
      ...
      if (decodeURIComponent(url.pathname).endsWith("/me/drive/root:/Redrob Work/notes.txt:/content")) {
    ...
    const fileResponse = await app.request("http://den-api.local/v1/capabilities/microsoft-365/drive-files", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ path: "Redrob Work/notes.txt", content: "Notes" }),
    })
  ...
  test("accepts stronger scopes for enabled reads but rejects writes without their feature or grant", async () => {
    ...
    expect(await denied.json()).toEqual({
      error: "needs_connection",
      message: "Your connected Microsoft account is missing the Outlook mail read/write permission. An admin can enable it on the Microsoft 365 connector in Redrob Work Cloud -> Connectors; then reconnect your account.",
    })
    expect(graphCalls).toBe(1)
  })
