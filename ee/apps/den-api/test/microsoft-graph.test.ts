...
describe("MicrosoftGraphClient", () => {
  ...
  test("creates drafts and events, writes OneDrive text, and reads/sends Teams chat", async () => {
    ...
    const fetchMock: typeof fetch = async (input, init) => {
      ...
      if (decodeURIComponent(url.pathname).endsWith("/me/drive/root:/Redrob Work/Review notes.txt:/content")) {
    ...
    expect((await client.putDriveTextFile({ path: "Redrob Work/Review notes.txt", content: "Notes" })).id).toBe("file_1")
...
test("OneDrive path search escapes OData apostrophes", () => {
  ...
  expect(encodeOneDrivePath("Redrob Work/Review notes.txt")).toBe("Redrob Work/Review%20notes.txt")
})
