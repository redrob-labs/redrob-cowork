...
describe("public API URL configuration", () => {
  test("callback base resolution keeps the configured prefix", async () => {
    ...
    expect(resolvePublicApiBaseUrl(
      ...
      "https://redrob.example/api/den/",
    )).toBe("https://redrob.example/api/den")
  })

  test("preserves and normalizes an HTTPS pathname prefix", () => {
    expect(normalizeConfiguredPublicApiBaseUrl("https://redrob.example/api/den/", {
      allowInsecureHttp: false,
    })).toBe("https://redrob.example/api/den")
  })

  test("requires HTTPS outside development and localhost", () => {
    expect(() => normalizeConfiguredPublicApiBaseUrl("http://redrob.example/api/den", {
      allowInsecureHttp: false,
    ...
    })).toBe("http://redrob.example/api/den")
  })
  ...
  test("rejects malformed and non-base URLs", () => {
    ...
    expect(() => normalizeConfiguredPublicApiBaseUrl("https://redrob.example/api?tenant=one", {
      allowInsecureHttp: false,
