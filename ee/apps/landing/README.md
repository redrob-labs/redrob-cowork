...
1. Install deps from repo root:
   `pnpm install`
2. Run the app:
   `pnpm --filter @redrob-ee/landing dev`
...
- `LOOPS_INTERNAL_FEEDBACK_EMAIL` - legacy feedback recipient override, used only when `REDROB_FEEDBACK_EMAIL` is not set
...
2. Build command: `pnpm --filter @redrob-ee/landing build`
3. Output: `.next`
4. Start command: `pnpm --filter @redrob-ee/landing start`
5. Enable Vercel BotID for the project so protected form routes can reject automated submissions.

### Self-hosted

1. Build: `pnpm --filter @redrob-ee/landing build`
2. Start: `pnpm --filter @redrob-ee/landing start`
