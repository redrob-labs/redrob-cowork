...
//
// Resolves the latest Redrob Work desktop release on GitHub and returns an install
// manifest in the shape the `redrob-bootstrap install app` command expects:
//
...
const GITHUB_REPO = "redrob-labs/redrob-work";
...
// Only treat Redrob Work desktop-app installers as artifacts. Historical sidecar
// and CLI release assets must NOT be treated as the desktop app, so we
// positively require the desktop app's OS-tagged naming (redrob-mac /
// redrob-linux / redrob-win + an installer extension).
const SIDECAR_HINTS = ["orchestrator", "server", "bun-", "sidecar", "opencode"];

function isDesktopAppAsset(name: string): boolean {
  const lower = name.toLowerCase();
  if (!lower.startsWith("redrob")) return false;
  // exclude update metadata / checksums
  if (/\.(blockmap|yml|yaml|json|txt|sig|sha256)$/i.test(lower)) return false;
  // exclude any sidecar/CLI binary that also happens to start with "redrob".
  ...
  for (const asset of release.assets) {
    ...
    if (!existing || pref > existing._pref) {
      artifacts[target.platform][target.arch] = {
        ...
        ...(type === "dmg" ? { appName: "Redrob Work.app" } : {}),
      };
    }
  }
  ...
  try {
    const response = await fetch(RELEASES_API, {
      headers: {
        ...
        "user-agent": "redrob-install-manifest",
      },
      next: { revalidate: 300 },
    });
    if (!response.ok) {
    ...
    // Redrob Work desktop installer, not sidecar or CLI bundles.
