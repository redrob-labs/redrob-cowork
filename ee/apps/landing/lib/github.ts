...
export const getGithubData = async () => {
  ...
  const [repo, latestRelease, releases] = await Promise.all([
    fetchJson<Repo>("https://api.github.com/repos/redrob-labs/redrob-work"),
    fetchJson<Release>(
      "https://api.github.com/repos/redrob-labs/redrob-work/releases/latest"
    ),
    fetchJson<Release[]>(
      "https://api.github.com/repos/redrob-labs/redrob-work/releases?per_page=50"
    )
  ]);
  ...
  const isElectronDesktopAsset = (name: string) =>
    name.startsWith("redrob-mac-") ||
    name.startsWith("redrob-win-") ||
    name.startsWith("redrob-linux-");
  ...
  const hasWindowsDesktopAsset = (release: Release) => {
    ...
    return assets.some((asset) => {
      const name = String(asset?.name || "").toLowerCase();
      return name.startsWith("redrob-win-x64-") && name.endsWith(".exe");
    });
  };
  ...
  const isNonPublicDesktopAsset = (asset: ReleaseAsset) => {
    const name = String(asset?.name || "").toLowerCase();
    return name.startsWith("redrob-installer-")
      || name.startsWith("redrob-cloud-")
      || name.startsWith("redrob-enterprise-");
  };
  ...
  const dmg = selectAsset(assets, [".dmg"], ["redrob-mac-"]);
  const exe = selectAsset(windowsAssets, [".exe"], ["redrob-win-"]);
