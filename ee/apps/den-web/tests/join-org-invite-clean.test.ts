...
describe("join organization invite clean layout contract", () => {
  ...
  test("removes stacked frames while using the shared wide welcome card hierarchy", () => {
    ...
    expect(cardSource).toContain("/redrob-mark.svg");
    expect(cardSource).toContain("Redrob Work Cloud");
  ...
  test("keeps onboarding focused and reuses the responsive platform grid", () => {
    ...
    expect(successSource).toContain("Already have Redrob Work? Open it.");
    ...
    expect(successSource).toContain("Return to Redrob Work");
    ...
    expect(successSource).not.toContain("Open Redrob Work");
    ...
    expect(installSource).toContain("Set up Redrob Work Enterprise");
    expect(installSource).toContain("Download Redrob Work");
