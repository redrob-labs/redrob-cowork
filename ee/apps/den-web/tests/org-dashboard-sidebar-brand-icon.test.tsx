...
describe("Den dashboard sidebar brand icon", () => {
  ...
  test("renders the canonical managed square icon without first rendering the Redrob Work mark", () => {
    const markup = renderToStaticMarkup(
  ...
  test("uses the Redrob Work mark when no managed square icon exists", () => {
    const markup = renderToStaticMarkup(
      <SidebarBrandMark metadata={null} organizationName="Acme" />,
    );

    expect(markup).toContain("<svg");
    expect(markup).toContain('aria-label="Redrob Work"');
