...
describe("mcpAuthorizationPendingDocument", () => {
  test("renders a concise accessible connection screen before provider redirect", () => {
    ...
    expect(document).toContain("Redrob Work Connect")
  ...
  test("keeps OAuth failures visible with the exact redirect URI", () => {
    const document = mcpAuthorizationErrorDocument({
      ...
      details: {
        ...
        redirectUri: "https://api.redrob.example/v1/mcp-connections/oauth/callback",
        clientMetadataUrl: "https://api.redrob.example/.well-known/oauth-client",
        responseJson: JSON.stringify({
          error: "mcp_oauth_configuration_required",
          callbackUrl: "https://api.redrob.example/v1/mcp-connections/oauth/callback",
        }, null, 2),
      },
    })
    ...
    expect(document).toContain("https://api.redrob.example/v1/mcp-connections/oauth/callback")
