...
export function diagnosisLayerLabel(layer: ExternalMcpToolCallInspection["diagnosis"]["layer"]): string {
  if (layer === "redrob") return "Redrob Work before send";
...
function transportLabel(inspection: ExternalMcpToolCallInspection | null): string {
  ...
  if (inspection?.request && inspection.diagnosis.layer !== "redrob") return "No response";
  return "Not sent";
}
...
function traceStates(outcome: McpToolCallOutcome): [HopState, HopState, HopState] {
  ...
  if (!inspection || inspection.diagnosis.layer === "redrob") return ["failed", "unreached", "unreached"];
...
function RedactionNotice() {
  return (
    <div className="rounded-lg border border-amber-100 bg-amber-50/70 px-3 py-2 text-[10px] leading-4 text-amber-800">
      Credential and session headers are redacted. Bodies may contain sensitive provider data; this inspection is returned only for this run and is not stored in Redrob Work logs.
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {inspection.request ? (
          ...
          <p className="mt-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[11px] text-gray-500">No tools/call request left Redrob Work.</p>
        )}
      </div>
      {inspection.request ? (
        <>
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white" aria-label="Tool call inspection">
      ...
      <div className="flex flex-wrap items-center px-4 py-3">
        <TracePill label="Redrob Work" state={states[0]} />
        ...
        {activeTab === "result" ? (
          <div className="space-y-2">
            ...
            <p className="inline-flex items-center gap-1.5 text-[10px] text-gray-500">
              <LockKeyhole className="h-3 w-3" aria-hidden="true" /> Credential headers redacted. Returned only for this run — never stored in Redrob Work logs.
            </p>
          </div>
        ) : activeTab === "request" ? (
