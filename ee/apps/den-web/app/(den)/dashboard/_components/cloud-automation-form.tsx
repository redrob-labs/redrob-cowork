...
export function CloudAutomationForm({ automation, workflow, onClose, onSaved }: Props) {
  ...
  const models = useMemo(() => llmProviders.flatMap((provider) => provider.models.map((model) => ({
    key: `${provider.source === "redrob" ? "redrob" : provider.id}:${model.id}`,
    providerId: provider.source === "redrob" ? "redrob" : provider.id,
  ...
  return (
    <section className="mt-6 rounded-2xl border border-sky-100 bg-sky-50/40 p-5">
      ...
      {!cloudEnabled ? <p className="mt-4 rounded-xl border border-amber-100 bg-amber-50 px-3 py-2 text-[12px] text-amber-700">Redrob Work Cloud is not enabled for this workspace.</p> : null}
