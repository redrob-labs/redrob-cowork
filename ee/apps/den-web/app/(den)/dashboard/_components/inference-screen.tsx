...
const PILLARS = [
  {
    ...
    body: "One subscription activates Redrob Models across your organization and lets everyone use battle-tested LLMs without setting anything up.",
  },
  {
...
const STEPS: ReactNode[] = [
  ...
  <>
    Open Redrob and pick any model from the{" "}
    <code className="rounded-md bg-gray-100 px-2 py-1 font-mono text-[12px] text-gray-700">Redrob</code> group
  </>,
...
export function InferenceScreen() {
  ...
  async function startSubscribeCheckout() {
    if (!canManageModels) {
      setError("Only workspace admins can start Redrob Models checkout.");
      return;
    }
  ...
  async function toggleEnabled() {
    if (!canManageModels) {
      setError("Only workspace admins can manage Redrob Models.");
      return;
    }
    if (!status) return;
  ...
  return (
    <div className="mx-auto grid max-w-[860px] gap-6 px-8 pb-16 pt-8">
      ...
      {canManageModels ? null : (
        <DenNotice
          tone="info"
          message="Only workspace admins can subscribe or enable Redrob Models. Ask an owner, super-admin, or admin for this workspace."
        />
      )}
