export type ExternalMcpDiagnostic = {
  ...
  code?: string;
  retryable?: boolean;
  actionOwner?: "redrob" | "network_admin" | "provider_admin" | "organization_admin" | "member";
...
function lastBoundaryFromDiagnostic(diagnostic: ExternalMcpDiagnostic | null): string | undefined {
  if (diagnostic?.highestPassed === "operation_ready") return "Redrob Work started remote tool execution";
  if (diagnostic?.highestPassed === "catalog_ready") return "Redrob Work loaded the remote MCP tool catalog";
  if (diagnostic?.highestPassed === "protocol_ready") return "Redrob Work initialized the remote MCP session";
  if (diagnostic?.highestPassed === "authorized") return "Remote MCP accepted the connection credential";
  if (diagnostic?.highestPassed === "reachable") return "Redrob Work reached the remote MCP endpoint";
  if (diagnostic?.highestPassed === "configured") return "Redrob Work loaded the MCP connection configuration";
  return undefined;
}
...
export function attributeExternalMcpToolFailure(input: {
  ...
  if (browserTimeout) {
    ...
    return {
      summary: `Redrob Work stopped waiting after ${duration} seconds. The operation’s outcome is unknown.`,
      lastConfirmedBoundary: "Redrob Work dashboard sent the request",
      likelySource: "Source unclear after Redrob Work timeout",
      confidence: "Inferred",
  ...
  if (blockedBeforeSend) {
    return {
      summary: "Redrob Work blocked the request before it was sent.",
      lastConfirmedBoundary: "Redrob Work evaluated the outbound request",
      likelySource: "Redrob Work",
      confidence: "Confirmed",
      retryGuidance: diagnostic?.operatorAction ?? "Resolve the Redrob Work policy or connection configuration, then run the tool again.",
      outcome: "failed",
      ...details,
    };
  }
  ...
  if (deadlineAfterSend) {
    return {
      summary: "Redrob Work sent the request, but the remote MCP did not respond before Redrob Work’s deadline.",
      lastConfirmedBoundary: "Redrob Work started the outbound tools/call",
  ...
  if (inspection?.request && !inspection.response) {
    return {
      summary: "Redrob Work started the outbound request, but no HTTP response was captured. This does not prove the remote MCP caused the failure.",
      lastConfirmedBoundary: "Redrob Work started the outbound tools/call",
  ...
  return {
    summary: inspection?.diagnosis?.summary
      ?? diagnostic?.message
      ?? "The request failed before Redrob Work received a tool result.",
    lastConfirmedBoundary: lastBoundaryFromDiagnostic(diagnostic)
      ?? (diagnostic ? "Redrob Work returned a structured diagnostic" : "Redrob Work dashboard sent the request"),
    likelySource: networkSetup ? "Connection path or remote MCP" : "Redrob Work or MCP setup",
    confidence: "Inferred",
    retryGuidance: diagnostic?.operatorAction ?? "Use the diagnostic reference to inspect Redrob Work and MCP connection health before retrying.",
    outcome: "failed",
    ...details,
  };
}
