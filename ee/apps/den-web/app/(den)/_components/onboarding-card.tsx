...
const REDROB_MARK_SRC = "/redrob-mark.svg";
...
function BrandIcon({ iconUrl }: { iconUrl: string | null }) {
  ...
  const iconSrc = iconUrl && iconUrl !== failedIconUrl ? iconUrl : REDROB_MARK_SRC;
  const isFallbackIcon = iconSrc === REDROB_MARK_SRC;
...
function BrandLabel({ organization }: { organization: OnboardingCardOrganization | null }) {
  if (!organization) {
    return <span>Redrob Work Cloud</span>;
  }
