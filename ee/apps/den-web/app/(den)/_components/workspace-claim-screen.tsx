  ...
  return (
    <section className="den-page py-4 lg:py-6">
      <div className="den-frame grid max-w-[44rem] gap-4 p-6 md:p-7">
        <p className="den-eyebrow">Redrob Work Cloud</p>
...
const AUTO_ACCEPT_WORKSPACE_CLAIM_STORAGE_KEY = "redrob:web:auto-accept-workspace-claim";
...
function getRedrobUrl(payload: unknown): string | null {
  if (typeof payload !== "object" || payload === null) {
    return null;
  }

  const url = (payload as { redrobUrl?: unknown }).redrobUrl;
...
export function WorkspaceClaimScreen({
  ...
  async function createDesktopHandoff(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(getErrorMessage(payload, `Could not prepare an Redrob Work link (${response.status}).`));
    }

    const redrobUrl = getRedrobUrl(payload);
    if (!redrobUrl) {
      throw new Error("No Redrob Work link was returned.");
    }
  ...
  async function handleOpenDesktop() {
    ...
    try {
      ...
      setHandoffError(error instanceof Error ? error.message : "Could not open Redrob Work.");
    } finally {
      setHandoffBusy(false);
    }
  }

  async function handleCopyRedrobWorkLink() {
    setCopyBusy(true);
    setLinkCopied(false);
    setHandoffError(null);
    ...
    try {
      ...
      const redrobUrl = await createDesktopHandoff();
      await navigator.clipboard.writeText(redrobUrl);
      setLinkCopied(true);
      window.setTimeout(() => setLinkCopied(false), 1800);
    } catch (error) {
      setHandoffError(error instanceof Error ? error.message : "Could not copy the Redrob Work link.");
    } finally {
      setCopyBusy(false);
    }
  }
        ...
        <div className={`den-frame mx-auto grid w-full max-w-[38rem] gap-7 p-7 text-center md:p-10 ${isLoopback ? "border-blue-200/80 bg-white/90 shadow-[0_28px_80px_-44px_rgba(37,99,235,0.45)]" : ""}`}>
          ...
          <div className="grid justify-items-center gap-3">
            ...
            <p className="den-copy max-w-[46ch]">
              Copy the Redrob Work link, open Redrob Work Enterprise, and paste it to connect. You will not need to enter your password again.
            </p>
          </div>
          ...
          <div className="grid gap-3">
            <button
              ...
              onClick={() => void handleCopyRedrobWorkLink()}
              disabled={handoffBusy || copyBusy}
            >
              <Copy className="size-4" aria-hidden />
              {copyBusy ? "Copying..." : linkCopied ? "Link copied" : "Copy Redrob Work link"}
            </button>
            ...
            <div className="flex flex-col items-center justify-center gap-2 text-sm sm:flex-row sm:gap-5">
              ...
              >
                ...
                {handoffBusy ? "Opening Redrob Work..." : "Open Redrob Work"}
              </button>
          ...
          {linkCopied ? (
            <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-800">
              Open Redrob Work Enterprise and paste the Redrob Work link to connect.
            </div>
          ) : null}

          {handoffAttempted && !handoffError ? (
            <p className="den-copy text-sm">
              Opening Redrob Work now. If nothing happens (for example, the app isn&apos;t installed on this machine), use &quot;Continue in browser instead&quot;.
            </p>
          ) : null}
