  ...
  async function submitAccount(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const { response, payload } = await requestJson("/api/auth/sign-up/email", {
        ...
        body: JSON.stringify({
          email,
          name: name.trim() || "Redrob Work Administrator",
          password,
      ...
      <div className="den-frame mx-auto grid w-full max-w-[680px] gap-8 p-5 sm:p-8 md:p-10" data-testid="initial-admin-setup">
        <div className="grid gap-3">
          ...
          <p className="den-copy">
            {complete
              ? "This Redrob Work deployment already has its first administrator. Continue with the normal sign-in flow."
              : "Create the first administrator for this private Redrob Work deployment. Public signup remains disabled during setup."}
          </p>
        </div>
        ...
        {status === "loading" ? (
          ...
          <form className="grid gap-5" onSubmit={submitAccount}>
            <div className="grid gap-2">
              ...
              <p className="den-copy">Enter your name and password. Redrob Work will create the first account, sign you in, and permanently close setup.</p>
            </div>
            <div className="grid gap-2">
