export const RedrobWorkConnectInstaller = () => {
  ...
  const CODEX_LOGIN_COMMAND = "codex mcp login redrob";
  const CODEX_RECONNECT_COMMAND = `codex mcp logout redrob
codex mcp login redrob`;
  const OPENCODE_AUTH_COMMAND = "opencode mcp auth redrob";
  const OPENCODE_RECONNECT_COMMAND = `opencode mcp logout redrob
opencode mcp auth redrob`;
  const installs = [
    ...
    },
    {
      ...
      supportStatus: "Setup only",
      supportExplanation: "Setup guide only: add Redrob Work, run codex mcp login redrob, and reconnect with logout then login. Native proof must be rerun on this exact branch.",
      copyText: `codex mcp add redrob --url ${MCP_SERVER_URL}`,
    ...
    },
    {
      ...
      copyText: `claude mcp add --transport http redrob ${MCP_SERVER_URL}`,
    },
    {
      id: "opencode",
      label: "OpenCode",
  ...
  "mcp": {
    ...
    },
    {
      ...
      copyText: `code --add-mcp '{"name":"redrob","type":"http","url":"${MCP_SERVER_URL}"}'`,
    },
    {
      id: "any-client",
      label: "Any client",
  ...
  return (
    <div id="connect-mcp-install" className="not-prose my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-950">
      ...
      <div className="overflow-x-auto border-b border-gray-100 px-4 pt-4 dark:border-white/10" role="tablist" aria-label="Redrob Work MCP client install options">
      ...
      <div id={`connect-mcp-panel-${activeInstall.id}`} role="tabpanel" aria-labelledby={`connect-mcp-install-${activeInstall.id}`} className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="m-0 text-xs text-gray-500">Works with your Redrob Work account — <a href="https://app.redrob.io?mode=sign-up" className="font-medium underline">create one free</a>.</p>
          <button type="button" aria-label="Copy the Redrob Work MCP install command" onClick={() => copy(activeInstall.id, activeInstall.copyText)} className="shrink-0 rounded-lg bg-[#011627] px-4 py-2 text-xs font-medium text-white">
      ...
      <div className="flex flex-col gap-2 border-t border-gray-100 px-5 py-4 text-xs sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button type="button" aria-label="Copy the Redrob Work MCP server URL" onClick={() => copy("server-url", MCP_SERVER_URL)} className="shrink-0 font-medium underline">
