export const RedrobWorkDownloads = () => {
  const STABLE_RELEASES_URL = "https://api.github.com/repos/redrob-labs/redrob-work/releases?per_page=30";
  const ALPHA_RELEASE_URL = "https://api.github.com/repos/redrob-labs/redrob-work/releases/tags/alpha-macos-latest";
    ...
    return [
      {
        label: "macOS",
        downloads: [
          { label: "Mac (Apple Silicon)", asset: findAsset(`redrob-mac-arm64-${version}.dmg`) },
          { label: "Mac (Intel)", asset: findAsset(`redrob-mac-x64-${version}.dmg`) },
        ].filter((download) => download.asset),
      },
      {
        label: "Windows",
        downloads: [
          { label: "Windows (x64)", asset: findAsset(`redrob-win-x64-${version}.exe`) },
          { label: "Windows (ARM64)", asset: findAsset(`redrob-win-arm64-${version}.exe`) },
        ].filter((download) => download.asset),
      },
      {
        label: "Linux",
        downloads: [
          { label: "AppImage (x64)", asset: findAsset(`redrob-linux-x86_64-${version}.AppImage`) },
          { label: "AppImage (ARM64)", asset: findAsset(`redrob-linux-arm64-${version}.AppImage`) },
          { label: "tar.gz (x64)", asset: findAsset(`redrob-linux-x64-${version}.tar.gz`) },
          { label: "tar.gz (ARM64)", asset: findAsset(`redrob-linux-arm64-${version}.tar.gz`) },
        ].filter((download) => download.asset),
      },
    ];
  };
  const detectPlatform = () => {
    if (typeof navigator === "undefined") {
      return { label: "macOS", assetName: (version) => `redrob-mac-arm64-${version}.dmg` };
    }
    ...
    if (platform.includes("win")) {
      return { label: "Windows", assetName: (version) => `redrob-win-x64-${version}.exe` };
    }
    if (platform.includes("linux")) {
      return { label: "Linux", assetName: (version) => `redrob-linux-x86_64-${version}.AppImage` };
    }
    return { label: "macOS", assetName: (version) => `redrob-mac-arm64-${version}.dmg` };
  };
  ...
  const loadAlpha = async () => {
    ...
    try {
      ...
      release.assets.forEach((asset) => {
        if (!asset.name.startsWith("redrob-mac-arm64-") || (!asset.name.endsWith(".dmg") && !asset.name.endsWith(".zip"))) return;
      ...
      setAlphaBuilds(
        Object.values(buildsByRun)
          ...
          .map((build) => ({
            ...build,
            version: build.dmg.name.replace("redrob-mac-arm64-", "").replace(/\.dmg$/, ""),
          }))
  ...
  return (
    <div id="redrob-downloads" className="not-prose my-8">
      {latestStable ? (
              ...
              return (
                <div key={release.id} className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-gray-950">
                  ...
                  <div className="mt-5 border-t border-gray-100 pt-4 dark:border-white/10">
                    <a href={`https://github.com/redrob-labs/redrob-work/releases/tag/${release.tag_name}`} className="text-sm font-medium text-gray-600 hover:text-gray-950 dark:text-gray-400 dark:hover:text-white">Release notes →</a>
                  </div>
                </div>
              );
            })}
