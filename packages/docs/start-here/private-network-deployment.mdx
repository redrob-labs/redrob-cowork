...
description: "Run Redrob Work when laptops have internet and VPN access while Den stays inside a private network."
---

Most enterprise Redrob Work installs are not fully air-gapped. The common shape is:
...
- The Redrob Work chart and images at deploy or image-pull time, unless you mirror the Helm chart and enabled images into an internal registry.
...
**Why:** `DEN_DIAGNOSTICS_ORIGIN` defaults to the public Redrob Work diagnostic service.
...
| Cloud MCP works in a real chat | In a chat, the agent can search available Redrob Work capabilities and execute one that the user is allowed to run. | `Settings → Connect` for provider sign-in or admin setup. Use `Settings → Debug` to interpret diagnostics, especially a skipped catalog probe. |
