...
description: "Run Redrob Cowork when laptops have internet and VPN access while Den stays inside a private network."
---

Most enterprise Redrob Cowork installs are not fully air-gapped. The common shape is:
...
Set `DEN_BASE_URL` to the Den web origin. Den derives Better Auth, CORS/trusted origins, the web-app host allowlist, API defaults, and MCP resource defaults from that single value. If you need a separate externally reachable API origin or a proxied API path for external MCP clients, set `DEN_API_PUBLIC_URL` as a compatibility override, for example `https://redrob.example.internal/api/den`.
...
External MCP clients such as Cursor, Claude Code, VS Code, and OpenCode are separate from the desktop. If your deployment publishes an MCP endpoint for them, they must be able to reach it, usually `<DEN_API_PUBLIC_URL>/mcp/agent`; with the single-origin proxy example above, that is `https://redrob.example.internal/api/den/mcp/agent`. To drive a user's own desktop app instead, no network access is needed at all: see [Control Redrob Cowork from any MCP client](/model-context-protocol/overview), which runs over a local stdio process.
...
- The Redrob Cowork chart and images at deploy or image-pull time, unless you mirror the Helm chart and enabled images into an internal registry.
...
**Why:** `DEN_DIAGNOSTICS_ORIGIN` defaults to the public Redrob Cowork diagnostic service.
...
| Cloud MCP works in a real chat | In a chat, the agent can search available Redrob Cowork capabilities and execute one that the user is allowed to run. | `Settings → Connect` for provider sign-in or admin setup. Use `Settings → Debug` to interpret diagnostics, especially a skipped catalog probe. |
