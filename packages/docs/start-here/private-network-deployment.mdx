...
External MCP clients such as Cursor, Claude Code, VS Code, ChatGPT, and OpenCode are separate from the desktop. They must reach the MCP endpoint you configure for them, usually `<DEN_API_PUBLIC_URL>/mcp/agent`; with the single-origin proxy example above, that is `https://redrob.example.internal/api/den/mcp/agent`. See [Redrob Work Connect MCP](/cloud/run-in-the-cloud/cloud-mcp) for the external-client flow.
