...
description: "Connect a custom or local MCP server that is not available through Redrob Work Connect"
---

<Note>
**Local workspace setup.** Custom MCP servers are supported without an Redrob Work Cloud account, so this path stays free and local to the current workspace. For a connection managed and assigned by your organization, use [Redrob Work Cloud Connectors](/cloud/share-with-your-team/shared-mcp-connections).
</Note>

Use this guide only when you need a custom or local MCP server that is not available through Redrob Work Connect.
...
Connecting a custom app with Redrob Work takes four clicks.
