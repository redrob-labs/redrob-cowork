...
sign in to Redrob Cowork and start with
[Connect your services](/start-here/connect-your-stack/connect-services).

Slack's official MCP server does not support automatic OAuth client registration. To connect it as a custom local MCP in Redrob Cowork, ask a Slack admin to create or approve a Slack app for MCP, then add the app's OAuth client credentials in Redrob Cowork.
...
In Slack, an admin should create or approve an app for Redrob Cowork's MCP access.
...
4. Add the `User Token Scopes` for the Slack MCP tools your team wants Redrob Cowork to use.
...
In Redrob Cowork, open your workspace and go to `Settings` > `Library`, expand `Advanced settings`, and click `Add workspace MCP`.

<Frame>
  ![Redrob Cowork Library page listing skills, apps, and connections](/images/library-slack-connection.png)
</Frame>
...
Redrob Cowork writes the MCP entry with the OAuth client credentials, similar to this:
...
If you entered scopes, Redrob Cowork also saves them as `oauth.scope`. This is the scope list Slack sees during OAuth consent.

## Sign in to Slack

After the MCP app is added, reload the engine if Redrob Cowork asks you to. Then start Slack MCP sign-in from the Slack app card or the MCP auth prompt.

Slack opens in your browser and shows the scopes you configured. Approve the Slack app for the workspace you want Redrob Cowork to use, then return to Redrob Cowork when authorization finishes.
...
If Slack says a scope is invalid or unavailable, remove that scope from both the Slack app and Redrob Cowork's `OAuth scopes` field, then retry. Slack workspace policies and plan settings can limit which scopes an app may request.

If your Slack workspace blocks app installation, ask a Slack admin to approve the app first. Redrob Cowork cannot bypass Slack workspace approval policies.
