...
description: "Sign in to Redrob Work and connect the accounts your agent can use"
---

<Info>
**Redrob Work Cloud organization setup.** This path requires an Redrob Work Cloud sign-in and an organization that has made services available to you. To stay local, keep using Redrob Work for free and [add a custom MCP server](/start-here/connect-your-stack/add-an-mcp-server) to your workspace. Admins can manage team connections with [Connectors](/cloud/share-with-your-team/shared-mcp-connections).
</Info>

Use `Settings` > `Redrob Work Connect` for services your organization makes available,
...
Connect requires signing in to an Redrob Work account and joining an organization
that has Connect enabled. This is separate from signing in to an LLM provider
such as ChatGPT.

1. Open the Redrob Work desktop app.
2. Click `Sign in`, or open `Settings` > `Redrob Work Connect` and choose `Sign in`.
3. Complete sign-in in your browser and return to Redrob Work.
4. Choose the Redrob Work organization you want to use if prompted.
...
1. Open `Settings` > `Redrob Work Connect`.
2. Find the service under `Needs your sign-in`.
3. Click `Connect` and approve access on the provider's sign-in page.
4. Return to Redrob Work. The service moves to `Ready to use` automatically.

Some connections are managed by your organization. If a service is not
listed, ask an organization admin to publish it from the Redrob Work Cloud
...
send you back to `Settings` > `Redrob Work Connect` with the action to take instead of
...
- A **connector** is an MCP service your organization adds on its own, without
  ...
  Redrob Work Cloud `Connectors` page and grant member or team access.
...
- The **MCP Gateway** is the hosted endpoint that lets external MCP clients
  use capabilities from an Redrob Work organization. See
  [Redrob Work MCP Gateway](/cloud/run-in-the-cloud/cloud-mcp).
