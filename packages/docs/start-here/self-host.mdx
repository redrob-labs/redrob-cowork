---
title: "Self-host"
description: "Run Redrob Cowork on your own server or infrastructure."
---

Most users just use the [desktop app](https://redrob.io/download). Self-hosting is for teams that want Redrob Cowork to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

There is no hosted Redrob Cowork to fall back on. The desktop app is the product, and this page is for teams who want the server side on their own infrastructure. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5).

## What you deploy

Redrob Cowork is split into normal services:

- **Redrob Cowork app**: the desktop or web client people use.
...
- **Inference service**: optional Redrob Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want your own accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
The chart pulls the matching GHCR images for `redrob-den-api`, `redrob-den-web`, and optional `redrob-inference` unless you mirror them into an internal registry.
...
For containerized services, start from the [Docker packaging](https://github.com/redrob-labs/redrob-work/tree/dev/packaging/docker) in the repo.
...
- Hosted Redrob Cowork uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/redrob_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. Redrob Cowork uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
Private deployments should keep public signup disabled and use the one-time [first-administrator setup flow](/self-host/deploy-to-your-cloud/first-administrator). Configuring an owner or platform-admin email does not create an account or password, and Redrob Cowork does not ship a default administrator password.
...
If you do not deploy Den, worker access is token-based with `REDROB_TOKEN` and `REDROB_HOST_TOKEN`.

## Minimal working stack

### A self-hosted server and the desktop app

Cloud-style self-hosting is composed of three main pieces:
...
Redrob Cowork does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Then don't. The [desktop app](https://redrob.io/download) needs no server: it runs a
local `redrob-server` and the Redrob Code engine on each machine, and people connect a
model with their own Redrob Key. Skip this whole page unless you specifically need a
shared remote runtime.

## Looking for support for a larger rollout?

If you're rolling Redrob Cowork out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
...
[Book a 30-minute call](https://calendar.app.google/86QpCENvhfEzDFLu5) to talk it through.
