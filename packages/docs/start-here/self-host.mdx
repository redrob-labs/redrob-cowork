---
title: "Self-host"
description: "Run Redrob Work on your own server or infrastructure."
---

Most users just use the [desktop app](https://redrob.io/download). Self-hosting is for teams that want Redrob Work to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

There is no hosted Redrob Work to fall back on. The desktop app is the product, and this page is for teams who want the server side on their own infrastructure. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5).

## What you deploy

Redrob Work is split into normal services:

- **Redrob Work app**: the desktop or web client people use.
...
- Hosted Redrob Work uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. Redrob Work uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
Private deployments should keep public signup disabled and use the one-time [first-administrator setup flow](/self-host/deploy-to-your-cloud/first-administrator). Configuring an owner or platform-admin email does not create an account or password, and Redrob Work does not ship a default administrator password.
...
Redrob Work does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
If you're rolling Redrob Work out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
