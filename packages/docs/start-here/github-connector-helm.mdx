...
description: "Create a GitHub App and configure the Redrob Cowork EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted Redrob Cowork deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
config:
  public:
    ...
    apiOrigin: "https://api.redrob.example.com"
```

The GitHub App needs these Redrob Cowork URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://redrob.example.com` |
...
## What Redrob Cowork does with it

When an admin clicks **Connect GitHub** in Redrob Cowork:
...
```text
https://redrob.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to Redrob Cowork, show repositories visible to the installation, and allow discovery on a selected repository.
