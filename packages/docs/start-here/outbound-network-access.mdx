...
For a practical Redrob Work Cloud desktop install, allow TCP 443 to these hosts:
...
| --- | --- | --- | --- |
| `app.redrob.io` | Required for Redrob Work Cloud | Hosted Cloud web origin. The desktop uses one base URL and derives Den API/MCP calls through `/api/den/...`. | Cloud sign-in, org settings, Collections, and remote workspace flows cannot load. |
| `api.redrob.io` | Required for hosted install and Redrob Work Connect | Hosted public API and Redrob Work Connect MCP endpoint used by install and external-client flows. | Hosted install checks and public Redrob Work Connect fail. |
| `app.redrob.software`, `api.redrob.software` | Required only for legacy hosted configurations | Older desktop configurations may still derive Cloud and Redrob Work Connect traffic from these legacy hosted origins. | Legacy configurations cannot reconcile Redrob Work Connect or native MCP Apps until moved to the current hosted origins. |
| Your self-hosted Den web origin | Required for self-hosted desktop | Replaces `app.redrob.io` for signed-in desktop traffic. In the single-origin shape, API and MCP use `/api/den/...` on the same origin. | The desktop cannot sign in or reach Den. |
...
| `redrob.io`, `redrob.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |
