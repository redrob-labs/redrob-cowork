...
- `console.redrob.ai` is the Redrob model catalog and inference endpoint. The Redrob Code engine fetches `/api/backend/v1/models` with your `REDROB_API_KEY`; there is no mirror override. The same host also serves the device authorization endpoints behind **Redrob로 연결**, which is how Redrob Cowork is given a workspace key without anyone copying one by hand.
...
| `registry.npmjs.org` | Required in practice unless npm is mirrored | `npx` package resolution for `redrob-ui-mcp` and development-only helper commands. | Redrob Cowork UI-control MCP is unavailable. |
| `console.redrob.ai` | Required | Redrob inference endpoint and model catalog, plus the device authorization endpoints used to connect a workspace. The Redrob Code engine fetches the catalog itself with `REDROB_API_KEY`. | Inference, model selection, and connecting through the browser are unavailable. Pasting a key still works. |
...
`code.claude.com` and `schemas.agentskills.io` appear in source or schema metadata. They are findable in the manifest, but Redrob Cowork does not fetch them for normal runtime behavior.
...
| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `ghcr.io` | Required unless mirrored | Redrob Cowork Helm chart and container images. | Kubernetes cannot pull the chart or `redrob-den-api`, `redrob-den-web`, and optional `redrob-inference` images. |
