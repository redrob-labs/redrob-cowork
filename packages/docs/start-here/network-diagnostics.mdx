...
description: "Diagnose DNS, TLS, proxy, catalog, and Den outbound issues in locked-down Redrob Work deployments."
---

Use this page when Redrob Work reaches the wrong server, cannot complete TLS, is blocked by a proxy, or needs proof that Den can reach an approved diagnostic origin. For non-network runtime, MCP, and stale-tool problems, use [Diagnostic prompts](/start-here/troubleshooting/diagnostic-prompts).
...
| --- | --- | --- |
| `LIKELY TLS INTERCEPTION` | The leaf issuer is an internal or proxy CA instead of the expected public issuer. | Allowlist the Redrob Work hosts through the proxy, or install the enterprise root in every Redrob Work trust surface that needs it. |
...
For self-hosted Den origins, set `REDROB_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS` before Redrob Work launches:
