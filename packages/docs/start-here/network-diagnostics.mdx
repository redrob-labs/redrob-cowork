...
description: "Diagnose DNS, TLS, proxy, catalog, and Den outbound issues in locked-down Redrob Cowork deployments."
---

Use this page when Redrob Cowork reaches the wrong server, cannot complete TLS, is blocked by a proxy, or needs proof that Den can reach an approved diagnostic origin. For non-network runtime, MCP, and stale-tool problems, use [Diagnostic prompts](/start-here/troubleshooting/diagnostic-prompts).
...
If raw download is blocked, copy `scripts/support/redrob-doctor.ps1` to the machine through your approved offline channel and run:
...
| --- | --- | --- |
| `LIKELY TLS INTERCEPTION` | The leaf issuer is an internal or proxy CA instead of the expected public issuer. | Allowlist the Redrob Cowork hosts through the proxy, or install the enterprise root in every Redrob Cowork trust surface that needs it. |
...
Settings → Debug includes a cloud catalog diagnostic that probes the managed `redrob-cloud` MCP entry and verifies that Cloud exposes exactly `search_capabilities` and `execute_capability`.

For self-hosted Den origins, set `REDROB_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS` before Redrob Cowork launches:

```bash
REDROB_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS=https://redrob.example.com
```
...
- The default diagnostics origin is `https://diagnostic.redrob.io`.
