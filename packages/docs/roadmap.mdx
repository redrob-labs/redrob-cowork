---
title: "Roadmap"
description: "The roadmap for the Redrob Work desktop app, portable capabilities, hosted workspaces, and every surface where work happens."
---
...
Redrob Work implements the [MCP authorization specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization) on both sides of the wire: as an MCP client (the desktop app and Redrob Work Cloud connections) and as an MCP server with its own authorization server (Redrob Work Cloud). Items marked Building or Next track the 2026-07-28 MCP specification release.
...
| Client ID Metadata Documents (SEP-991) | A single, stable HTTPS client identity for Redrob Work that enterprises allowlist once, replacing per-connection registrations and wildcard redirect allowlists. | Building |
...
| URL-based client IDs in Redrob Work Cloud | Accept Client ID Metadata Document identities in the Redrob Work Cloud authorization server. | Next |

> Following the spec is what makes Redrob Work drop-in compatible with enterprise MCP gateways: paste the gateway URL, sign in with your identity provider, and every downstream system it fronts becomes available.
...
| Capability | What it does | Status |
| --- | --- | --- |
| Desktop policies | Control custom providers, OpenCode models, workspaces, settings, extensions, built-in tools, and onboarding by organization, team, or member. | Live |
...
| Anthropic-compatible plugins | Import Claude-compatible plugin and marketplace manifests and normalize their skills, MCPs, commands, and tools into Redrob Work extensions. | Live |
...
| OpenTelemetry coverage | Extend OTLP traces, metrics, and logs across Redrob Work services and deployment paths. | Building |
...
| Surface | Status |
| --- | --- |
| Redrob Work desktop | Live |
| Existing AI agents through MCP | Live |
| Redrob Work Web — the full workspace in your browser, nothing to install | Preview |
| Slack | Next |
| Mobile | Next |
