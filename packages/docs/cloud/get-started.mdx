...
OpenWork Cloud is available at [app.redrob.io](https://app.redrob.io/). It is for teams that want shared identity, Collections, managed connections and models, desktop policies, or hosted workspaces.
...
4. If the browser does not return to OpenWork automatically, click `Paste sign-in code`, paste the `redrob://den-auth?...` link or one-time code from OpenWork Cloud, then click `Finish sign-in`.
