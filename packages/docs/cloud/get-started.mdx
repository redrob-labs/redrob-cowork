---
title: "Get started with Redrob Work Cloud"
...
<Note>
Redrob Work Cloud is optional. The local desktop app stays free, requires no Cloud sign-in, and supports your own providers, skills, plugins, and custom MCP servers. Use Cloud when an organization needs to assign and manage those resources for a team.
</Note>

Redrob Work Cloud is available at [app.redrob.io](https://app.redrob.io/). It is for teams that want shared identity, Collections, managed connections and models, desktop policies, or hosted workspaces.

First, create or join an organization and connect the desktop app to it.

<Frame>
  ![Redrob Work Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.redrob.io](https://app.redrob.io/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if Redrob Work Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.redrob.io/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to Redrob Work automatically, click `Paste sign-in code`, paste the `redrob://den-auth?...` link or one-time code from Redrob Work Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. The hosted
default is `https://app.redrob.io`.
...
- Redrob Work Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
back to `https://app.redrob.io`. When you change the Cloud URL in
`Settings > Account`, Redrob Work writes the new `baseUrl` back to the bootstrap
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted Redrob Work runtimes that teammates can open from the desktop app.
