...
description: "Provision Redrob Work members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate Redrob Work
members automatically. Entra security groups can also become Redrob Work teams
...
- An Redrob Work organization with Enterprise SSO and SCIM.
- An Redrob Work owner, or a member who can manage security configuration.
- A working Redrob Work SAML connection. Complete and test
...
sure at least one Redrob Work owner can sign in through SAML before relying on
Entra as the identity source.

## 1. Get the Redrob Work connection values

In Redrob Work, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. Redrob Work shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as Redrob Work teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | Redrob Work **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for Redrob Work SAML SSO.
3. Open **Provisioning**.
...
6. Paste the Redrob Work **SCIM base URL** into **Tenant URL**.
7. Paste the Redrob Work **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to Redrob Work
...
create a second Redrob Work member for the same person.
...
not enough; assign it to the Redrob Work enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In Redrob Work, open **Members** and confirm that the member appears with the
   expected email.
...
6. In Redrob Work, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to Redrob Work. Add members and select them explicitly
...
5. Refresh Redrob Work **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that Redrob Work removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected Redrob Work
   deprovisioning behavior.
3. Add another assigned group and confirm that Redrob Work creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in Redrob Work. Redrob Work keeps manually
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from Redrob Work, then retest |
...
| On-demand group provisioning reports an internal server error without an Redrob Work request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no Redrob Work team appears | Redrob Work team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate Redrob Work member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but Redrob Work still shows the old state | The UI or incremental cycle has not refreshed | Refresh Redrob Work, check Entra **Provisioning logs**, and review the SCIM health status in Redrob Work |
