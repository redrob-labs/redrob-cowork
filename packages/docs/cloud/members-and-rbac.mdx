...
Redrob Work Cloud separates org membership from team assignment. Teams are used for Collection and provider access; roles control what a person can manage across the org.

## Default roles

Redrob Work Cloud comes with three default roles:
...
<Frame>
  ![Members and RBAC dashboard in Redrob Work Cloud](/images/cloud-members-and-rbac-dashboard.png)
</Frame>
...
When SSO just-in-time provisioning adds someone to an organization on first sign-in, Redrob Work assigns the baseline `Member` role. IdP attributes such as `role`, `groups`, or `admin` are not used to grant Redrob Work organization roles.

Treat `Admin`, `Owner`, and custom-role elevation as explicit access changes made inside Redrob Work. Review those changes in the member list and audit trail instead of relying on unvalidated IdP attributes for authorization.
...
When this is enabled, Redrob Work Cloud only lets people join with email addresses from the approved domains. Existing invites still target a specific email address, so both checks apply: the invitee must use the invited email address and that address must match the allowlist.
...
When SCIM deprovisions a user, Redrob Work retains a disconnected member record for organization history. The global user is deleted only when they have no other active organization memberships. Reactivate the user through SCIM before they can regain SAML access.
