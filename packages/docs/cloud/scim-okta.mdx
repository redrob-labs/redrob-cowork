...
description: "Provision Redrob Work members and teams from Okta"
---

Use Okta Lifecycle Management to create, update, deactivate, and reactivate
Redrob Work members. Okta groups can also become SCIM-managed Redrob Work teams.
...
Keep Redrob Work SSO enforcement off until both SAML and a narrowly scoped SCIM
test pass. Make sure at least one Redrob Work owner can sign in through Okta.

## 1. Create the Redrob Work SCIM connector

In Redrob Work, select the organization, then open **Settings → SCIM**. Confirm
the page reports that SAML/SSO is active, then:

1. Select **Create connector**.
2. Copy the **SCIM base URL**.
3. Copy the bearer token immediately. Redrob Work displays the full token only
...
<Note>
Use the base URL Redrob Work displays. In a split-host Helm deployment it may use
...
1. Open **Applications → Applications → Redrob Work**.
...
| Okta field | Value |
| --- | --- |
| SCIM connector base URL | Redrob Work **SCIM base URL** |
...
| Authorization | Redrob Work SCIM bearer token |
...
## 3. Configure provisioning to Redrob Work
...
does not create an Redrob Work password credential.
...
can create a duplicate Redrob Work member instead of linking SSO and SCIM.
...
5. In Redrob Work, open **Members** and verify the same member appears without a
...
1. Changing the profile name in Okta updates the Redrob Work member.
...
3. The user cannot regain access through password sign-up while SCIM marks the
   identity inactive.
4. Reactivating and reassigning the user restores the same Redrob Work identity.
5. An unrelated Redrob Work member remains unchanged throughout the test.

Redrob Work retains a disconnected member record for organization history. The
global user is deleted only after their final active organization membership
is removed.

## 6. Push Okta groups as Redrob Work teams

After user provisioning works:

1. In Redrob Work **Settings → SCIM**, enable
...
5. In Redrob Work, open **Members → Teams** and verify a team labeled
   **Managed by SCIM** appears with the expected members.
6. Add and remove one member in Okta and verify the Redrob Work team follows.

Manage pushed groups in Okta, not in Redrob Work. Manually created Redrob Work teams
remain independent.
...
3. Review Redrob Work's SCIM health and unresolved-failure count.
...
Use **Settings → SCIM → Run reconciliation** to check Redrob Work's local
SCIM-managed identities for inconsistent organization membership or missing
provider-account state. Redrob Work records unresolved drift for retry or manual
review.
...
Assignments page and System Log for outbound provisioning. When Redrob Work has no
...
| --- | --- | --- |
| **Create connector** is disabled | Redrob Work does not see an enabled SSO provider | Complete and test Okta SAML first; if SAML returns `invalid_saml_configuration`, upgrade and save the connection again |
| **Test API Credentials** fails | Incorrect base URL, stale token, whitespace, or a rewritten hostname | Copy both values again from Redrob Work and use the advertised base URL unchanged |
| Okta reports `409 User already exists` | The user existed before SCIM, or more than one create job ran for the same assignment | Inspect the System Log first. If a successful create immediately precedes the conflict and the exact member exists in Redrob Work, stop triggering **Provision User**. Use a never-before-provisioned user for the clean lifecycle test |
...
| **Run reconciliation** does not provision an assigned user | Reconciliation only checks Redrob Work's local SCIM identity state | Trigger and inspect outbound provisioning in Okta instead |
| Deactivation does not remove access | **Deactivate Users** is disabled or Okta has not sent the event | Enable it and inspect the Okta System Log and Redrob Work SCIM health |
| A pushed group does not create a team | Redrob Work team synchronization is off | Enable **Create teams from SCIM groups**, then retry the Okta group push |
