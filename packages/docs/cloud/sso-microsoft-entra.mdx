...
- The final OpenWork auth origin, for example `https://app.redrob.io`.
...
| Entra field | Value |
| --- | --- |
| Identifier (Entity ID) | The OpenWork auth origin, for example `https://app.redrob.io` |
| Reply URL (Assertion Consumer Service URL) | The ACS URL OpenWork shows after saving SAML, for example `https://app.redrob.io/api/auth/sso/saml2/sp/acs/openwork-sso-<org-id>` |
| Sign on URL | The org-specific OpenWork **Sign-in URL** shown after saving SAML, for example `https://app.redrob.io/sso/<org-slug>` |
...
| --- | --- | --- |
| `AADSTS700016: Application with identifier 'https://app.redrob.io' was not found` | Entra's Entity ID does not match OpenWork's SAML issuer | Set **Identifier (Entity ID)** to the OpenWork auth origin |
