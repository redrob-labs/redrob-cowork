...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in Redrob Work Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in Redrob Work Cloud

1. Open `Connectors` in the Redrob Work Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in Redrob Work Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted Redrob Work Cloud instance, the dashboard is
`https://app.redrob.io` and its public Den API origin is
`https://api.redrob.io`. Register this exact redirect URL with the OAuth
provider:

```text
https://api.redrob.io/v1/mcp-connections/oauth/callback
```

For a self-hosted instance whose public Den API is
`https://api.redrob.example.com`, register:

```text
https://api.redrob.example.com/v1/mcp-connections/oauth/callback
```
...
3. In Redrob Work Cloud, add the Slack connection and paste the app's client ID
   and client secret.
...
Granted members find the connection in `Settings` > `Redrob Work Connect`, grouped by what
they need to do:
...
1. Sign in to Redrob Work, then open `Settings` > `Redrob Work Connect`.
...
4. The browser shows `Connected` — return to Redrob Work.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in Redrob Work Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through Redrob Work Connect,
...
action and direct you to `Settings` > `Redrob Work Connect`.
...
services managed through Redrob Work Cloud.
