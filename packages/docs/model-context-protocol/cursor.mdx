---
title: "Redrob Work MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for Redrob Work Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The Redrob Work MCP server ([Redrob Work Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the Redrob Work capabilities your
...
Linear, Slack, Stripe, Exa, and Context7, shared skills, and Redrob Work Cloud
...
Add Redrob Work as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to Redrob Work and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. Redrob Work accepts exactly this
...
Other private-use callback schemes remain rejected: Redrob Work otherwise accepts
...
Redrob Work, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which Redrob Work
accepts.

## Switch organizations

Remove the stored MCP auth token for `redrob`, then connect again and choose
...
steps match Redrob Work Connect's OAuth profile: HTTPS or HTTP loopback callbacks
