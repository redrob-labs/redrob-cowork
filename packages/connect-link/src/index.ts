...
} from "@redrob/types/connect-link"
...
export function buildConnectDeepLink(token: string, scheme = "redrob"): string {
  return `${scheme}://${CONNECT_LINK_ROUTE}?token=${encodeURIComponent(token)}`
}

export function buildConnectExchangeDeepLink(code: string, apiBaseUrl: string, scheme = "redrob"): string {
