...
Drive a native Redrob Cowork thread from code, without rendering the app.

A "thread" here is an ordinary Redrob Cowork session. Same workspace, same managed
...
surfaces the Redrob Cowork server already serves.

## Why

Anything that drives Redrob Cowork without a UI — a load check, an automation
...
`thread.id` is the native session id. Opening Redrob Cowork on the same workspace
...
| `getThreadSnapshot` | Status, messages, and todos as Redrob Cowork stores them. |
...
```bash
pnpm --filter redrob-server test src/headless-threads.e2e.test.ts
```
