...
Drive a native Redrob Work thread from code, without rendering the app.

A "thread" here is an ordinary Redrob Work session. Same workspace, same managed
...
surfaces the Redrob Work server already serves.

## Why

Anything that drives Redrob Work without a UI — a load check, an automation
...
`thread.id` is the native session id. Opening Redrob Work on the same workspace
...
| `getThreadSnapshot` | Status, messages, and todos as Redrob Work stores them. |
