  ...
  if (request.method === "GET" && url.pathname === profile.oauth.authorizationPath) {
    ...
    if (faultApplies(context, "dcr-required")) {
      ...
      sendOAuthError(
        ...
        "The MCP connection failed before Redrob Work could complete the protocol lifecycle. Dynamic client registration is required before this gateway accepts OAuth authorization.",
      )
      return true
    }
    if (
