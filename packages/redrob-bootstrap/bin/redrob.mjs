...
function printHelp() {
  console.log([
    ...
    "  redrob-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  REDROB_OWNER_PASSWORD=<password> redrob-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  redrob-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  redrob-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
...
function defaultDeviceKeyPath() {
  return process.env.REDROB_DEVICE_KEY_PATH || join(configHomeDir(), "redrob", "bootstrap-device-key.json")
}
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "redrob-bootstrap-skill"
}
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by redrob bootstrap.\nredrobBootstrapTrigger: bootstrap.verify\nredrobBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm Redrob Work cloud onboarding can create and trigger a deterministic skill.`
}
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "redrobBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "redrobBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "redrob-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First Redrob Work Skill")
  ...
  jsonOut({
    ok: true,
    message: "Redrob Work cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  ...
  jsonOut({
    ok: true,
    message: "Redrob Work workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "redrob-bootstrap cloud claim-link --role ${link.role}" to view`
