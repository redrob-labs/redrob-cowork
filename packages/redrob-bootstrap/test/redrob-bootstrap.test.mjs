...
try {
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First Redrob Work Skill", output: "REDROB_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  await withStubDenApi(
    ...
    async (baseUrl) => {
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ brandAppName: "Redrob Work Demo A" })}\n`,
        "utf8",
      )
      ...
      assert.equal(writtenExplicit.brandAppName, "Redrob Work Demo A", "prepared desktop setup must preserve its profile identity")
    },
  )
