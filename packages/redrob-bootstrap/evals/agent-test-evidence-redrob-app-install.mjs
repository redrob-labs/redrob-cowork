...
if (process.platform !== "darwin") {
  throw new Error("redrob install app DMG test evidence requires macOS")
}
...
`)
  ...
  prove("The bootstrap CLI can be installed from a script", {
    ...
    assert: "exit 0 and an redrob executable exists",
  ...
  const doctor = run(installedRedrob, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "redrob doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with redrobApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "redrobApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
