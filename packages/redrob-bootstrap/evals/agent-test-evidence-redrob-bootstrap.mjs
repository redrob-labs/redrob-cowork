// Live E2E test evidence for the script-installable Redrob Work bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... pnpm exec node evals/agent-test-evidence-redrob-bootstrap.mjs
...
const cli = join(packageRoot, "bin", "redrob.mjs")
const temp = mkdtempSync(join(tmpdir(), "redrob-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installedRedrob = join(binDir, process.platform === "win32" ? "redrob-bootstrap.cmd" : "redrob-bootstrap")
const outDir = join(repoRoot, "evals", "results", "redrob-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the redrob CLI", {
    action: "node bin/redrob.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and installed executable path returned",
    evidence: { status: install.status, body: install.json },
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedRedrob)

  const doctor = run(installedRedrob, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "redrob doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedRedrob, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
  ...
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "redrob cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
...
<title>Redrob Work Bootstrap CLI — live E2E test evidence</title>
...
<h1>Redrob Work Bootstrap CLI — live E2E test evidence</h1>
