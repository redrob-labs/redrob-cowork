// Linux E2E test evidence for Redrob Work bootstrap without shipping to production.
//
// This runs inside a real Linux container and proves:
// - the bootstrap CLI installs on Linux,
// - `redrob install app` installs a Linux tar.gz app artifact from a manifest,
// - `redrob doctor --app` verifies the installed app,
// - `redrob cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/redrob-bootstrap:
...
const outDir = join(repoRoot, "evals", "results", "redrob-linux-container")
...
const artifactPath = join(fixtureDir, "redrob-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/REDROB_OWNER_PASSWORD=[^\s]+/g, "REDROB_OWNER_PASSWORD=[redacted]")
}
...
try {
  const appBinary = join(artifactRoot, "redrob")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho Redrob Work Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "redrob"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/redrob-linux.tar.gz", sha256: digest, appName: "redrob" },
        arm64: { type: "tar.gz", url: "file:///fixture/redrob-linux.tar.gz", sha256: digest, appName: "redrob" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local redrob tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/redrob.mjs install --install-dir /tmp/redrob/install --bin-dir /tmp/redrob/bin --json >/tmp/install.json",
    "/tmp/redrob/bin/redrob-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/redrob/apps --json >/tmp/app-install.json",
    "/tmp/redrob/bin/redrob-bootstrap doctor --install-dir /tmp/redrob/install --bin-dir /tmp/redrob/bin --app --app-dir /tmp/redrob/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run redrob install, redrob install app, and redrob doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/redrob.mjs install --install-dir /tmp/redrob/install --bin-dir /tmp/redrob/bin --json >/tmp/install.json",
    `/tmp/redrob/bin/redrob-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/redrob/desktop-bootstrap.json --skills-dir /tmp/redrob/skills --json >/tmp/onboard.json`,
    "/tmp/redrob/bin/redrob-bootstrap doctor --install-dir /tmp/redrob/install --bin-dir /tmp/redrob/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/redrob/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `REDROB_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run redrob cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "REDROB_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
<title>Redrob Work Linux Bootstrap — test evidence</title>
...
</head><body><main><h1>Redrob Work Linux Bootstrap — test evidence</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Screenshots: ${frames.length}</div>
