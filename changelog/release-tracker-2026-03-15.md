...
- Fixed the feedback flow so submitted messages are sent to the Redrob Work team inbox.
...
#### One-line summary
Moves CI and release workflows onto Blacksmith-backed runners, with no visible Redrob Work app, web, or server workflow change.
...
#### Release importance
Minor release: repairs a core live-session behavior without materially changing Redrob Work's overall workflow model.
...
#### Release importance
Minor release: improves session clarity and fixes a few focused interaction issues without changing the broader Redrob Work model.
...
#### Release importance
Minor release: meaningfully improves the hosted cloud flow and corrects a couple of visible web regressions without redefining Redrob Work's overall product shape.
...
#### Release importance
Minor release: delivers a collection of focused UX and reliability fixes across key web and session surfaces without changing the core Redrob Work workflow.
...
#### Main changes
Removes the extra intent step, drops the transient marketing-heavy auth shell, and adds a dedicated final connect screen so new Den users can launch a worker and open it in Redrob Work with fewer detours.

#### Lines of code changed since previous release
448 lines changed since `v0.11.160` (198 insertions, 250 deletions).

#### Release importance
Minor release: improves a focused hosted onboarding path without materially changing Redrob Work's broader product model.
...
#### Title
Docker dev prints LAN-ready Redrob Work URLs
...
#### Release importance
Minor release: improves local stack accessibility for testing and self-hosted development without changing the main Redrob Work product flow.
...
#### Major improvement details
- Improved Docker dev-stack defaults so Redrob Work is easier to access from other devices on local networks.
...
#### Major improvement details
- Added custom GitHub skill hub repository support so organizations can use their own hosted skill sources inside Redrob Work.
...
#### Release importance
Minor release: improves visibility, recovery, and localization across key flows without materially changing Redrob Work's core architecture.
...
#### One-line summary
Adds an in-app Redrob Work Cloud settings flow for sign-in, org selection, and worker opening, while smoothing desktop auth handoff and share reliability.

#### Main changes
- Added a Cloud tab in Settings for sign-in, org selection, worker lists, and opening ready Den workers into Redrob Work.
...
#### Release importance
Major release: introduces a substantial new Redrob Work Cloud workflow and expands how users authenticate and open cloud workers from the product.
...
#### Major improvement details
- Added Redrob Work Cloud authentication and worker-open controls directly in Settings.
...
#### Major bug fix details
- Restored shared bundle installs and repeat app opens in Redrob Work Share.
...
#### Main changes
Fixes the Settings tab list and default settings route so Cloud controls only appear in developer mode, matching the intended rollout of the new Redrob Work Cloud panel.
...
- Simplified Redrob Work Share OG previews and cleaned up Den landing CTAs.
...
#### Title
Redrob Work Cloud web flows and remote reconnects improve
...
#### Release importance
Major release: substantially changes the hosted Redrob Work Cloud experience and remote-connect workflow across web, desktop, and cloud surfaces.
...
#### Major improvement details
- Tailored the hosted web app UI and Den onboarding flow for Redrob Work Cloud deployments.
...
#### Release importance
Minor release: improves worker runtime observability and local spawn compatibility without materially changing how most users operate Redrob Work.
