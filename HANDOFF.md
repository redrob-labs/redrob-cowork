...
5. **`packages/ui/src/react/roadmap.tsx` still says Cloud is the control plane.**
   `packages/docs/roadmap.mdx` was rewritten; the in-app roadmap component was not, so the two
   now disagree.
