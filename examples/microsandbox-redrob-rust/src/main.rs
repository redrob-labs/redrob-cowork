...
async fn main() -> Result<()> {
    let image = env::var("REDROB_MICROSANDBOX_IMAGE")
        .unwrap_or_else(|_| "redrob-microsandbox:dev".to_string());
    let name = env::var("REDROB_MICROSANDBOX_NAME")
        .unwrap_or_else(|_| "redrob-microsandbox-rust".to_string());
...
mod tests {
    ...
    async fn rust_example_smoke_test_checks_health_and_session_endpoints() -> Result<()> {
        let image = env::var("REDROB_MICROSANDBOX_IMAGE")
            .unwrap_or_else(|_| "ttl.sh/redrob-microsandbox-11559:1d".to_string());
