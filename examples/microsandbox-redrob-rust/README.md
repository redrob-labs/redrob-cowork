...
```bash
cargo run --manifest-path examples/microsandbox-redrob-rust/Cargo.toml
```

Useful environment overrides:

- `REDROB_MICROSANDBOX_IMAGE` - OCI image reference to boot. Defaults to `redrob-microsandbox:dev`.
- `REDROB_MICROSANDBOX_NAME` - sandbox name. Defaults to `redrob-microsandbox-rust`.
- `REDROB_MICROSANDBOX_WORKSPACE_DIR` - host directory bind-mounted at `/workspace`. Defaults to `examples/microsandbox-redrob-rust/.state/<sandbox-name>/workspace`.
- `REDROB_MICROSANDBOX_DATA_DIR` - host directory bind-mounted at `/data`. Defaults to `examples/microsandbox-redrob-rust/.state/<sandbox-name>/data`.
...
```bash
REDROB_MICROSANDBOX_IMAGE=ghcr.io/example/redrob-microsandbox:dev \
REDROB_MICROSANDBOX_WORKSPACE_DIR="$PWD/examples/microsandbox-redrob-rust/.state/demo/workspace" \
REDROB_MICROSANDBOX_DATA_DIR="$PWD/examples/microsandbox-redrob-rust/.state/demo/data" \
...
```bash
REDROB_MICROSANDBOX_IMAGE=ttl.sh/redrob-microsandbox-11559:1d \
cargo test --manifest-path examples/microsandbox-redrob-rust/Cargo.toml -- --ignored --nocapture
```

## Persistence behavior

By default, the example creates and reuses two host directories under `examples/microsandbox-redrob-rust/.state/<sandbox-name>/`:
...
`microsandbox` expects an OCI image reference. If `redrob-microsandbox:dev` only exists in your local Docker daemon, the SDK may not be able to resolve it directly. In that case, push the image to a registry or otherwise make it available as a pullable OCI image reference first, then set `REDROB_MICROSANDBOX_IMAGE` to that ref.
