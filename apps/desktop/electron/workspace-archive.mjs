...
function defaultRedrobConfig(targetDir, preset = "starter") {
  return {
    version: 1,
    workspace: {
...
export async function importWorkspaceConfig({ archivePath, targetDir, name }) {
  ...
  const redrobPath = path.join(opencodeDir, "redrob.json");
  let preset = "starter";
  ...
  if (await pathExists(redrobPath)) {
    ...
    try {
      ...
      const config = defaultRedrobConfig(targetDir, preset);
      await writeFile(redrobPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
    }
  } else {
    const config = defaultRedrobConfig(targetDir, preset);
