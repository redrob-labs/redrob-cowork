...
describe("targetedStableUpdaterFeed", () => {
  it("builds a fixed CDN release feed from a strict stable version", () => {
    assert.equal(
      ...
      "https://github.com/redrob-labs/redrob-cowork/releases/download/v0.17.23",
    );
  });
  ...
  it("allows only an explicit exact recovery downgrade", () => {
    assert.equal(
      ...
      "https://github.com/redrob-labs/redrob-cowork/releases/download/v0.17.22",
    );
    assert.throws(
...
describe("recovery metadata and candidates", () => {
  ...
  it("accepts only the exact platform, architecture, distribution release artifact", () => {
    ...
    assert.deepEqual(selectRecoveryArtifact(files, {
      ...
      arch: "arm64",
      distribution: "public",
      url: "https://github.com/redrob-labs/redrob-cowork/releases/download/v1.2.3/redrob-mac-arm64-1.2.3.dmg",
      sha512: "verified",
    });
  ...
  it("accepts each artifact flavor only for its matching distribution", () => {
    ...
    for (const [distribution, fileName] of Object.entries(artifacts)) {
      ...
      })?.url, `https://github.com/redrob-labs/redrob-cowork/releases/download/v1.2.3/${fileName}`);
...
`);
    ...
    try {
      await assert.rejects(
        cacheVerifiedRecoveryArtifact({
          ...
          artifact: { url: "https://github.com/redrob-labs/redrob-cowork/releases/download/v1.2.3/redrob.dmg", sha512: "invalid" },
  ...
  it("rejects cached metadata with a modified URL or wrong installer filename", async () => {
    ...
    const artifact = {
      ...
      arch: "arm64",
      distribution: "public",
      url: "https://github.com/redrob-labs/redrob-cowork/releases/download/v0.18.18/redrob-mac-arm64-0.18.18.dmg",
      sha512: createHash("sha512").update(bytes).digest("base64"),
    };
    ...
    try {
      ...
      await writeFile(metadataPath, JSON.stringify({ ...metadata, url: "https://tampered.invalid/Redrob Cowork.dmg" }), "utf8");
      assert.equal(await readCachedRecoveryArtifact(app, expected), null);
      await writeFile(metadataPath, JSON.stringify({ ...metadata, fileName: "Redrob Cowork.dmg" }), "utf8");
      assert.equal(await readCachedRecoveryArtifact(app, expected), null);
    } finally {
      await rm(userData, { recursive: true, force: true });
    }
  ...
  it("discovers and opens a reverified cached healthy installer while offline", async () => {
    ...
    const artifact = {
      ...
      arch: "arm64",
      distribution: "public",
      url: "https://github.com/redrob-labs/redrob-cowork/releases/download/v1.9.0/redrob-mac-arm64-1.9.0.dmg",
      sha512: createHash("sha512").update(bytes).digest("base64"),
    };
    const handlers = new Map();
    const opened = [];
    ...
    try {
      ...
      isolated.registerUpdaterIpc({
        ...
        electronNet: { fetch: async (url) => {
          if (!url.includes("/releases/download/v1.9.0/")) return new Response("missing", { status: 404 });
          candidateFetches += 1;
  ...
  it("pins enterprise builds to their parallel stable manifest channel", async () => {
    ...
    try {
      ...
      assert.deepEqual(await setChannel(null, "alpha"), {
        channel: "stable",
        feedUrl: "https://github.com/redrob-labs/redrob-cowork/releases/latest/download",
