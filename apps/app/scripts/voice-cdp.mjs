...
async function main() {
  ...
  try {
    await waitFor(client, "Boolean(window.__redrobControl)", 15000);
...
async function runPreflight(client) {
  ...
  const controlReady = await evaluate(client, "Boolean(window.__redrobControl)");
  const actions = controlReady
    ? await evaluate(client, "window.__redrobControl.listActions().map((action) => action.id)")
    : [];
...
async function ensureVoicePanel(client) {
  await evaluate(client, "window.__redrobControl.setEnabled(true)");
  await evaluate(client, "window.localStorage.setItem('redrob.extension.enabled.redrob-voice', '1'); window.dispatchEvent(new CustomEvent('redrob:extension-state-changed', { detail: { id: 'redrob-voice', enabled: true } }))");
  let actions = await evaluate(client, "window.__redrobControl.listActions().map((action) => action.id)");
  ...
  if (actions.includes("voice.panel.open")) {
    ...
    await waitFor(client, "window.__redrobControl.listActions().some((action) => action.id === 'voice.inject_audio')", 8000);
    return;
  }
...
async function readProof(client) {
  return evaluate(client, `({
    href: location.href,
    narration: window.__redrobControl?.snapshot?.().narration ?? "",
    route: window.__redrobControl?.snapshot?.().route ?? "",
    bodyText: document.body.innerText.slice(-2400),
  })`);
}

async function executeControl(client, actionId, actionArgs = undefined) {
  const expression = `window.__redrobControl.execute(${JSON.stringify(actionId)}, ${JSON.stringify(actionArgs)})`;
...
async function synthesizePcm16Base64(input) {
  ...
  const dir = await mkdtemp(join(tmpdir(), "redrob-voice-cdp-"));
