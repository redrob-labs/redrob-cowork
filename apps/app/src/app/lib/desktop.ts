...
export async function openDesktopUrl(url: string): Promise<void> {
  ...
  const openExternal = window.__REDROB_ELECTRON__?.shell?.openExternal;
  if (openExternal) {
...
export async function applyBrandIcon(url: string | null): Promise<BrandIconApplyResult> {
  const apply = typeof window !== "undefined" ? window.__REDROB_ELECTRON__?.brandIcon?.apply : undefined;
  if (!apply) return { ok: false, reason: "bridge-unavailable" };
  return apply(url);
}

export async function getBrandIconState(): Promise<BrandIconState | null> {
  const getState = typeof window !== "undefined" ? window.__REDROB_ELECTRON__?.brandIcon?.getState : undefined;
...
export async function relaunchDesktopApp(): Promise<void> {
  await window.__REDROB_ELECTRON__?.shell?.relaunch?.();
}
...
export async function subscribeDesktopDeepLinks(
  ...
  const initialUrls = window.__REDROB_ELECTRON__?.meta?.initialDeepLinks;
...
export function readInitialDesktopBootstrapConfig(): DesktopBootstrapConfig | null | undefined {
  ...
  return window.__REDROB_ELECTRON__?.meta?.desktopBootstrap;
}
