const PENDING_CHANGES_KEY = "redrob.settings.environment.pendingChanges";
...
export function buildRedrobEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "redrob"}::${runtime || "runtime"}`;
}

export function readRedrobEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeRedrobEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
