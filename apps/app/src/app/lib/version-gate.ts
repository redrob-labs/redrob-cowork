...
declare global {
  interface Window {
    __redrobReadDesktopVersionMetadataEval?: () => DenAppVersionMetadata | Promise<DenAppVersionMetadata>;
  }
}
...
export async function readFreshDenAppVersionMetadata(): Promise<DenAppVersionMetadata> {
  if (import.meta.env.DEV && typeof window !== "undefined" && window.__redrobReadDesktopVersionMetadataEval) {
    return window.__redrobReadDesktopVersionMetadataEval();
  }
