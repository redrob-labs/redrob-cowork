...
type DevRoot = typeof globalThis & {
  __redrobDevLogSeq?: number;
  __redrobDevLogs?: DevLogRecord[];
};
...
export const recordDevLog = (
  ...
  const id = (root.__redrobDevLogSeq ?? 0) + 1;
  root.__redrobDevLogSeq = id;
...
export const clearDevLogs = () => {
  const root = globalThis as DevRoot;
  root.__redrobDevLogs = [];
  root.__redrobDevLogSeq = 0;
};
