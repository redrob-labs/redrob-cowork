...
export type RedrobCloudMcpProviderModelContext = {
  provider: string;
  model: string;
};

export type RedrobCloudMcpFailureStage =
  | "prerequisites"
  | "token_mint"
  | "desired_config"
  | "engine_delivery"
...
export type RedrobCloudMcpFailureCode =
...
export type RedrobCloudMcpFailure = {
  code: RedrobCloudMcpFailureCode;
  stage: RedrobCloudMcpFailureStage | string;
  retryable: boolean;
...
export type RedrobCloudMcpCompatibility = {
  redrob: {
...
export type RedrobCloudMcpHealthPhase =
  | "missing_desired"
  | "workspace_ambiguous"
  | "engine_unconfigured"
  | "engine_unreachable"
...
export type RedrobCloudMcpDeliverySnapshot = {
  ...
  trigger?: string;
  failure?: RedrobCloudMcpFailure;
};

export type RedrobCloudMcpProbeStep = {
...
export type RedrobCloudMcpProbeTrace = {
  endpoint: string | null;
  startedAt: string;
  latencyMs: number;
  protocolVersion: string | null;
  serverInfo: { name: string | null; version: string | null } | null;
  steps: RedrobCloudMcpProbeStep[];
};

export type RedrobCloudMcpEngineRefreshStep = {
...
export type RedrobCloudMcpEngineRefresh = {
  performed: boolean;
  reason?: "desired_missing" | string;
  trigger: string;
  startedAt: string;
  finishedAt: string;
  steps: RedrobCloudMcpEngineRefreshStep[];
};

export type RedrobCloudMcpEngineRefreshResult = {
  refresh: RedrobCloudMcpEngineRefresh;
  health: RedrobCloudMcpHealth;
};

export type RedrobCloudMcpHealth = {
  schemaVersion: 1;
  phase: RedrobCloudMcpHealthPhase;
  usable: boolean;
  ...
  delivery: RedrobCloudMcpDeliverySnapshot;
  engine: {
  ...
  tools: {
    ...
    direct: {
      ...
      present: string[];
      missing: string[];
      trace?: RedrobCloudMcpProbeTrace;
      error?: unknown;
      failure?: RedrobCloudMcpFailure;
    };
    providerProjection: {
  ...
  compatibility: RedrobCloudMcpCompatibility;
  toolDenies: unknown[];
  firstFailure: RedrobCloudMcpFailure | null;
  checkedAt: string;
  durationMs?: number;
};

export type RedrobCloudMcpReconcilePayload = {
  workspaceId: string;
  name: "redrob-cloud";
  config: Record<string, unknown>;
...
async function requestAgentContextDiagnosticsJson(
  ...
  if (!result.response.ok) {
    ...
    const details = payload && typeof payload === "object" && "details" in payload
      ? payload.details
      : undefined;
    throw new RedrobServerError(result.response.status, code, message, details);
  }
...
export function createRedrobServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    getRedrobCloudMcpHealth: (
      ...
      // probe=1 verifies the Cloud endpoint directly from the Redrob Work server
      // (initialize + tools/list), independent of the engine's own connection.
      if (options?.probe) query.set("probe", "1");
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<RedrobCloudMcpHealth>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/health${suffix}`,
        { token, hostToken, timeoutMs: options?.probe ? timeouts.cloudMcpProbeHealth : timeouts.cloudMcpHealth },
      );
    },
    reconcileRedrobCloudMcp: (workspaceId: string, payload: RedrobCloudMcpReconcilePayload) =>
      requestJson<RedrobCloudMcpHealth>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/reconcile`,
        {
          token,
          hostToken,
          method: "POST",
    ...
    refreshRedrobCloudMcpEngine: (
      ...
      requestJson<RedrobCloudMcpEngineRefreshResult>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/mcp/redrob-cloud/engine-refresh`,
        {
          token,
          hostToken,
          method: "POST",
