...
type PerfRoot = typeof globalThis & {
  __redrobPerfSeq?: number;
  __redrobPerfLogs?: PerfLogRecord[];
  __redrobPerfConsoleAt?: Record<string, number>;
  __redrobPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__redrobPerfSeq ?? 0) + 1;
  root.__redrobPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__redrobPerfConsoleAt ?? (root.__redrobPerfConsoleAt = {});
    const suppressedByKey =
      root.__redrobPerfConsoleSuppressed ?? (root.__redrobPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__redrobPerfLogs = [];
  root.__redrobPerfSeq = 0;
};
