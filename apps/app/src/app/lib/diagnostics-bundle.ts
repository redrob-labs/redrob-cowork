...
function pickAppInfo(info: AppBuildInfo | null) {
  ...
  return {
    ...
    redrobDevMode: info.redrobDevMode ?? null,
  };
}
...
function pickHostInfo(info: RedrobServerInfo | null) {
  if (!info) return null;
  return {
...
function defaultHostConnectUrl(hostInfo: RedrobServerInfo | null) {
...
function collectSecretValues(input: DiagnosticsBundleInputs) {
  ...
  addSecretValue(secrets, input.redrobServerSettings.token);
  addSecretValue(secrets, input.redrobServerSettings.hostToken);
...
export function composeDiagnosticsBundleJson(input: DiagnosticsBundleInputs): string {
  ...
  const urlOverride = input.redrobServerSettings.urlOverride?.trim() ?? "";
  const token = input.redrobServerSettings.token?.trim() ?? "";
  ...
  const bundle = {
    ...
    redrobServer: {
      status: context?.redrobServerStatus ?? (clientConnected ? "connected" : "disconnected"),
      url: context?.redrobServerUrl ?? "",
      settings: {
