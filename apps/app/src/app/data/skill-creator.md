---
name: skill-creator
description: Create or update skills in Redrob Work Cloud when available, or locally when requested.
---
...
A local skill is a folder under `.opencode/skills/<skill-name>/` or `.claude/skills/<skill-name>/` anchored by `SKILL.md`. A remote skill is stored in Redrob Work Cloud as a plugin skill component.

## Redrob Work authoring contract

Follow the runtime `Skill creation:` instruction for this workspace/model:

- `Cloud`: create or update the skill in Redrob Work Cloud first when it is available, then follow the remote flow below.
...
1. Search Redrob Work Cloud with 2-4 variants for existing skills/config objects, plugins, marketplaces, plugin creation, config-object version creation, and read operations. Use only exact capability names returned by search.
