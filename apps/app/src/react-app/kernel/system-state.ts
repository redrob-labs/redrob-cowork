...
import { relaunchDesktopApp, resetRedrobState } from "../../app/lib/desktop";
import {
...
export type ResetState = {
  ...
  resetModalMode: ResetRedrobMode;
...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetRedrobMode) => void;
...
function clearRedrobLocalStorage(mode: ResetRedrobMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (/redrob/.test(key)) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("redrob_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetRedrobMode>("onboarding");
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetRedrobState(resetModalMode);
      }
      clearRedrobLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
