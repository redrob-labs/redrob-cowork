...
import type {
  RedrobConversationLayout,
  RedrobContextSnapshot,
  RedrobPanelTab,
  RedrobResourceDescriptor,
  RedrobScreen,
} from "@redrob/types/redrob-context";
...
type RedrobContextProjectorInput = {
  route: string;
  revision: number;
  capturedAt: string;
  ...
  availableAffordances: RedrobAffordanceDescriptor[];
};
...
export function screenFromRoute(route: string): RedrobScreen {
...
function panelTab(tab: PanelTabStore["sessions"][string]["tabs"][number]): RedrobPanelTab {
  if (tab.type === "browser") {
    return {
      id: tab.id,
      kind: "browser",
...
export function buildRedrobContext(
  input: RedrobContextProjectorInput,
): RedrobContextSnapshot {
  const primarySessionId = input.workbench.primarySessionId;
  const splitSessionId = input.workbench.splitSessionId;
  const layout: RedrobConversationLayout = primarySessionId && splitSessionId
    ? {
        kind: "split",
        primarySessionId,
  ...
  const provider: RedrobProviderRef = { id: "redrob-ui", kind: "builtin" };
