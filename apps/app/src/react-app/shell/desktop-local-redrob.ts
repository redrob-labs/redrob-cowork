...
export function isReadyLocalRedrobServerInfo(
  info: RedrobServerInfo | null | undefined,
): info is RedrobServerInfo {
  return Boolean(
    info?.running === true &&
      info.baseUrl?.trim() &&
...
export function redrobServerSettingsChanged(
  previous: RedrobServerSettings,
  next: RedrobServerSettings,
): boolean {
 ...
 */
export async function waitForReadyLocalRedrobServerInfo(options?: {
  fetchInfo?: () => Promise<RedrobServerInfo | null>;
  maxAttempts?: number;
  wait?: (delayMs: number) => Promise<void>;
}): Promise<RedrobServerInfo | null> {
  const fetchInfo =
    options?.fetchInfo ?? (() => redrobServerInfo() as Promise<RedrobServerInfo | null>);
 ...
 * sequence owns starting the server, and probing `redrobServerInfo`
...
export async function ensureDesktopLocalRedrobConnection(
  options: EnsureDesktopLocalRedrobOptions,
) {
  ...
  recordInspectorEvent("route.local_redrob.ensure.start", {
    route: options.route,
  ...
  try {
    ...
    const nextSettings = writeRedrobServerSettings({
    ...
    if (startedEngine || redrobServerSettingsChanged(previousSettings, nextSettings)) {
      emitRedrobSettingsChanged();
    }

    recordInspectorEvent("route.local_redrob.ensure.success", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
    ...
    recordInspectorEvent("route.local_redrob.ensure.error", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
      message,
