...
export const orgOnboardingVisibilityEvent = "redrob-org-onboarding-visibility";
...
export function ReloadCoordinatorProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("redrob-reload-required", handler);

    return () => window.removeEventListener("redrob-reload-required", handler);
