...
`apps/app` is a React 19 + Vite app. It is the UI for every Redrob Cowork
deployment: the Electron desktop shell loads it, plain web serves it, and it
talks to redrob-server / opencode / Den over HTTP. (The Solid runtime it
...
│   ├── lib/                   Clients + bridges: opencode, redrob-server, den, desktop (IPC),
...
2. Leaf modules (`runtime-env`, `desktop-types`, `den-types`, `extensions`)
   ...
   (`opencode`, `redrob-server`, `den`) import leaves — never the `utils/`
...
- The legacy `redrob.react.activeWorkspace` and
  `redrob.react.sessionByWorkspace` values are only restore/fallback memory.
...
`shell/redrob-context-projector.ts` combines route state, the workbench store,
UI chrome state, and panel-tab state into the shared
`RedrobContextSnapshot` contract. `RedrobContextPublisher` publishes that
snapshot through `window.__redrobControl`; the desktop loopback bridge and
`redrob-ui-mcp` expose the same contract without requiring every element to
...
`apps/app/tests/**/*.test.ts` is the executable coverage home for these rails;
add an observable assertion there whenever a row changes.
