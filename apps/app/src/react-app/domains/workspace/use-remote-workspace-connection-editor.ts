...
import { buildRedrobWorkspaceBaseUrl, type RedrobServerClient } from "../../../app/lib/redrob-server";
...
export function useRemoteWorkspaceConnectionEditor<TWorkspace extends WorkspaceInfo>(input: {
  ...
  const initialValues = useMemo(
    () => {
      const hostUrl = workspace?.redrobHostUrl ?? workspace?.baseUrl ?? "";
      const mountedUrl = workspace?.remoteType === "redrob"
        ? buildRedrobWorkspaceBaseUrl(hostUrl, workspace.redrobWorkspaceId) ?? hostUrl
        : hostUrl;
      return {
        redrobHostUrl: mountedUrl,
        redrobToken:
  ...
  const save = useCallback(
    async (fields: RemoteWorkspaceInput) => {
      ...
      const baseUrl = fields.redrobHostUrl?.trim() ?? "";
      if (!id || !baseUrl) {
      ...
      try {
        ...
        const redrobToken = fields.redrobToken?.trim() ?? "";
        if (isDesktopRuntime()) {
          await workspaceUpdateRemote({
            workspaceId: id,
            baseUrl,
            redrobHostUrl: baseUrl,
            redrobToken,
            redrobClientToken: "",
            redrobHostToken: "",
            displayName,
          ...
          const connectionChanged = baseUrl !== (initialValues.redrobHostUrl?.trim() ?? "") ||
            redrobToken !== (initialValues.redrobToken?.trim() ?? "") ||
            directory !== (initialValues.directory?.trim() || null);
          if (connectionChanged) {
            const result = await client.createRemoteWorkspace({
              baseUrl,
              redrobHostUrl: baseUrl,
              redrobToken: redrobToken || null,
              displayName,
              directory,
              remoteType: "redrob",
            });
    ...
    [client, initialValues.directory, initialValues.redrobHostUrl, initialValues.redrobToken, onSaved, workspaceId],
  );
