    ...
    },
    {
      ...
      parts: [
        ...
        },
        {
          ...
          state: "output-available",
          input: { filePath: "/tmp/redrob-eval/plan-tomorrow.md", oldString: "", newString: "" },
          output: "",
        },
        {
          type: "dynamic-tool",
          ...
          state: "output-available",
          input: { filePath: "/tmp/redrob-eval/meeting-notes.md" },
          output: "",
        },
        {
          type: "dynamic-tool",
...
function statusLabel(snapshot: RedrobSessionSnapshot | undefined, busy: boolean) {
...
function hiddenMessageCount(snapshot: RedrobSessionSnapshot, revertMessageId: string): number {
...
export function SessionSurface(props: SessionSurfaceProps) {
  ...
  const [rendered, setRendered] = useState<{ sessionId: string; snapshot: RedrobSessionSnapshot } | null>(null);
  ...
  const opencodeClient = useMemo(
    () => createClient(props.opencodeBaseUrl, undefined, { token: props.redrobToken, mode: "redrob" }),
    [props.opencodeBaseUrl, props.redrobToken],
  );
  ...
  const snapshotQuery = useQuery<RedrobSessionSnapshot>({
  ...
  const seedMarkdownPrimitiveControlAction = useMemo<RedrobControlAction | null>(() => {
  ...
  const seedMarkdownMathControlAction = useMemo<RedrobControlAction | null>(() => {
    if (!import.meta.env.DEV) return null;
  ...
  const seedChatTranscriptControlAction = useMemo<RedrobControlAction | null>(() => {
    if (!import.meta.env.DEV) return null;
  ...
  useEffect(() => {
    ...
    window.addEventListener("redrob:voice-transcript", handleVoiceTranscript);
    return () => window.removeEventListener("redrob:voice-transcript", handleVoiceTranscript);
  }, [attachments, buildDraft, props.onDraftChange, props.sessionId, props.workspaceId, typeComposerText]);

  const composerSetTextControlAction = useMemo<RedrobControlAction>(() => ({
  ...
  const composerSendControlAction = useMemo<RedrobControlAction>(() => ({
    id: "composer.send",
  ...
  const composerStopControlAction = useMemo<RedrobControlAction>(() => ({
    id: "composer.stop",
  ...
  const sessionScrollTopControlAction = useMemo<RedrobControlAction>(() => ({
  ...
  const sessionScrollBottomControlAction = useMemo<RedrobControlAction>(() => ({
  ...
  const sessionLatestMessageControlAction = useMemo<RedrobControlAction>(() => ({
  ...
  const sessionReadTranscriptControlAction = useMemo<RedrobControlAction>(() => ({
