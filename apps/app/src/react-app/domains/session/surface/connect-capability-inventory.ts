  ...
  return {
    ...
    error: plugin.cloudReadiness?.state === "needs_admin_setup"
      ...
      : plugin.cloudReadiness?.state === "not_synced"
        ...
        : "This Redrob Work Connect capability is not ready.",
  };
}
...
function toSkill(
  ...
  return {
    name: object.title,
    path: `redrob-connect://${marketplace.id}/${plugin.id}/${object.id}`,
...
function toMcpEntries(
  ...
  return specs.map((spec) => {
    const id = `redrob-connect:${plugin.id}:${object.id}:${spec.name}`;
    ...
    for (const membership of resolved.memberships) {
      ...
      if (object.objectType === "skill") {
        ...
        pluginCard.files.push({
          ...
          objectType: object.objectType,
          title: object.title,
          path: `redrob-connect://${marketplace.id}/${resolved.plugin.id}/${object.id}`,
          versionId: object.latestVersion?.id ?? null,
          updatedAt: object.updatedAt,
        });
      }
