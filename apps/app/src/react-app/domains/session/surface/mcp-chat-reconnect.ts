...
export async function waitForFreshMcpAuthorization(input: {
  ...
  while (now() - startedAt < timeoutMs) {
    if (!input.isScopeCurrent()) {
      throw new Error("The active Redrob Work Cloud account changed while reconnecting. Try again in this workspace.")
    }
    try {
      ...
      if (error instanceof Error && error.message.startsWith("The active Redrob Work Cloud account changed")) throw error
