...
export function SessionPage(props: SessionPageProps) {
  ...
  const openBrowserUrlControlAction = useMemo<RedrobControlAction>(() => ({
    ...
    description: "Create or select a Redrob Cowork built-in browser tab, navigate it to a URL, and return the CDP handle for browser automation.",
  ...
  const focusWorkbenchSessionControlAction = useMemo<RedrobControlAction>(() => ({
    ...
    args: [{
      ...
      type: "string",
      required: true,
      description: "Session id from the Redrob Cowork context resources or conversation tabs.",
    }],
    execute: (args) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[radial-gradient(circle_at_top,color-mix(in_srgb,var(--primary)_12%,transparent),transparent_42%),var(--dls-app-bg)] text-dls-text max-lg:pt-[env(safe-area-inset-top)] mac:bg-transparent">
      <SidebarProvider
        open={sidebarOpen}
