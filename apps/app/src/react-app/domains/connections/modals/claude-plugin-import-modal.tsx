...
export type ClaudePluginImportModalProps = {
  open: boolean;
  onClose: () => void;
  onPreview: (url: string) => Promise<RedrobClaudePluginPreview>;
...
type ModalState = {
  url: string;
  preview: RedrobClaudePluginPreview | null;
...
type ModalAction =
  | Partial<ModalState>
  | "reset"
  | { kind: "preview-success"; url: string; preview: RedrobClaudePluginPreview };
    ...
    >
      <DialogContent className="flex max-h-[90vh] min-h-0 w-full max-w-lg flex-col overflow-hidden sm:max-w-lg">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto">
          ...
          {state.preview ? (
            <div className="space-y-3 rounded-xl border border-dls-border bg-dls-hover/40 p-4">
              ...
              {state.preview.warnings.length > 0 ? (
                <div className="rounded-lg border border-warning-muted bg-warning-soft px-3 py-2 text-xs text-warning-ink">
          ...
          {state.error ? (
            <div className="rounded-lg border border-destructive-muted bg-destructive-soft px-3 py-2 text-xs text-destructive-ink">
              {state.error}
            </div>
          ) : null}
        </div>
