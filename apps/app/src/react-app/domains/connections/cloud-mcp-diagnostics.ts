 ...
 */
export function cloudMcpEngineErrorText(health: RedrobCloudMcpHealth | null): string | null {
...
export function cloudMcpProbeTraceLines(trace: RedrobCloudMcpProbeTrace | null | undefined): string[] {
  if (!trace) return [];
...
export function cloudMcpEngineRefreshLines(refresh: RedrobCloudMcpEngineRefresh | null | undefined): string[] {
  if (!refresh) return [];
 ...
 */
export function cloudMcpAdvancedRows(health: RedrobCloudMcpHealth | null): CloudMcpAdvancedRow[] {
  if (!health) return [];
  ...
  if (inspection?.checked) {
    if (inspection.cloudPresent === false) {
      rows.push({
        ...
        value: "redrob-cloud is not registered in the engine (the dynamic entry is lost after an engine restart) — use Refresh engine connection",
        tone: "error",
      });
    }
  ...
  rows.push({
    label: "Versions",
    value: `app ${describeCloudMcpErrorDetail(compatibility.redrob.app?.version) ?? "unknown"} · server ${compatibility.redrob.serverVersion ?? "unknown"} · engine ${compatibility.opencode.actualVersion ?? "unknown"}${compatibility.opencode.expectedVersion ? ` (expected ${compatibility.opencode.expectedVersion})` : ""}`,
    tone: "muted",
  });
  rows.push({
    label: "Checked",
...
export function buildCloudMcpSupportBundle(input: {
  ...
  const bundle = {
    kind: "redrob-cloud-mcp-diagnostic",
