...
export type CloudMcpClient = {
  baseUrl: string;
  getRedrobCloudMcpHealth: (
    workspaceId: string,
    providerModel?: RedrobCloudMcpProviderModelContext,
    options?: { probe?: boolean },
  ) => Promise<RedrobCloudMcpHealth>;
  reconcileRedrobCloudMcp: (
    workspaceId: string,
    payload: RedrobCloudMcpReconcilePayload,
  ) => Promise<RedrobCloudMcpHealth>;
  refreshRedrobCloudMcpEngine?: (
    workspaceId: string,
    payload?: { provider?: string; model?: string; trigger?: string },
  ) => Promise<RedrobCloudMcpEngineRefreshResult>;
};
...
type CloudMcpReconcilerInput = {
  ...
  /**
   * Ask the Redrob Work server to also verify the Cloud endpoint directly
...
function isProviderProjectionFailure(failure?: RedrobCloudMcpFailure | null): boolean {
...
export function buildRedrobCloudMcpReconcilePayload(input: {
  context: CloudMcpOperationContext;
  token: DenMcpToken;
}): RedrobCloudMcpReconcilePayload | null {
...
export function isCloudMcpAuthTokenFailureCode(code: string | null | undefined): boolean {
  ...
  return normalized === "redrob_cloud_auth_required" ||
    normalized === "redrob_cloud_auth_invalid" ||
    normalized === "redrob_cloud_token_expired" ||
 ...
 * `redrob_cloud_token_expired`). Remint decisions must consider both.
 */
export function isCloudMcpAuthTokenFailure(failure: Pick<RedrobCloudMcpFailure, "code" | "aliases"> | null | undefined): boolean {
  if (!failure) return false;
...
async function probeHealth(input: CloudMcpReconcilerInput, scope: CloudMcpScope, options?: { writeFreshnessMarker?: boolean }): Promise<CloudMcpOperationResult> {
  const health = await input.client.getRedrobCloudMcpHealth(
    scope.workspaceId,
    input.context.providerModel,
...
async function mintAndPost(input: CloudMcpReconcilerInput, scope: CloudMcpScope): Promise<{ health: RedrobCloudMcpHealth | null; token: DenMcpToken | null }> {
  ...
  const payload = buildRedrobCloudMcpReconcilePayload({ context: { ...input.context, ...scope }, token });
  if (!payload) return { health: null, token };
  return {
    health: await input.client.reconcileRedrobCloudMcp(scope.workspaceId, payload),
    token,
  };
}
...
async function repairCloudMcp(input: CloudMcpReconcilerInput, scope: CloudMcpScope): Promise<CloudMcpOperationResult> {
  ...
  if (!input.force && marker && isCloudMcpSyncMarkerFresh({
    ...
    const health = await input.client.getRedrobCloudMcpHealth(scope.workspaceId, input.context.providerModel);
...
export async function runRedrobCloudMcpReconciler(input: CloudMcpReconcilerInput): Promise<CloudMcpOperationResult> {
...
export type CloudMcpEngineRefreshRunResult = {
  ...
  health: RedrobCloudMcpHealth | null;
  refresh: RedrobCloudMcpEngineRefresh | null;
};

const engineRefreshInFlight = new Map<string, Promise<CloudMcpEngineRefreshRunResult>>();

/**
 * Force the engine to drop its redrob-cloud MCP client and reconnect.
 ...
 */
export async function runRedrobCloudMcpEngineRefresh(input: {
  client: CloudMcpClient;
  context: CloudMcpOperationContext;
  ...
  const refreshEngine = input.client.refreshRedrobCloudMcpEngine;
  if (!refreshEngine) {
...
export function cloudMcpFailureStageLabel(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Redrob Work components need updating";
...
export function cloudMcpRecommendedAction(input: {
  ...
  if (!input.signedIn) return "Sign in to Redrob Work Cloud.";
  ...
  if (code.includes("scope")) return "Reconnect Redrob Work Cloud with the required permissions.";
  ...
  if (isProviderProjectionFailure(input.health?.firstFailure)) return "Choose a model that can use Redrob Work Cloud tools.";
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Update Redrob Work, then retry.";
  if (code === "extensions_plugin_missing") return "Reload the agent so Redrob Work instructions are current.";
  if (code === "cloud_tools_missing") return "Reconnect Redrob Work Cloud so the endpoint exposes search_capabilities and execute_capability.";
...
export async function cleanupRedrobCloudMcpAfterSignOut(input: {
  context: CloudMcpScope;
  redrobClient: CleanupClient | null;
  ...
  await Promise.all([
    input.redrobClient && scope
      ? input.redrobClient.removeMcp(scope.workspaceId, CLOUD_MCP_SERVER_NAME).catch(() => null)
