...
type CloudMcpSubmitReadinessClient = Pick<
  RedrobServerClient,
  "baseUrl" | "getRedrobCloudMcpHealth" | "reconcileRedrobCloudMcp" | "listMcp"
>;
...
function missingContextIssue(input: {
  ...
  if (!input.client || !input.workspaceId) {
    return {
      ...
      retryable: true,
      message: "Redrob Work could not resolve the workspace server before checking connected service tools.",
  ...
  return {
    ...
    stage: "provider_projection",
    retryable: false,
    message: "Redrob Work could not verify connected service tools for this submission.",
    ...
    if (gateRequired) {
      prepare = async () => {
        ...
        const result = await ensureCloudMcpSubmissionReadiness({
          providerModel,
          check: () => client.getRedrobCloudMcpHealth(activeWorkspaceId, providerModel, { probe: true }),
          repair: async () => {
