...
function MarkdownBlockInner({
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      const copyButton = event.target.closest("[data-redrob-code-copy]");
      if (copyButton instanceof HTMLButtonElement) {
        event.preventDefault();
        event.stopPropagation();

        const codeBlock = copyButton.closest("[data-redrob-code-block]");
      ...
      if (chevron instanceof HTMLElement) {
        event.preventDefault();
        event.stopPropagation();
        const href = chevron.dataset.redrobLinkChevron ?? "";
      ...
      const link = event.target.closest("a[data-redrob-link-href]");
      if (link instanceof HTMLAnchorElement) {
        const href = link.dataset.redrobLinkHref ?? link.getAttribute("href") ?? "";
        const target = openTargetForHref(href, openTargets);
      ...
      const preview = event.target.closest("[data-redrob-image-preview]");
