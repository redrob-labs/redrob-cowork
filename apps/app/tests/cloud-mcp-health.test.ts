...
import type { RedrobCloudMcpFailure, RedrobCloudMcpHealth, RedrobCloudMcpReconcilePayload } from "../src/app/lib/redrob-server";
import {
...
import {
  buildRedrobCloudMcpReconcilePayload,
...
function health(input: { usable: boolean; failure?: RedrobCloudMcpFailure | null; projectionChecked?: boolean }): RedrobCloudMcpHealth {
  ...
  return {
    ...
    pluginCanaries: { expected: ["redrob_docs_search"], present: usable ? ["redrob_docs_search"] : [], missing: usable ? [] : ["redrob_docs_search"] },
    compatibility: {
...
describe("Redrob Work Cloud MCP reconciler", () => {
  beforeEach(() => installStorageStub());

  test("uses the minted web proxy resource instead of a stale direct API fallback", () => {
    const payload = buildRedrobCloudMcpReconcilePayload({
      context: {
        ...context,
        fallbackUrl: "https://api.redrob.test/mcp/agent",
      },
      token: {
        ...token,
        resource: "https://app.redrob.test/api/den/mcp",
      },
    });

    expect(payload?.config.url).toBe("https://app.redrob.test/api/den/mcp/agent");
  });
  ...
  test("Test now with probe asks the server for a direct endpoint verification", async () => {
    ...
    await runRedrobCloudMcpReconciler({
      mode: "health",
      client,
      context,
  ...
  test("engine refresh maps the endpoint result and skips unsupported servers", async () => {
    ...
    const result = await runRedrobCloudMcpEngineRefresh({
      client: {
        baseUrl: scope.serverBaseUrl,
        getRedrobCloudMcpHealth: async () => refreshedHealth,
        reconcileRedrobCloudMcp: async () => refreshedHealth,
        refreshRedrobCloudMcpEngine: async (workspaceId, payload) => {
    ...
    expect(calls).toEqual([
      {
        ...
        payload: { provider: "redrob", model: "gpt-5", trigger: "desktop-engine-refresh" },
      },
    ]);

    const failed = await runRedrobCloudMcpEngineRefresh({
      client: {
        baseUrl: scope.serverBaseUrl,
        getRedrobCloudMcpHealth: async () => refreshedHealth,
        reconcileRedrobCloudMcp: async () => refreshedHealth,
        refreshRedrobCloudMcpEngine: async () => ({
    ...
    const skipped = await runRedrobCloudMcpEngineRefresh({
      client: {
        baseUrl: scope.serverBaseUrl,
        getRedrobCloudMcpHealth: async () => refreshedHealth,
        reconcileRedrobCloudMcp: async () => refreshedHealth,
  ...
  test("writes marker only when returned health is usable", async () => {
    const client = {
      baseUrl: scope.serverBaseUrl,
      getRedrobCloudMcpHealth: async () => health({ usable: false, failure: failure("cloud_status_missing") }),
      reconcileRedrobCloudMcp: async () => health({ usable: false, failure: failure("cloud_status_missing") }),
    };

    await runRedrobCloudMcpReconciler({ mode: "repair", client, context, mintToken: async () => token, force: true, refreshMarginMs: 1 });
    expect(readCloudMcpSyncMarker(scope)).toBeNull();

    await runRedrobCloudMcpReconciler({
      mode: "repair",
      client: { ...client, reconcileRedrobCloudMcp: async () => health({ usable: true }) },
      context,
      mintToken: async () => token,
      force: true,
      refreshMarginMs: 1,
  ...
  test("auth failures remint exactly once", async () => {
    let mintCount = 0;
    const posts: RedrobCloudMcpReconcilePayload[] = [];
    const result = await runRedrobCloudMcpReconciler({
      mode: "repair",
      client: {
        baseUrl: scope.serverBaseUrl,
        getRedrobCloudMcpHealth: async () => health({ usable: false }),
        reconcileRedrobCloudMcp: async (_workspaceId, payload) => {
          posts.push(payload);
          return posts.length === 1
            ? health({ usable: false, failure: failure("redrob_cloud_token_expired") })
            : health({ usable: true });
        },
      },
      context,
  ...
  test("membership and scope failures do not retry", async () => {
    for (const code of ["redrob_cloud_membership_required", "redrob_cloud_scope_missing", "redrob_cloud_resource_forbidden"]) {
    ...
    await runRedrobCloudMcpReconciler({
      ...
      client: {
        ...
        reconcileRedrobCloudMcp: async () => {
          postCount += 1;
          return health({ usable: false, failure: failure("redrob_cloud_membership_required") });
        },
      },
      context,
      mintToken: async () => {
  ...
  test("expired first-party token codes count as auth failures", () => {
    ...
    expect(isCloudMcpAuthTokenFailureCode("redrob_cloud_token_expired")).toBe(true);
    expect(isCloudMcpAuthTokenFailureCode("invalid_token")).toBe(true);
    // Exclusions still hold.
    expect(isCloudMcpAuthTokenFailureCode("redrob_cloud_client_registration_required")).toBe(false);
  ...
  test("auth aliases trigger the remint retry when the primary code is unrecognized", async () => {
    expect(isCloudMcpAuthTokenFailure({ code: "cloud_connection_failed", aliases: ["redrob_cloud_token_expired"] })).toBe(true);
    ...
    const result = await runRedrobCloudMcpReconciler({
      ...
      client: {
        ...
        reconcileRedrobCloudMcp: async (_workspaceId, payload) => {
          posts.push(payload);
          return posts.length === 1
            ? health({ usable: false, failure: { ...failure("invalid_mcp_token"), aliases: ["redrob_cloud_token_expired"] } })
            : health({ usable: true });
        },
      },
      context,
