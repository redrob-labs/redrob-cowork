...
import type { RedrobServerInfo } from "../src/app/lib/desktop";
import {
  isReadyLocalRedrobServerInfo,
  LOCAL_REDROB_READINESS_RETRY_DELAY_MS,
  redrobServerSettingsChanged,
  shouldAttemptDesktopLocalReconnect,
  waitForReadyLocalRedrobServerInfo,
  type DesktopLocalReconnectInput,
} from "../src/react-app/shell/desktop-local-redrob";

describe("redrobServerSettingsChanged", () => {
  test("does not refresh the renderer for an unchanged healthy server", () => {
    ...
    expect(redrobServerSettingsChanged(settings, { ...settings })).toBe(false);
    expect(redrobServerSettingsChanged(settings, { ...settings, token: "new-owner" })).toBe(true);
  });
});

function serverInfo(overrides: Partial<RedrobServerInfo> = {}): RedrobServerInfo {
  return {
    running: false,
...
describe("isReadyLocalRedrobServerInfo", () => {
  test("requires running, a base URL, and at least one token", () => {
    expect(isReadyLocalRedrobServerInfo(readyInfo())).toBe(true);
    expect(
      isReadyLocalRedrobServerInfo(
    ...
    expect(
      isReadyLocalRedrobServerInfo(serverInfo({ running: true, ownerToken: "tok_owner" })),
    ).toBe(false);
    expect(
      isReadyLocalRedrobServerInfo(serverInfo({ running: true, baseUrl: "http://127.0.0.1:4187" })),
    ).toBe(false);
    expect(isReadyLocalRedrobServerInfo(null)).toBe(false);
  });
});

describe("waitForReadyLocalRedrobServerInfo", () => {
  test("keeps polling through a restart gap until the server is ready", async () => {
    const responses: Array<RedrobServerInfo | null> = [
    ...
    const result = await waitForReadyLocalRedrobServerInfo({
    ...
    expect(isReadyLocalRedrobServerInfo(result)).toBe(true);
  });
  ...
  test("is bounded: reports the last unready observation after max attempts", async () => {
    ...
    expect(calls).toBe(4);
    expect(isReadyLocalRedrobServerInfo(result)).toBe(false);
