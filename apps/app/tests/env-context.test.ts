...
import {
  buildRedrobEnvSystemContext,
  clearRedrobEnvSystemContextCache,
} from "../src/react-app/domains/session/sync/env-context";

function client(keys: string[], calls: { count: number }): RedrobServerClient {
  return {
...
describe("buildRedrobEnvSystemContext", () => {
  test("lists configured key names without inventing secret values", async () => {
    clearRedrobEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildRedrobEnvSystemContext(
  ...
  test("caches key context per session", async () => {
    ...
    await buildRedrobEnvSystemContext(server, {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
    await buildRedrobEnvSystemContext(server, {
  ...
  test("does not truncate long key lists", async () => {
    ...
    const context = await buildRedrobEnvSystemContext(client(keys, calls), {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
  ...
  test("skips context while environment changes are pending", async () => {
    clearRedrobEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildRedrobEnvSystemContext(client(["ANTHROPIC_API_KEY"], calls), {
      cacheKey: "session-a",
