...
import type { RedrobCloudMcpHealth } from "../src/app/lib/redrob-server";
import {
...
function baseHealth(overrides?: Partial<RedrobCloudMcpHealth>): RedrobCloudMcpHealth {
  return {
    schemaVersion: 1,
    phase: "engine_failed",
    usable: false,
    ...
    engineInspection: {
      ...
      serverCount: 2,
      servers: [
        { name: "redrob-cloud", status: "failed", error: "fetch failed" },
    ...
    tools: {
      ...
      present: [],
      missing: ["redrob-cloud_search_capabilities", "redrob-cloud_execute_capability"],
      direct: {
        checked: false,
    ...
    compatibility: {
      redrob: { serverVersion: "0.17.36", app: { version: "0.17.36" } },
...
describe("cloud MCP advanced diagnostics", () => {
  ...
  test("advanced rows expose failure code, engine detail, sibling servers, token, and timings", () => {
    ...
    expect(byLabel.get("Engine MCP servers")?.value).toContain("redrob-cloud failed — fetch failed");
  ...
  test("shows the probe transport cause from the direct check", () => {
      ...
      tools: {
        ...
        direct: {
          ...
          failure: {
            ...
            message: "The Redrob Work server could not reach the Cloud MCP endpoint.",
            details: {
              transport: {
          ...
          trace: {
            endpoint: "https://api.redrob.test/mcp/agent",
    ...
    expect(rows.find((row) => row.label === "Endpoint")?.value).toBe("https://api.redrob.test/mcp/agent");
  });
  ...
  test("formats probe trace and engine refresh step lines", () => {
    expect(cloudMcpProbeTraceLines({
      ...
      serverInfo: { name: "redrob-cloud", version: "1.0.0" },
      steps: [
  ...
  test("support bundle is JSON, carries context, and redacts bearer material", () => {
    ...
    expect(parsed.kind).toBe("redrob-cloud-mcp-diagnostic");
