...
import {
  buildRedrobContext,
  screenFromRoute,
} from "../src/react-app/shell/redrob-context-projector";
import type { WorkbenchSnapshot } from "../src/react-app/domains/session/chat/workbench-store";

type ContextProjectorInput = Parameters<typeof buildRedrobContext>[0];
...
describe("Redrob Work context projector", () => {
  ...
  test("does not leak artifact tabs into a non-panel surface", () => {
    const context = buildRedrobContext({
      ...baseInput,
      ui: {
        ...baseInput.ui,
