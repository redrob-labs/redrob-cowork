...
function instance(input: Partial<DenCloudInstance> = {}): DenCloudInstance {
  return {
    ...
    imageVersion: "imageVersion" in input ? input.imageVersion ?? null : "redrob-0.18.8",
    ...(typeof input.instanceName === "string" ? { instanceName: input.instanceName } : {}),
    latestVersion: "latestVersion" in input ? input.latestVersion ?? null : "redrob-0.18.8",
  };
}
...
describe("cloud workspace overlay state", () => {
  ...
  test("maps stale and legacy workers to Update available", () => {
    const stale = mapCloudWorkspaceState({
      instance: instance({ imageVersion: "redrob-0.18.2", latestVersion: "redrob-0.18.8" }),
      updating: false,
    });
    const legacyInstance = instance({ imageVersion: null, latestVersion: "redrob-0.18.8" });
