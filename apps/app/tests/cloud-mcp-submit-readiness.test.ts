...
const PROVIDER_MODEL = { provider: "redrob", model: "gpt-5" };

function failure(input?: Partial<RedrobCloudMcpFailure>): RedrobCloudMcpFailure {
  return {
...
function health(input?: {
  ...
  const projected = usable
    ? ["redrob-cloud_search_capabilities", "redrob-cloud_execute_capability"]
    : [];
  ...
  return {
    ...
    tools: {
      ...
      providerProjection: {
        ...
        missing: projectionPresent.length ? [] : ["redrob-cloud_search_capabilities", "redrob-cloud_execute_capability"],
      },
    },
