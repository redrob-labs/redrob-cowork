...
export function workspaceIdForRedrob(hostUrl: string, workspaceId?: string | null): string {
  const normalizedHostUrl = hostUrl.trim();
  const normalizedWorkspaceId = workspaceId?.trim() ?? "";
  const key = normalizedWorkspaceId
    ? `redrob::${normalizedHostUrl}::${normalizedWorkspaceId}`
    : `redrob::${normalizedHostUrl}`;
  return workspaceIdForKey(key);
}
  ...
  return workspaces.map((workspace) => {
    ...
    const id = workspace.id?.trim()
      || (workspaceType === "remote"
        ? remoteType === "redrob"
          ? workspaceIdForRedrob(workspace.redrobHostUrl ?? workspace.baseUrl ?? "", workspace.redrobWorkspaceId)
    ...
    return {
      ...
      redrobHostUrl: workspace.redrobHostUrl,
      redrobToken: workspace.redrobToken,
      redrobWorkspaceId: workspace.redrobWorkspaceId,
      redrobWorkspaceName: workspace.redrobWorkspaceName,
