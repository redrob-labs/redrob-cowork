...
import { redrobExtensionsPreviewPluginPath, redrobPluginPath } from "./redrob-extensions-plugin-path.js";

async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "redrob-workspace-init-"));
  try {
    await fn(root);
  } finally {
...
describe("ensureWorkspaceFiles", () => {
  test("does not write an redrob.json file (config is DB-backed now)", async () => {
    await withWorkspace(async (root) => {
      const result = await ensureWorkspaceFiles(root, "starter");
      // redrob config no longer lands on disk; it is seeded into the runtime
      // DB by the workspace-creation route.
      await expect(
        readFile(join(root, ".opencode", "redrob.json"), "utf8"),
      ).rejects.toThrow();
  ...
  test("defaultWorkspaceRedrobConfig carries authorizedRoots + workspace metadata", async () => {
    await withWorkspace(async (root) => {
      const config = defaultWorkspaceRedrobConfig(root, "starter");
  ...
  test("uses shipped extension preview plugin", async () => {
    const pluginPath = redrobExtensionsPreviewPluginPath();
    const plugin = await readFile(pluginPath, "utf8");
    expect(pluginPath).toContain(join("opencode-plugins", "redrob-extensions-preview.ts"));
    expect(plugin).toContain("redrob_execute");
  });
  ...
  test("does not create workspace extension preview plugin", async () => {
    await withWorkspace(async (root) => {
      ...
      await expect(stat(join(root, ".opencode", "plugins", "redrob-extensions-preview.ts"))).rejects.toThrow();
    });
  });
...
`;
  ...
  test("does not repair or inject into desktop-created schema-only opencode config", async () => {
    await withWorkspace(async (root) => {
      ...
      await writeFile(join(root, ".opencode", "redrob.json"), "{}\n", "utf8");
      const configPath = join(root, "opencode.jsonc");
...
describe("ensureLocalWorkspaceFiles", () => {
  ...
  test("provisions local workspaces and skips remote ones", async () => {
    await withWorkspace(async (root) => {
      ...
      // No redrob.json file is written; provisioning does not crash on the
