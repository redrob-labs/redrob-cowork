...
const VAULT_AAD = Buffer.from("redrob-local-managed-mcp-v1", "utf8");
...
async function recoverVaultLocked(config: ServerConfig, file: VaultFileState): Promise<LoadedVault> {
  const path = vaultPath(config);
  const backupName = `${basename(path)}.redrob-backup-${backupTimestamp(new Date())}`;
...
export async function handleLocalManagedMcpGateway(
  ...
  const server = new Server(
    { name: `redrob-local-${name}`, version: "1.0.0" },
    { capabilities: { tools: {} } },
  );
