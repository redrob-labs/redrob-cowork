...
type PersistedConnectState = {
  ...
  updatedAt: number;
  /**
   * Server-scoped Redrob Work Connect (`redrob-cloud`) MCP desired config.
...
export function googleWorkspaceConnectGuidance(cloudHealthOrReady: CloudMcpHealth | boolean | null): string {
  ...
  if (usable) {
    return "Google Workspace is available through the Redrob Work Cloud connection: call search_capabilities to find the capability, then execute_capability to run it. Do not tell the user to reconfigure extensions; the relevant settings surface is Settings > Connect.";
  }
  if (cloudHealthOrReady && typeof cloudHealthOrReady !== "boolean" && cloudHealthOrReady.desired.present) {
    ...
    return `Google Workspace is connected through Redrob Work Connect, but agent access needs attention for this workspace. Direct the user to Settings > Connect.${suffix}`;
  }
...
/** Read the host-level redrob-cloud MCP config used for Connect catalog/skills. */
...
/** Persist the host-level redrob-cloud MCP config (server-scoped Connect). */
...
async function resolveCloudHealth(config: ServerConfig, options: ConnectSnapshotOptions): Promise<{ cloudHealth: CloudMcpHealth | null; workspace: ConnectSnapshot["workspace"] }> {
  ...
  const cloudHealth = await readRedrobCloudMcpHealth({
    config,
...
async function inspectConnectRuntime(
  ...
  for (const workspace of config.workspaces) {
    ...
    if (Object.hasOwn(runtimeMcpMap(inspection.config), REDROB_CLOUD_MCP_NAME)) {
