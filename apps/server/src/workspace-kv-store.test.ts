...
async function tempWorkspace(): Promise<{ root: string; dbPath: string; config: ServerConfig }> {
  const root = await mkdtemp(join(tmpdir(), "redrob-workspace-kv-store-"));
  roots.push(root);
...
function corruptStoreJson(dbPath: string): void {
  ...
  try {
    ...
    sqlite.query("UPDATE redrob_workspace_configs SET config_json = ? WHERE workspace_id = ?").run("{", WORKSPACE_ID);
...
describe("workspace kv store", () => {
  ...
  test("returns each migrated store's documented defaults for missing and malformed rows", async () => {
    ...
    expect(await readRuntimeOpencodeConfig(config, WORKSPACE_ID)).toEqual({});
    expect(await readRedrobWorkspaceConfig(config, WORKSPACE_ID)).toEqual({});
    ...
    await writeRedrobWorkspaceConfig(config, WORKSPACE_ID, () => ({ workspace: { name: "Runtime" } }));
