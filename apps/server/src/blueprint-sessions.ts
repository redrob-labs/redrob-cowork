...
export function normalizeBlueprintSessionTemplates(redrob: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(redrob?.blueprint);
...
export function readMaterializedBlueprintSessions(redrob: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(redrob?.blueprint);
...
export function sanitizeRedrobTemplateConfig(redrob: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(redrob ?? {});
...
export function applyMaterializedBlueprintSessions(
  redrob: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeRedrobTemplateConfig(redrob);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
