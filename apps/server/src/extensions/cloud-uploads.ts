...
export const REDROB_CLOUD_UPLOADS_EXTENSION_ID = "redrob-cloud-uploads";
...
export const REDROB_CLOUD_UPLOAD_ACTIONS = [
  {
    ...
    description: "Uploads a workspace file up to 4 MiB directly to Google Drive outside model context. Redrob Work preserves the file bytes, basename, and source MIME type; it does not convert Office files.",
    inputSchema: {
      type: "object",
      properties: {
...
async function cloudUploadEndpoint(config: ServerConfig, suffix: string, dependencies: CloudUploadDependencies) {
  ...
  if (!endpoint || !authorization) {
    throw new ApiError(409, "cloud_not_connected", "Redrob Work Cloud must be connected before uploading files.");
  }
  let url: URL;
  try {
    url = new URL(endpoint);
  } catch {
    throw new ApiError(409, "cloud_endpoint_invalid", "The configured Redrob Work Cloud endpoint is invalid.");
  }
  const mcpSuffix = "/mcp/agent";
  if (!url.pathname.replace(/\/+$/, "").endsWith(mcpSuffix)) {
    throw new ApiError(409, "cloud_endpoint_invalid", "The configured Redrob Work Cloud endpoint must end in /mcp/agent.");
  }
...
async function postDirectUpload(
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status || 502, "cloud_upload_failed", `Redrob Work Cloud could not upload the file: ${message}`);
  }
  return payload;
}
...
export async function callRedrobWorkCloudUploadAction(
  config: ServerConfig,
  action: string,
  args: Record<string, unknown>,
  context: Record<string, unknown>,
