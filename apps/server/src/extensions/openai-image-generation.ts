...
export const OPENAI_IMAGE_GENERATION_EXTENSION_ACTIONS = [
  {
    ...
    description: "Check whether OpenAI image generation is configured and ready for Redrob Cowork extension actions.",
...
async function generateOpenAiImageArtifact(config: ServerConfig, env: EnvService, args: Record<string, unknown>, context: Record<string, unknown>) {
  ...
  const apiKey = await resolveOpenAiImageApiKey(env);
  if (!apiKey) {
    throw new ApiError(400, "openai_api_key_missing", "OpenAI API key missing. Save OPENAI_API_KEY in Redrob Cowork Environment Variables or configure the OpenAI Image Gen extension.");
  }
