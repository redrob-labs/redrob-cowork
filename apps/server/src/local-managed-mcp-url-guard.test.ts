...
const previousPrivateOverride = process.env.REDROB_ALLOW_PRIVATE_MCP_URLS;

afterEach(() => {
  if (previousDevMode === undefined) delete process.env.REDROB_DEV_MODE;
  else process.env.REDROB_DEV_MODE = previousDevMode;
  if (previousPrivateOverride === undefined) delete process.env.REDROB_ALLOW_PRIVATE_MCP_URLS;
  else process.env.REDROB_ALLOW_PRIVATE_MCP_URLS = previousPrivateOverride;
});
