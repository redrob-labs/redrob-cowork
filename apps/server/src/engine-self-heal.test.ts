...
const previousSpawnInterval = process.env.REDROB_ENGINE_MIN_SPAWN_INTERVAL_MS;

afterEach(() => {
  if (previousSpawnInterval === undefined) delete process.env.REDROB_ENGINE_MIN_SPAWN_INTERVAL_MS;
  else process.env.REDROB_ENGINE_MIN_SPAWN_INTERVAL_MS = previousSpawnInterval;
});
...
describe("managed engine self-heal", () => {
  ...
  test("requires three consecutive connection failures and throttles later bursts", async () => {
    process.env.REDROB_ENGINE_MIN_SPAWN_INTERVAL_MS = "30000";
