...
describe("desktop cloud sync", () => {
  test("queues provider update and remove changes", () => {
    ...
    const result = syncDesktopCloudResources({ now: 1780442400000, redrob, snapshot: baseSnapshot });
    ...
    const state = readDesktopCloudSyncState(result.redrob);
  ...
  test("syncs large provider snapshots within an interactive budget", () => {
    ...
    const redrob = { cloudImports: { providers: importedProviders } };

    const start = performance.now();
    const result = syncDesktopCloudResources({ now: 1780442400000, redrob, snapshot });
