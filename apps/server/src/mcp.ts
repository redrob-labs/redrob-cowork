...
function diagnosticToolIdsForMcp(name: string): string[] {
  return name === "redrob-cloud" ? REDROB_CLOUD_DIAGNOSTIC_TOOL_IDS : [];
}
...
function isMcpDisabledByTools(config: Record<string, unknown>, name: string): boolean {
  ...
  return deniedToolIds([config], "", [`${sanitizedName}___redrob_mcp_probe__`]).length > 0;
}
 ...
 * static config merge and Redrob Cowork's later dynamic MCP registration can make
