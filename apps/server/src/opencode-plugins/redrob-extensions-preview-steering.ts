...
export type RedrobWorkExtensionConnectState = {
  connectEnabled: boolean;
  connectCatalogEnabled: boolean;
  cloudMcpPresent: boolean;
  cloudHealth: RedrobWorkCloudHealthSummary | null;
  workspace?: {
    resolution?: string;
    id?: string | null;
...
export type RedrobWorkCloudHealthSummary = {
  usable: boolean;
...
type RedrobWorkFetch = (url: string, init?: RequestInit) => Promise<Response>;
...
export const REDROB_CLOUD_SKILL_AUTHORING_INSTRUCTION =
  "Skill creation: Cloud. When the user asks to create a skill, retrieve and follow the listed create-skill remote skill by calling redrob-cloud_execute_capability with its exact <capability>. Create the skill in Redrob Work Cloud as a private plugin, not in the workspace. For later steps, use share-plugin when the user wants a specific person or team to use a skill, and use add-to-marketplace or add-user-to-marketplace only when the user asks. Use a workspace-local skill only when the user explicitly requests one. Do not create both copies.";
...
export const REDROB_CLOUD_CONNECTION_INSTRUCTION =
  "The Redrob Work Cloud connection is verified ready for this exact workspace/model. For org-connected services, use redrob-cloud_search_capabilities with 2-4 keyword variants, then redrob-cloud_execute_capability with an exact returned name — and only mention services that search (or available_skills) actually returns. When a search result has kind mcp_app, execute that exact capability normally so Redrob Work can render its originating standard MCP App; do not import it, ask for a standalone HTML URL, or require a generated direct-tool name. When a remote skill is listed under available_skills, call redrob-cloud_execute_capability with its <capability> directly; do not treat the local OpenCode skill list as the full inventory. Local Redrob Work extensions remain available through redrob_query/redrob_execute with extension.actions and extension.call. Settings > Extensions is the member inventory surface for org and local apps. A successful search proves Redrob Work Cloud itself is authorized, so a downstream connector failure does not mean Redrob Work Cloud needs to be reconnected. If a result has kind connection_status, execute that exact capability once so Redrob Work renders an actionable connection card, then name connectionStatus.connectionName and relay connectionStatus.action exactly: use Your Connections for the member, the organization Connections dashboard for an org admin, or the provider admin console for a provider-side failure. After the requested human fixes that connector, search again in the same task because results are live, not cached, so unchanged retries return the same error.";

export const REDROB_CONNECT_SIGN_IN_INSTRUCTION =
  `${REDROB_EXTENSION_DISCOVERY_INSTRUCTION} Redrob Work Cloud is not signed in or no desired agent access configuration exists for this workspace. Direct the user to sign in to Redrob Work and connect the service in Settings → Connect.`;

export const REDROB_CONNECT_DISABLED_INSTRUCTION =
  `${REDROB_EXTENSION_DISCOVERY_INSTRUCTION} Redrob Work Cloud agent access is explicitly disabled for this workspace. Explain that the user can enable agent access in Settings → Connect.`;
...
async function fetchEngineMcpStatus(input: unknown, engine: RedrobWorkEngineMcpStatusSource): Promise<EngineMcpStatusResult> {
...
async function fetchRedrobWorkConnectState(input: unknown, fetcher: RedrobWorkFetch): Promise<RedrobWorkExtensionConnectState> {
  ...
  if (!response.ok) throw new Error(errorMessage(payload, "Redrob Work connect state request failed"));
...
export function resetRedrobWorkExtensionDiscoveryInstructionCacheForTests(): void {
...
export async function resolveRedrobWorkExtensionDiscoveryInstruction(
  input?: unknown,
  fetcher: RedrobWorkFetch = fetch,
  engine: RedrobWorkEngineMcpStatusSource = {},
): Promise<string> {
  if (engine.client) {
    try {
  ...
  try {
    return composeRedrobWorkExtensionDiscoveryInstruction(await fetchRedrobWorkConnectState(input, fetcher));
  } catch {
    return REDROB_EXTENSION_DISCOVERY_INSTRUCTION;
  }
}
