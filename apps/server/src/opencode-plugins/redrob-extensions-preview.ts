...
const redrobAffordanceRequestSchema = z.object({
  id: z.string().trim().min(1).describe("Semantic affordance id from redrob_context."),
  args: z.record(z.string(), z.unknown()).optional().describe("JSON arguments for the affordance."),
  expectedRevision: z.number().int().nonnegative().optional().describe("Context revision from redrob_context. Use for commands to prevent stale writes."),
...
type RedrobWorkWorkspace = z.infer<typeof workspaceSchema>;
...
type CreatedRedrobWorkSessionResult = {
  ok: true;
  sessionId: string;
  title: string;
  started: boolean;
...
type FailedRedrobWorkSessionResult = {
  ok: false;
  title: string;
  error: string;
};
...
function preserveMcpResult(output: unknown): void {
  ...
  output.metadata = {
    ...
    redrobMcpApp: appResult,
  };
}

const affordanceReadEffects: RedrobAffordanceEffects = { data: "read", ui: "none", external: false };
const affordanceWriteEffects: RedrobAffordanceEffects = { data: "write", ui: "none", external: false };
const affordanceExternalWriteEffects: RedrobAffordanceEffects = { data: "write", ui: "none", external: true };
// A proposal writes nothing anywhere: it is rendered for a person to act on.
const affordanceProposalEffects: RedrobAffordanceEffects = { data: "none", ui: "none", external: false };
...
type RedrobWorkEngineMcpStatusFunction = RedrobWorkEngineMcpStatusClient["mcp"]["status"];

function isRedrobWorkEngineMcpStatusFunction(value: unknown): value is RedrobWorkEngineMcpStatusFunction {
  return typeof value === "function";
}

function readEngineMcpStatusClient(value: unknown): RedrobWorkEngineMcpStatusClient | undefined {
  ...
  if (!isRedrobWorkEngineMcpStatusFunction(status)) return undefined;
...
function uiControlDiscoveryPaths(): string[] {
  return [
    ...
    join(userAppDataDir(), "io.redrob.work", "redrob-ui-control.json"),
    join(userAppDataDir(), "io.redrob.work.dev", "redrob-ui-control.json"),
...
async function readRedrobAgentContext(
...
async function queryRedrobAffordance(rawArgs: unknown): Promise<unknown> {
  const request = redrobAffordanceRequestSchema.parse(rawArgs);
  if (request.id === "session.search") {
    return affordanceResult(
      request.id,
      await searchRedrobWorkSessions(request.args ?? {}),
  ...
  if (request.id === "session.read") {
    return affordanceResult(
      request.id,
      await readRedrobWorkSession(request.args ?? {}),
      affordanceReadEffects,
    );
  }
  ...
  if (request.id.startsWith("connect.")) {
    return unavailableAffordance(
      request.id,
      "This affordance declares a dedicated Connect executor. Call the tool named in redrob_context.",
    );
  }
...
async function executeRedrobAffordance(
  ...
  if (request.id === "session.create") {
    return affordanceResult(
      request.id,
      await createRedrobWorkSessions(request.args ?? {}, context),
...
function workspaceLabel(workspace: RedrobWorkWorkspace): string {
...
function titleSearchResult(workspace: RedrobWorkWorkspace, session: SessionInfo, queryLower: string): SessionSearchResult | null {
...
function messageSearchResult(workspace: RedrobWorkWorkspace, session: SessionInfo, messages: SessionMessage[], queryLower: string): SessionSearchResult | null {
...
async function listRedrobWorkWorkspaces(): Promise<RedrobWorkWorkspace[]> {
  return workspaceListEnvelopeSchema.parse(await serverGet("/workspaces")).items;
}

function filterWorkspaces(workspaces: RedrobWorkWorkspace[], workspaceId?: string): RedrobWorkWorkspace[] {
...
async function listWorkspaceSessions(workspace: RedrobWorkWorkspace, limit: number): Promise<SessionInfo[]> {
...
async function readWorkspaceSession(workspace: RedrobWorkWorkspace, sessionId: string): Promise<SessionInfo> {
...
async function readSessionMessages(workspace: RedrobWorkWorkspace, sessionId: string, limit: number): Promise<SessionMessage[]> {
...
async function searchRedrobWorkSessions(rawArgs: unknown): Promise<object> {
...
async function readRedrobWorkSession(rawArgs: unknown): Promise<object> {
...
function requireRedrobWorkServer(): { url: string; token: string } {
  const url = serverUrl();
...
async function createRedrobWorkSessions(rawArgs: unknown, context: OpenCodeContext): Promise<object> {
  ...
  const results = await Promise.all(args.sessions.map(async (session): Promise<CreatedRedrobWorkSessionResult | FailedRedrobWorkSessionResult> => {
    try {
  ...
  const created = results.filter((result): result is CreatedRedrobWorkSessionResult => result.ok);
  const failures = results.filter((result): result is FailedRedrobWorkSessionResult => !result.ok);
  return {
...
export const RedrobWorkExtensionsPreview = async (factoryInput?: unknown) => {
  ...
  tool: {
    redrob_context: {
      ...
      async execute() {
        return JSON.stringify(
          await readRedrobAgentContext(engineMcpStatusClient, engineMcpStatusDirectory),
          null,
          2,
        );
      },
    ...
    redrob_execute: {
      ...
      async execute(rawArgs: unknown, context: OpenCodeContext) {
        ...
        return JSON.stringify(await executeRedrobAffordance(rawArgs, mergedContext), null, 2);
      },
    },
  },
  };
