...
import { RedrobWorkAnthropicToolSchema } from "./redrob-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await RedrobWorkAnthropicToolSchema();
...
describe("RedrobWorkAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./redrob-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["RedrobWorkAnthropicToolSchema"]);
  });
});
