...
import { RedrobWorkAnthropicAdaptiveThinking } from "./redrob-anthropic-adaptive-thinking.js";

async function runHook(apiId: string, options: Record<string, unknown>) {
  const hooks = await RedrobWorkAnthropicAdaptiveThinking();
  const output: { options: Record<string, unknown> } = { options };
  await hooks["chat.params"]({ model: { id: apiId, api: { id: apiId } } }, output);
  return output.options;
}

describe("RedrobWorkAnthropicAdaptiveThinking chat.params", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./redrob-anthropic-adaptive-thinking.js");
    expect(Object.keys(mod)).toEqual(["RedrobWorkAnthropicAdaptiveThinking"]);
  });
});
