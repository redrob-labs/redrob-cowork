...
import { redrobOfficeAttachmentsPluginPath } from "../redrob-extensions-plugin-path.js";
import { RedrobWorkOfficeAttachments } from "./redrob-office-attachments.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "redrob-office-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string, messages: unknown[]) {
  const plugin = await RedrobWorkOfficeAttachments({ directory: root });
...
describe("RedrobWorkOfficeAttachments", () => {
  ...
  test("rejects file URLs that resolve outside the workspace through a symlink", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "redrob-office-outside-"));
      try {
        const docx = docxFixture();
  ...
  test("uses factory directory and ignores Daytona non-git worktree root", async () => {
    await withWorkspace(async (root) => {
      const docx = docxFixture();
      const plugin = await RedrobWorkOfficeAttachments({ directory: root, worktree: "/" });
      ...
      expect(materialized).toContain(".opencode/redrob/inbox/chat-attachments/");
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    const runtime = await buildRedrobRuntimeConfigObject();
    const plugin = runtime.plugin;
    if (!Array.isArray(plugin)) throw new Error("Expected plugin list");
    expect(plugin).toContain(redrobOfficeAttachmentsPluginPath());
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./redrob-office-attachments.js");
    expect(Object.keys(mod)).toEqual(["RedrobWorkOfficeAttachments"]);
  });
});
