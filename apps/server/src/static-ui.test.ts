...
describe("serveStaticUi", () => {
  test("is a no-op when REDROB_WEB_ROOT is unset", async () => {
    await withWebRoot(null, async () => {
      const response = await serveStaticUi(new Request("http://redrob.test/"), staticConfig());
  ...
  test("rejects path traversal", async () => {
    ...
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://redrob.test/safe%2F..%2F..%2Fsecret.txt"), staticConfig());
  ...
  test("serves configured MIME types", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const js = await serveStaticUi(new Request("http://redrob.test/assets/app.123.js"), staticConfig());
      const css = await serveStaticUi(new Request("http://redrob.test/assets/style.123.css"), staticConfig());
      const json = await serveStaticUi(new Request("http://redrob.test/data.json"), staticConfig());
      const svg = await serveStaticUi(new Request("http://redrob.test/icon.svg"), staticConfig());
  ...
  test("uses immutable cache headers for assets", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://redrob.test/assets/app.123.js"), staticConfig());
  ...
  test("falls back to index.html for SPA routes", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const response = await serveStaticUi(new Request("http://redrob.test/settings/general"), staticConfig(""));
  ...
  test("serves overlay.html directly without using it as the fallback", async () => {
    const root = await createWebRoot();
    await withWebRoot(root, async () => {
      const overlay = await serveStaticUi(new Request("http://redrob.test/overlay.html"), staticConfig());
      const fallback = await serveStaticUi(new Request("http://redrob.test/missing-route"), staticConfig(""));
  ...
  test("injects escaped bootstrap JSON into index.html", async () => {
    ...
    await withBootstrapTokenEnv(null, async () => {
      await withWebRoot(root, async () => {
        const response = await serveStaticUi(new Request("http://redrob.test/"), staticConfig("tok<\u2028>\u2029&"));
  ...
  test("can disable bootstrap token injection", async () => {
    ...
    for (const value of ["0", "false"]) {
      await withBootstrapTokenEnv(value, async () => {
        await withWebRoot(root, async () => {
          const response = await serveStaticUi(new Request("http://redrob.test/"), staticConfig("client-token"));
          if (!response) throw new Error("expected index response");
          const body = await response.text();
