...
async function startRedrob(options?: { branch?: string }) {
  const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-claude-plugin-"));
...
describe("claude plugin bundles", () => {
  test("dryRun returns the Will-install preview with warnings", async () => {
    ...
    const response = await fetch(`${redrob.base}/workspace/ws_1/claude-plugins`, {
      method: "POST",
      headers: redrob.headers,
    ...
    expect(existsSync(join(redrob.workspaceRoot, ".opencode/skills/slack-plugin"))).toBe(false);
  });

  test("resolves branch names containing slashes in /tree/ URLs", async () => {
    const redrob = await startRedrob({ branch: "release/v1" });
  ...
  test("installs skills, commands, and MCP servers; uninstall cleans up", async () => {
    ...
    const installResponse = await fetch(`${redrob.base}/workspace/ws_1/claude-plugins`, {
      method: "POST",
      headers: redrob.headers,
    ...
    const skillPath = join(redrob.workspaceRoot, ".opencode/skills/slack-plugin/slack-search/SKILL.md");
    const commandPath = join(redrob.workspaceRoot, ".opencode/commands/slack-plugin/standup.md");
    ...
    const listResponse = await fetch(`${redrob.base}/workspace/ws_1/mcp`, { headers: redrob.headers });
    ...
    expect(redrob.engine.requests.some((entry) => entry.method === "POST" && entry.pathname === "/mcp")).toBe(true);

    // Uninstall through the shared cloud-plugins route.
    const removeResponse = await fetch(
      `${redrob.base}/workspace/ws_1/cloud-plugins/${encodeURIComponent(installBody.item.pluginId)}`,
      { method: "DELETE", headers: redrob.headers },
    );
    expect(removeResponse.status).toBe(200);
    expect(existsSync(skillPath)).toBe(false);
    expect(existsSync(commandPath)).toBe(false);
    const afterRemove = await fetch(`${redrob.base}/workspace/ws_1/mcp`, { headers: redrob.headers });
