...
import {
  buildRedrobRuntimeConfig,
  keepRedrobRuntimeConfigFileFresh,
  redrobRuntimeConfigFilePath,
  writeRedrobRuntimeConfigFile,
} from "./redrob-runtime-config.js";
...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), "redrob-runtime-config-file-"));
  roots.push(root);
...
async function readConfigFile(config: ServerConfig): Promise<Record<string, unknown>> {
  const raw = await readFile(redrobRuntimeConfigFilePath(config), "utf8");
  return JSON.parse(raw) as Record<string, unknown>;
}

describe("redrob runtime config file", () => {
  test("writes runtime-DB MCPs and redrob defaults into the file", async () => {
    const { config } = await setup();
    await writeRuntimeOpencodeConfig(config, "ws_1", (current) => ({
      ...current,
      mcp: {
        posthog: { type: "remote", url: "https://mcp.posthog.com/mcp", enabled: true },
        "redrob-connect-stale": { type: "remote", url: "https://cloud.example/stale", enabled: true },
      },
    }));
    ...
    expect(mcp["redrob-connect-stale"]).toBeUndefined();
    expect(parsed.default_agent).toBe("redrob");
  ...
  test("keepRedrobRuntimeConfigFileFresh rewrites the file on runtime-DB writes", async () => {
    const { config } = await setup();
    await writeRedrobRuntimeConfigFile(config, "ws_1");
    cleanups.push(keepRedrobRuntimeConfigFileFresh(config, "ws_1"));
  ...
  test("builds byte-stable config for repeated snapshots", async () => {
    ...
    const first = await buildRedrobRuntimeConfig(config, "ws_1");
    const second = await buildRedrobRuntimeConfig(config, "ws_1");
