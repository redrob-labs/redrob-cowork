...
export type RedrobWorkConnectSkill = z.infer<typeof skillIndexSchema>["skills"][number];
const catalogCache = new Map<string, { expiresAt: number; value: Promise<RedrobWorkConnectSkill[] | null> }>();

/**
 * Read the standards-shaped skill index through one redrob-cloud config.
 ...
 */
export async function readMcpSkillIndex(config: Record<string, unknown>, fetcher: McpFetch): Promise<RedrobWorkConnectSkill[] | null> {
  ...
  const initialized = await mcpPost(fetcher, url, baseHeaders, {
    ...
    params: {
      capabilities: {},
      clientInfo: { name: "redrob-server-skill-catalog", version: "1.0.0" },
...
async function readIndexCached(cloud: Record<string, unknown>, fetcher: McpFetch): Promise<RedrobWorkConnectSkill[] | null> {
...
/**
 * Resolve the skill catalog from the first *working* redrob-cloud config.
 ...
 */
export async function readRedrobWorkConnectSkillCatalog(
  config: ServerConfig,
  fetcher: McpFetch = externalFetch,
): Promise<RedrobWorkConnectSkill[]> {
  try {
...
export function resetRedrobWorkConnectSkillCatalogCacheForTests(): void {
  catalogCache.clear();
}
...
function logInjectedMarketplaceSkills(skills: InjectedMarketplaceSkill[]): void {
  ...
  console.log("[redrob:skills] marketplace skills injected into prompt", {
    count: skills.length,
    skills,
  });
}

export function renderRedrobWorkConnectSkillInstruction(skills: RedrobWorkConnectSkill[]): string {
  if (skills.length === 0) {
  ...
  const lines = [
    "Remote Agent Skills are available from Redrob Work Connect. The catalog below contains discovery metadata only.",
    ...
    "When a task matches a remote skill description, call redrob-cloud_execute_capability with the exact value from that skill's <capability> field as { name: <capability> }. Read the returned full SKILL.md body before following it. Do not call redrob-cloud_search_capabilities first when the exact capability is already listed here.",
