/**
 * Side-effect proof for the redrob.json -> runtime.sqlite migration.
 *
 ...
 *   - workspace creation writes the redrob config to the runtime DB, NOT to
 *     `.opencode/redrob.json`.
 *   - GET /config still returns the same redrob payload shape (version,
 *     workspace, authorizedRoots).
 *   - a legacy on-disk `.opencode/redrob.json` (pre-migration install) is
...
async function withSandbox(fn: (input: { root: string; config: ServerConfig }) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "redrob-config-migration-"));
...
describe("redrob config DB migration", () => {
  test("creating a workspace seeds the runtime DB and writes no redrob.json", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      try {
        ...
        const create = await fetch(`http://127.0.0.1:${server.port}/workspaces/local`, {
          method: "POST",
          headers: { "x-redrob-host-token": config.hostToken, "content-type": "application/json" },
        ...
        expect(await fileExists(join(workspacePath, ".opencode", "redrob.json"))).toBe(false);
        ...
        const stored = await readRedrobWorkspaceConfig(config, workspaceId);
        ...
        const body = (await configRes.json()) as { redrob: Record<string, unknown> };
        expect(body.redrob.version).toBe(1);
        expect(body.redrob.authorizedRoots).toEqual([resolve(workspacePath)]);
      } finally {
  ...
  test("back-compat: a legacy redrob.json is migrated into the DB on read", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      await writeFile(
        join(workspacePath, ".opencode", "redrob.json"),
        JSON.stringify({
          version: 1,
      ...
      try {
        ...
        expect(Object.keys(await readRedrobWorkspaceConfig(legacyConfig, workspaceId)).length).toBe(0);
        ...
        expect((body.redrob.workspace as { name?: string } | undefined)?.name).toBe("Legacy");
        expect((body.redrob.blueprint as { emptyState?: { title?: string } } | undefined)?.emptyState?.title)
          .toBe("Legacy starter");

        // Side effect: the legacy contents now live in the DB.
        const stored = await readRedrobWorkspaceConfig(legacyConfig, workspaceId);
  ...
  test("existing local workspace without legacy file gets default DB config on read", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      try {
        expect(await fileExists(join(workspacePath, ".opencode", "redrob.json"))).toBe(false);
        expect(Object.keys(await readRedrobWorkspaceConfig(existingConfig, workspaceId)).length).toBe(0);
        ...
        const body = (await configRes.json()) as { redrob: Record<string, unknown> };
        expect(body.redrob.version).toBe(1);
        expect(body.redrob.authorizedRoots).toEqual([workspacePath]);
        expect((body.redrob.workspace as { preset?: string } | undefined)?.preset).toBe("starter");

        const stored = await readRedrobWorkspaceConfig(existingConfig, workspaceId);
        expect(stored.authorizedRoots).toEqual([workspacePath]);
      } finally {
        await server.stop(true);
      }
