...
import { redrobServerDataDir } from "@redrob/paths";
import type { AuditEntry } from "./types.js";
import { ensureDir, exists } from "./utils.js";

function resolveRedrobDataDir(): string {
  return redrobServerDataDir();
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveRedrobDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "redrob", "audit.jsonl");
}
