...
async function connectGateway(runtimeConfig: Record<string, unknown>): Promise<Client> {
  ...
  const client = new Client({ name: "redrob-managed-mcp-test", version: "1.0.0" }, { capabilities: {} });
...
describe("Redrob Cowork-managed local MCP OAuth gateway", () => {
  ...
  test("rolls back a new managed connection when the initial OAuth handshake fails", async () => {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-rollback-"));
    roots.push(workspaceRoot);
    ...
    try {
      ...
      const created = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/managed`, {
        method: "POST",
      ...
      const status = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/unreachable-oauth/managed`,
        { headers: clientHeaders(config.token) },
      );
  ...
  test("returns safe connection errors for DCR reconnect and callback initialize failures", async () => {
    const previousRuntimeDb = process.env.REDROB_RUNTIME_DB;
    const previousDevMode = process.env.REDROB_DEV_MODE;
    const previousTelemetry = globalThis.__redrobDesktopTelemetry;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-handshake-errors-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.REDROB_DEV_MODE = "1";
    ...
    try {
      ...
      const reconnect = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/registration-failure/managed/connect`,
      ...
      const reconnectStatus = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/registration-failure/managed`,
        { headers: clientHeaders(config.token) },
      );
      ...
      const started = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/initialize-failure/managed/connect`,
        { method: "POST", headers: clientHeaders(config.token) },
      );
      ...
      const callbackStatus = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/initialize-failure/managed`,
        { headers: clientHeaders(config.token) },
      );
      ...
      const internalFailure = await fetch(
        `${redrobBaseUrl}/workspace/ws_managed/mcp/unexpected-sdk-failure/managed/connect`,
        { method: "POST", headers: clientHeaders(config.token) },
      );
      ...
      globalThis.__redrobDesktopTelemetry = previousTelemetry;
      if (previousRuntimeDb === undefined) delete process.env.REDROB_RUNTIME_DB;
      else process.env.REDROB_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.REDROB_DEV_MODE;
      else process.env.REDROB_DEV_MODE = previousDevMode;
  ...
  test("returns actionable input errors without persisting managed connections", async () => {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-input-errors-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    ...
    try {
      ...
      for (const input of cases) {
        ...
        const list = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp`, {
          headers: clientHeaders(config.token),
        });
        expect(list.status).toBe(200);
        expect(JSON.stringify(await list.json())).not.toContain(`"name":"${input.name}"`);
        const status = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/${input.name}/managed`, {
          headers: clientHeaders(config.token),
        });
        expect(status.status).toBe(404);
      }
  ...
  test("owns OAuth, exposes provider tools to OpenCode, refreshes, survives restart, and disconnects", async () => {
    const previousRuntimeDb = process.env.REDROB_RUNTIME_DB;
    const previousDevMode = process.env.REDROB_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.REDROB_DEV_MODE = "1";
    ...
    try {
      ...
      const redrobPort = await freePort();
      ...
      expect(callbackUrl).toStartWith(`${redrobBaseUrl}/mcp/oauth/callback`);
      const callback = await fetch(callbackUrl!);
      expect(callback.status).toBe(200);

      const status = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(config.token),
      });
      ...
      expect(String(firstRuntimeConfig?.url)).toStartWith(`${redrobBaseUrl}/mcp/managed/ws_managed/mock-oauth`);
      ...
      const revokedStatus = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
      ...
      const disconnected = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/auth`, {
        method: "DELETE",
        headers: clientHeaders(restartedConfig.token),
      });
      ...
      const reconnect = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed/connect`, {
        method: "POST",
        headers: clientHeaders(restartedConfig.token),
      });
      expect(reconnect.status).toBe(200);
      expect(await reconnect.json()).toMatchObject({ status: "needs_auth" });
      const reconnectStatus = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(restartedConfig.token),
      });
      ...
      expect((await readdir(runtimeStorageDir(restartedConfig)))
        .some((entry) => entry.includes(".redrob-backup-"))).toBe(false);
    } finally {
      if (previousRuntimeDb === undefined) delete process.env.REDROB_RUNTIME_DB;
      else process.env.REDROB_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.REDROB_DEV_MODE;
  ...
  test("quarantines and rebuilds the vault after a secure-storage key change, then reconnects", async () => {
    const previousRuntimeDb = process.env.REDROB_RUNTIME_DB;
    const previousDevMode = process.env.REDROB_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-rotation-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.REDROB_DEV_MODE = "1";
    ...
    try {
      ...
      const backups = (await readdir(storageDir))
        .filter((entry) => entry.startsWith("local-managed-mcp-vault.json.redrob-backup-"));
      ...
      const afterDiscoveryFailure = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
      ...
      const reconnected = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      expect(await reconnected.json()).toMatchObject({ status: "connected", hasCredential: true, enabled: true });
      const second = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp/mock-oauth-b/managed`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      ...
      const workspaceMcp = await fetch(`${redrobBaseUrl}/workspace/ws_managed/mcp`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      ...
      expect((await readdir(storageDir))
        .filter((entry) => entry.startsWith("local-managed-mcp-vault.json.redrob-backup-"))).toHaveLength(1);
    } finally {
      if (previousRuntimeDb === undefined) delete process.env.REDROB_RUNTIME_DB;
      else process.env.REDROB_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.REDROB_DEV_MODE;
  ...
  test("serves the plaintext index read-only while secure storage is unavailable", async () => {
    const previousRuntimeDb = process.env.REDROB_RUNTIME_DB;
    const previousDevMode = process.env.REDROB_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-unavailable-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.REDROB_DEV_MODE = "1";
  ...
  test("quarantines a legacy v1 vault it cannot decrypt and prunes orphaned gateway runtime entries", async () => {
    const previousRuntimeDb = process.env.REDROB_RUNTIME_DB;
    const previousDevMode = process.env.REDROB_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "redrob-local-managed-mcp-v1-"));
    roots.push(workspaceRoot);
    process.env.REDROB_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.REDROB_DEV_MODE = "1";
    ...
    try {
      ...
      cipher.setAAD(Buffer.from("redrob-local-managed-mcp-v1", "utf8"));
